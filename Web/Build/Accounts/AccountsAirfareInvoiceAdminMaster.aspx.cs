...
public partial class AccountsAirfareInvoiceAdminMaster : PhoenixBasePage
{
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["ID"] != null)
                {
                    ...
                    ifMoreInfo.Attributes["src"] = "../Accounts/AccountsAirfareInvoiceAdmin.aspx?ID=" + ViewState["ID"];
                }
            }
            PhoenixToolbar toolbargrid = new PhoenixToolbar();
            toolbargrid.AddImageButton("../Accounts/AccountsAirfareInvoiceAdminMaster.aspx", "Export to Excel", "icon_xls.png", "Excel");
            toolbargrid.AddImageLink("javascript:CallPrint('gvInvoice')", "Print Grid", "icon_print.png", "PRINT");
            toolbargrid.AddImageButton("../Accounts/AccountsAirfareInvoiceAdminFilter.aspx", "Find", "search.png", "FIND");
            toolbargrid.AddImageButton("../Accounts/AccountsAirfareInvoiceAdminMaster.aspx", "Clear Filter", "clear-filter.png", "CLEAR");
    ...
    private void BindData()
    {
        ...
        //if (hiddenkey != null && General.GetNullableInteger(hiddenkey.Value) != null)
        //{
        //    ViewState["ID"] = General.GetNullableInteger(hiddenkey.Value);
        //}

        int ioutputPageNumber = 0;
        NameValueCollection nvc = Filter.CurrentAirfareInvoiceAdminFilter;

        ds = PhoenixAccountsAirfareInvoiceAdmin.AirfareAdminSearch(nvc != null ? General.GetNullableString(nvc.Get("txtrequisitionno").ToString()) : null
        ...
        string[] alCaptions = { "File NO", "Passenger Name", "Departure Date", "Origin", "Destination", "Account Code", "Invoice Cancelled", "Invoice Status", "Charging Status" };
        ...
        General.SetPrintOptions("gvInvoice", "Airfare Invoice Admin", alCaptions, alColumns, ds);
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            if (ViewState["ID"] == null)
            {
                ...
                //gvInvoice.MasterTableView.Items[0].Selected = true;
                // gvInvoice.SelectedIndex = 0;
            }

            if (ViewState["PAGEURL"] == null)
            {
                ifMoreInfo.Attributes["src"] = "../Accounts/AccountsAirfareInvoiceAdmin.aspx?ID=" + ViewState["ID"].ToString();
            }
            SetRowSelection();
        }
        else
        {
            if (ViewState["PAGEURL"] == null)
            {
                ifMoreInfo.Attributes["src"] = "../Accounts/AccountsAirfareInvoiceAdmin.aspx?ID=";
            }
    ...
    protected void ShowExcel()
    {
        ...
        Response.AddHeader("Content-Disposition", "attachment; filename=AirfareInvoiceAdmin.xls");

        Response.ContentType = "application/vnd.msexcel";
        Response.Write("<TABLE BORDER='0' CELLPADDING='2' CELLSPACING='0' width='100%'>");
        Response.Write("<tr>");
        Response.Write("<td><img src='http://" + Request.Url.Authority + Session["images"] + "/esmlogo4_small.png" + "' /></td>");
        Response.Write("<td><h3>Airfare Invoice Admin</h3></td>");
    ...
    protected void MenuInvoiceMain_TabStripCommand(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                //ifMoreInfo.Attributes["src"] = "../Accounts/AccountsAirfareInvoiceAdmin.aspx?CREDITNOTEID=" + ViewState["CREDITNOTEID"].ToString() ;
            }
            else if (CommandName.ToUpper().Equals("ATTACHMENTS") && ViewState["ID"] != null && ViewState["ID"].ToString() != string.Empty)
            {
