...
public partial class AccountsCreditNoteVoucherLineItem : PhoenixBasePage
{
    ...
    protected void VoucherLineEdit(Guid gLineId)
    {
        ...
        if (txtBudgetId.Text != "" && txtAccountUsage.Text == "Vessel")
        {
            int iIncludeSOA = 0;
            int iPrincipalId = 0;
            DataSet ds1 = PhoenixAccountsVoucher.OwnerBudgetIncludeSOA(txtBudgetCode.Text, General.GetNullableInteger(txtAccountId.Text), ref iIncludeSOA, ref iPrincipalId);
            if (iIncludeSOA == 0)
            ...
            if ((iPrincipalId.ToString() != null) && iPrincipalId.ToString() != string.Empty)
                imgShowAccount1.Enabled = true;
            else
                imgShowAccount1.Enabled = false;
        }
