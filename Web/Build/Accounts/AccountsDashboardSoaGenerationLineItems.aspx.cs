...
public partial class Accounts_AccountsDashboardSoaGenerationLineItems : PhoenixBasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbarlineitem.AddButton("Un-Verify", "UNVERIFY", ToolBarDirection.Right);
    ...
    protected void MenuGenralSub_TabStripCommand(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                Response.Redirect("../Accounts/AccountsDashboardSOAGenerationLineitemupdate.aspx?debitnotereference=" + ViewState["debitnotereference"] + "&accountid=" + ViewState["accountid"] + "&Ownerid=" + ViewState["Ownerid"] + 
                    "&voucherdetailid=" + ViewState["FLDVOUCHERDETAILID"] + "&VesselId=" + null + "&debitnoteid=" + ViewState["debitnotereferenceid"] + "&SOAStatus=" + ViewState["SOAStatus"]+ "&status=" + ViewState["status"].ToString() +
                    "&type=" + ViewState["type"].ToString() + "&month=" + ViewState["month"].ToString() + "&year=" + ViewState["year"].ToString() + "&Condition=1", true);
            }
            else if (CommandName.ToUpper().Equals("REPORT"))
            {
                Response.Redirect("../Accounts/AccountsSoaGenerationReport.aspx?debitnoteid=" + ViewState["debitnotereferenceid"], true);
            }
        }
        catch (Exception ex)
        {
    ...
    protected void MenuGeneral_TabStripCommand(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                Response.Redirect("../Accounts/AccountsDashboardDebitNoteReferenceHistory.aspx?DebitNoteReferenceid=" + ViewState["debitnotereferenceid"] + "&qfrom=SOAGeneration"+"&status=" + ViewState["status"].ToString() + "&type=" + ViewState["type"].ToString() + "&month=" + ViewState["month"].ToString() + "&year=" + ViewState["year"].ToString() + "&Condition=1");
            }
        }
        catch (Exception ex)
        {
        ...
        try
        {
            ...
            else if (CommandName.ToUpper().Equals("ALLVERIFIED"))
            {
                ...
                if (drnxtrow != null)
                {
                    ...
                    {
                        Filter.CurrentOwnerBudgetCodeSelection = ds.Tables[0].Rows[0]["FLDOWNERBUDGETCODE"].ToString(); //drnxtrow[0]["FLDOWNERBUDGETCODE"].ToString();
                    }
                }

                ifMoreInfo.Attributes["src"] = "../Accounts/AccountsSOAGenerationLineItemDetailsSupport.aspx?debitnotereference=" + ViewState["debitnotereference"] + "&accountid=" + ViewState["accountid"] + "&Ownerid=" + ViewState["Ownerid"] + "&dtkey=" + ViewState["DtKey"] + "&voucherdtkey=" + ViewState["VoucherDtKey"] + "&debitnoteid=" + ViewState["debitnotereferenceid"];
            ...
            else if (CommandName.ToUpper().Equals("UNVERIFY"))
            {
                if (ViewState["FLDVOUCHERDETAILID"] != null)
                    PhoenixAccountsSOAGeneration.SOAGenerationLineItemUnVerifying(PhoenixSecurityContext.CurrentSecurityContext.UserCode, new Guid(ViewState["FLDVOUCHERDETAILID"].ToString()));
                Rebind();
            }
        }
        catch (Exception ex)
    ...
    private void BindData()
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            ViewState["SOAStatus"] = ds.Tables[0].Rows[0]["FLDSOASTATUS"].ToString();
            ...
            if (dsno.Tables[0].Rows.Count > 0 && ViewState["SOAStatus"] != null)
            {
                if (ViewState["SOAStatus"].ToString() == "Pending")
                {
                    ...
                    lnkNumber.Attributes.Add("onclick", "javascript:openNewWindow('NAFA','','" + Session["sitepath"] + "/Accounts/AccountsSoaCheckingVoucherUpdate.aspx?ACCOUNTCODE=" + ViewState["accountCode"].ToString() + "&OWNERBUDGETCODE=" + ViewState["ownerbudgetcode"].ToString() + "&debitnotereferenceid=" + ViewState["debitnotereferenceid"].ToString() + "'); return false;");
                }
            }
            else
   ...
   protected void gvOwnersAccount_ItemDataBound(object sender, GridItemEventArgs e)

    {
        ...
        if (e.Item is GridDataItem)
        {
            ...
            ImageButton cmdUntag = (ImageButton)e.Item.FindControl("cmdUntag");
            ...
            if (drv != null && drv["FLDVERFIED"].ToString() == "1")
            {
                ...
                if (imbverified != null)
                {
                    ...
                    imbverified.ToolTip = "Verified By: " + drv["FLDNAME"].ToString() + "\nVerified Date: " + drv["FLDVERFIEDDATE"].ToString();
                }
            }
            ...
            if (e.Item is GridDataItem)
            {
                ...
                if (drv["FLDISATTACHMENT"].ToString() == "1")
                {
                    if (cmdAttachment != null)
                    {
                        if (lblVoucherLineItemId != null && General.GetNullableGuid(lblVoucherLineItemId.Text) != null)
                        {
                            ...
                            + lblDtKey.Text + "&qvoucherlineid=" + lblVoucherLineItemId.Text + "&voucherdetailid=" + lblVoucherDetailId.Text + "&SOAStatus=" + drv["FLDSOASTATUS"] + "');return true;");
                        }
                        else
                        {
                            cmdAttachment.Attributes.Add("onclick", "parent.openNewWindow('codehelp','','" + Session["sitepath"] + "/Accounts/AccountsVoucherAttachments.aspx?dtkey="
                            + lblDtKey.Text + "&voucherdetailid=" + lblVoucherDetailId.Text + "&SOAStatus=" + drv["FLDSOASTATUS"] + "');return true;");
                        }
                ...
                if (drv["FLDSOASTATUS"].ToString() != "Pending")
                {
                    cmdEdit.Visible = false;
                    cmdUntag.Visible = false;
                }
                if (cmdEdit != null)
                    if (!SessionUtil.CanAccess(this.ViewState, cmdEdit.CommandName)) cmdEdit.Visible = false;

                if (cmdUntag != null)
                    if (!SessionUtil.CanAccess(this.ViewState, cmdEdit.CommandName)) cmdUntag.Visible = false;

                //ImageButton cmdDetails = (ImageButton)e.Row.FindControl("cmdDetails");
                //if (cmdDetails != null)
                //{
                //    cmdDetails.Visible = true;
                //    cmdDetails.Attributes.Add("onclick", "parent.Openpopup('codehelp','','AccountsSoaGenerationLineItemDetails.aspx?debitnotereference=" + ViewState["debitnotereference"] + "&accountid=" + ViewState["accountid"] + "&Ownerid=" + ViewState["Ownerid"]
    ...
    protected void gvOwnersAccount_ItemCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName.ToUpper().Equals("UNTAG"))
            {
                GridDataItem item = (GridDataItem)e.Item;
                int nCurrentRow = e.Item.ItemIndex;
                RadLabel lbl = ((RadLabel)gvOwnersAccount.Items[nCurrentRow].FindControl("lblVoucherId"));
                PhoenixAccountsSOAGeneration.SOAGenerationLineItemUntag(PhoenixSecurityContext.CurrentSecurityContext.UserCode, new Guid(lbl.Text));
                Rebind();
            }
            ...
            else if (e.CommandName.ToUpper().Equals("SAVE"))
            {
                ...
                int nCurrentRow = e.Item.ItemIndex;
                RadLabel lbl = ((RadLabel)gvOwnersAccount.Items[nCurrentRow].FindControl("lblVoucherId"));
                int chkYN = (((CheckBox)gvOwnersAccount.Items[nCurrentRow].FindControl("chkIncludeEdit")).Checked) ? 1 : 0;
                PhoenixAccountsSOAGeneration.SOAGenerationERMVoucherDetailUpdate(PhoenixSecurityContext.CurrentSecurityContext.UserCode, new Guid(lbl.Text), chkYN);
