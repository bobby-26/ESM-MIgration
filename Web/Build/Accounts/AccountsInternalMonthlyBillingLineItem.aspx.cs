...
public partial class AccountsInternalMonthlyBillingLineItem : PhoenixBasePage
{
    ...
    private void BindBillingItemData()
    {
        ...
        gvMonthlyBillingItem.VirtualItemCount = iRowCount;
        ...
        if (e.CommandName.ToUpper().Equals("DELETE"))
        {
            try
            {
                ...
                if (ViewState["PORTAGEBILLID"] != null && ViewState["PORTAGEBILLID"].ToString() != "" && ViewState["MONTHLYBILLINGEMPLOYEEID"] != null && ViewState["MONTHLYBILLINGEMPLOYEEID"].ToString() != "")
                {
                    ...
                    RadWindowManager1.RadConfirm("Do you want to delete this Crew " + ViewState["EMPLOYEENAME"] + " from the all the Billing Items?", "ucConfirmDelete", 320, 150, null, "Delete");
                    return;
                }
                RebindCrew();
            }
    ...
    protected void gvMonthlyBillingCrew_SortCommand(object sender, GridSortCommandEventArgs e)
    {
    ...
    protected void gvMonthlyBillingItem_ItemDataBound(object sender, GridItemEventArgs e)
    {
        if (e.Item is GridDataItem)
        {
            ...
            if (cmdPost != null)
            {
                ...
                if (!SessionUtil.CanAccess(this.ViewState, cmdRepost.CommandName)) cmdRepost.Visible = false;
    ...
    protected void gvMonthlyBillingItem_ItemCommand(object sender, GridCommandEventArgs e)
    {
        ...
        if (e.CommandName.ToUpper().Equals("POST"))
        {
            try
            {
                if (ViewState["PORTAGEBILLID"] != null && ViewState["PORTAGEBILLID"].ToString() != "")
                {
                    ...
                    RadWindowManager1.RadConfirm("Do you want to post the voucher for this Billing Item?", "PostRecord", 320, 150, null, "Post");
                    return;
                }
        ...
        if (e.CommandName.ToUpper().Equals("REPOST"))
        {
            try
            {
                ...
                PhoenixAccountsInternalBilling.VoucherRePost(PhoenixSecurityContext.CurrentSecurityContext.UserCode, new Guid(lblMonthlyBillingItemId.Text));
            }
            catch (Exception ex)
            {
                ucError.ErrorMessage = ex.Message;
    ...
    protected void gvMonthlyBillingItem_SortCommand(object sender, GridSortCommandEventArgs e)
    {
        ViewState["SORTEXPRESSION"] = e.SortExpression;
        switch (e.NewSortOrder)
        {
    ...
    protected void ucConfirmDelete_Click(object sender, EventArgs e)
    {
        try
        {
            PhoenixAccountsInternalBilling.EmployeeDelete(
                                                   PhoenixSecurityContext.CurrentSecurityContext.UserCode
                                                  , new Guid(ViewState["PORTAGEBILLID"].ToString())
    ...
    private void FinancialYearCheck(string date, string companyid, int postall, string budgetbillingid, ref int periodlockedyn)
    {
        ...
        if (periodlockedyn == 2)
        {
            String scriptinsert = String.Format("javascript:openNewWindow('codehelp1','','" + Session["sitepath"] + "/Accounts/AccountsInternalMonthlyBillingLineItemGeneral.aspx?portagebillid=" + ViewState["PORTAGEBILLID"] + "&budgetbillingid=" + General.GetNullableGuid(budgetbillingid) + "&postall=" + postall + "',null);");
