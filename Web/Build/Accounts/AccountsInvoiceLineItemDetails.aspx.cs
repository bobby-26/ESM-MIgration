...
public partial class AccountsInvoiceLineItemDetails : PhoenixBasePage
{
    ...
    protected void MenuLineItem_TabStripCommand(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                Response.Redirect("../Accounts/AccountsInvoiceLineItemDetails.aspx?qinvoicecode=" + ViewState["invoicecode"].ToString() + "&qcallfrom=" + ViewState["callfrom"]);
            }
            ...
            {
                Response.Redirect("../Accounts/AccountsInvoiceHistory.aspx?qinvoicecode=" + ViewState["invoicecode"].ToString() + "&qfrom=" + ViewState["callfrom"]);
            }
            if (CommandName.ToUpper().Equals("QUERIES") && ViewState["invoicecode"] != null && ViewState["invoicecode"].ToString() != string.Empty)
            {
                Response.Redirect("../Accounts/AccountsInvoiceQueryMaster.aspx?qinvoicecode=" +ViewState["invoicecode"].ToString() + "&qcallfrom=" + ViewState["callfrom"], false);
            }
        }
        catch (Exception ex)
        {
    ...
    protected void gvLineItem_ItemDataBound(Object sender, GridItemEventArgs e)
    {
        ...
        if (e.Item is GridDataItem)
        {
            ...
            ImageButton cmdshippedquantity = (ImageButton)e.Item.FindControl("cmdshippedquantity");
            if (cmdshippedquantity != null)
            {
                if (drv["FLDORDERFORMTYPE"].ToString() == "1" && SessionUtil.CanAccess(this.ViewState, cmdshippedquantity.CommandName))
                    cmdshippedquantity.Visible = true;

                cmdshippedquantity.Attributes.Add("onclick", "openNewWindow('detail', '', 'Purchase/PurchaseShippedQuantity.aspx?orderid=" + drv["FLDORDERID"] + "&VesselId=" + drv["FLDVESSELID"] + "&StockType=" + drv["FLDSTOCKTYPE"] + "&Callfrom=ACCOUNTS" + "'); return false;");
            }
            }
