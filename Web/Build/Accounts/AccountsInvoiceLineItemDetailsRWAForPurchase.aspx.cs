...
public partial class AccountsInvoiceLineItemDetailsRWAForPurchase : PhoenixBasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbargrid.AddImageButton("../Accounts/AccountsInvoiceLineItemDetailsRWAForPurchase.aspx", "Export to Excel", "icon_xls.png", "Excel");
            ...
            if (ViewState["SHORTNAME"] != null && ViewState["SHORTNAME"].ToString() != "AVN")
                ...
                if (ViewState["BULKPOINVOICE"] != null && ViewState["BULKPOINVOICE"].ToString() == "1")
                    toolbargrid.AddImageButton("../Accounts/AccountsInvoiceLineItemDetailsRWAForPurchase.aspx", "Bulk PO Approval", "approve.png", "BULKPOAPPROVAL");
    ...
    protected void InvoiceEdit()
    {
        if (ViewState["invoicecode"] != null)
        {
            ...
            if (dsInvoice.Tables.Count > 0)
            {
                ...
                ViewState["BULKPOINVOICE"] = drInvoice["FLDISBULKPOINVOICE"].ToString();
    ...
    protected void MenuLineItem_TabStripCommand(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                Response.Redirect("../Accounts/AccountsInvoiceLineItemDetailsRWAForPurchase.aspx?qinvoicecode=" + ViewState["invoicecode"].ToString() + "&qcallfrom=" + ViewState["callfrom"]);
            }
            else if (CommandName.ToUpper().Equals("INVOICE"))
            {
                ...
                {
                    Response.Redirect("../Accounts/AccountsInvoiceMasterRWAForPurchase.aspx?qinvoicecode=" + ViewState["invoicecode"].ToString());
                }
            }

            else if (CommandName.ToUpper().Equals("ATTACHMENTS"))
            {
                Response.Redirect("../Accounts/AccountsInvoiceAttachmentsRWAForPurchase.aspx?qinvoicecode=" + ViewState["invoicecode"].ToString() + "&qfrom=" + ViewState["callfrom"]);
            }
            else if (CommandName.ToUpper().Equals("HISTORY"))
            {
                Response.Redirect("../Accounts/AccountsInvoiceHistoryRWAForPurchase.aspx?qinvoicecode=" + ViewState["invoicecode"].ToString() + "&qfrom=" + ViewState["callfrom"]);
            }
        }
        catch (Exception ex)
        {
    ...
    protected void MenuOrderLineItem_TabStripCommand(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (CommandName.ToUpper().Equals("BULKPOAPPROVAL"))
            {
                PhoenixAccountsPOStaging.POStagingBulkPOApproval(new Guid(ViewState["invoicecode"].ToString()), PhoenixSecurityContext.CurrentSecurityContext.UserCode);
                 Rebind();
            }
