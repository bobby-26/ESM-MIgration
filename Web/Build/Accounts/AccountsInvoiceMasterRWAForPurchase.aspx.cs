...
public partial class AccountsInvoiceMasterRWAForPurchase : PhoenixBasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbargrid.AddImageButton("../Accounts/AccountsInvoiceMasterRWAForPurchase.aspx", "Export to Excel", "icon_xls.png", "Excel");
            ...
            toolbargrid.AddImageButton("../Accounts/AccountsInvoiceBasicFilterRWAForPurchase.aspx?qcalfrom=INVOICEFORPURCHASE&qcallfrom=inv", "Find", "search.png", "FIND");
            toolbargrid.AddImageButton("../Accounts/AccountsInvoiceMasterRWAForPurchase.aspx", "Clear Filter", "clear-filter.png", "CLEAR");
            ...
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["qinvoicecode"] != null)
                {
                    ...
                    ifMoreInfo.Attributes["src"] = "../Accounts/AccountsInvoiceRWAForPurchase.aspx?qinvoicecode=" + ViewState["invoicecode"];
                }
            }
    ...
    protected void MenuOrderFormMain_TabStripCommand(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                Response.Redirect("../Accounts/AccountsInvoiceLineItemDetailsRWAForPurchase.aspx?qinvoicecode=" + ViewState["invoicecode"] + "&qcallfrom=AD");
            }
            if (CommandName.ToUpper().Equals("ATTACHMENTS") && ViewState["invoicecode"] != null && ViewState["invoicecode"].ToString() != string.Empty)
            {
                Response.Redirect("../Accounts/AccountsInvoiceAttachmentsRWAForPurchase.aspx?qinvoicecode=" + ViewState["invoicecode"].ToString() + "&qfrom=AD");
            }
            if (CommandName.ToUpper().Equals("HISTORY") && ViewState["invoicecode"] != null && ViewState["invoicecode"].ToString() != string.Empty)
            {
                Response.Redirect("../Accounts/AccountsInvoiceHistoryRWAForPurchase.aspx?qinvoicecode=" + ViewState["invoicecode"].ToString() + "&qfrom=AD", false);
            }
            else
                MenuOrderFormMain.SelectedMenuIndex = 0;
        }
                                            ...
                                            , null
                                             ...
                                             , ref iRowCount, ref iTotalPageCount
                                              , nvc != null ? General.GetNullableInteger(nvc.Get("RadMcUserFM")) : null
                                               ...
                                               , nvc != null ? General.GetNullableInteger(nvc.Get("txtpicid")) : null);
    ...
    private void BindData()
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            {
                ifMoreInfo.Attributes["src"] = "../Accounts/AccountsInvoiceRWAForPurchase.aspx?qinvoicecode=" + ViewState["invoicecode"].ToString();
            }
            SetRowSelection();
        }
        else
        {
            if (ViewState["PAGEURL"] == null)
            {
                ifMoreInfo.Attributes["src"] = "../Accounts/AccountsInvoiceRWAForPurchase.aspx?voucherid=";
            }
        }
    }
    ...
    protected void gvInvoice_ItemDataBound(Object sender, GridItemEventArgs e)
    {
        if (e.Item is GridDataItem)
        {
            ...
            ImageButton cmdshippedqty = (ImageButton)e.Item.FindControl("cmdshippedqty");
            if (cmdshippedqty != null)
            {
                if (drv["FLDSHIPPEDDATE"].ToString() != "" && SessionUtil.CanAccess(this.ViewState, cmdshippedqty.CommandName))
                    cmdshippedqty.Visible = true;
            }

            ImageButton cmdapprovalhistoryremarks = (ImageButton)e.Item.FindControl("cmdapprovalhistoryremarks");
            ImageButton cmdPOApprove = (ImageButton)e.Item.FindControl("cmdPOApprove");
            if (cmdPOApprove != null)
            {
                if (drv["FLDREJECTEDYN"].ToString() == "1" && SessionUtil.CanAccess(this.ViewState, cmdPOApprove.CommandName))
                    cmdPOApprove.Visible = false;
            }

            ImageButton cmdMachingwithorginalpo = (ImageButton)e.Item.FindControl("cmdMachingwithorginalpo");
            if (cmdMachingwithorginalpo != null)
            {
                if (drv["FLDPOPAYABLEISORIGINALPOAMOUNT"].ToString() == "1" && SessionUtil.CanAccess(this.ViewState, cmdMachingwithorginalpo.CommandName))
                    cmdMachingwithorginalpo.Visible = true;
            }
            if (cmdapprovalhistoryremarks != null)
            {
                cmdapprovalhistoryremarks.Attributes.Add("onclick", "openNewWindow('codehelp1', '', '" + Session["sitepath"] + "/Accounts/AccountsRWAApprovalHistoryRemarks.aspx?orderID=" + strOrderId + "&Showall=1" + "',false,400,300); ");
                if (!SessionUtil.CanAccess(this.ViewState, cmdapprovalhistoryremarks.CommandName)) cmdapprovalhistoryremarks.Visible = true;
            }
            ImageButton cmdreapprovalremarks = (ImageButton)e.Item.FindControl("cmdreapprovalremarks");

            if (cmdreapprovalremarks != null)
            {
                cmdreapprovalremarks.Attributes.Add("onclick", "openNewWindow('codehelp1', '', '" + Session["sitepath"] + "/Accounts/AccountsRWAReApprovalHistoryRemarks.aspx?orderID=" + strOrderId + "&Showall=1" + "');return false;");
                if (drv["FLDREJECTEDYN"].ToString() == "1" && SessionUtil.CanAccess(this.ViewState, cmdreapprovalremarks.CommandName)) cmdreapprovalremarks.Visible = true;
            }

            ImageButton imbApprovalHistory = (ImageButton)e.Item.FindControl("imbApprovalHistory");

            if (imbApprovalHistory != null)
            {
                imbApprovalHistory.Attributes.Add("onclick", "openNewWindow('Approval', '', '" + Session["sitepath"] + "/Accounts/AccountsRWAApprovalHistory.aspx?orderID=" + strOrderId + "&Showall=1" + "');return false;");
                if (!SessionUtil.CanAccess(this.ViewState, imbApprovalHistory.CommandName)) imbApprovalHistory.Visible = true;
            }

            string dtkey = ((RadLabel)e.Item.FindControl("lbldtkey")).Text;
            HyperLink lnkInvoiceattachment = (HyperLink)e.Item.FindControl("lnkInvoiceattachment");
            ...
            if (dsattachment.Tables[0].Rows.Count > 0)
            {
                ...
                lnkInvoiceattachment.NavigateUrl = "../common/download.aspx?dtkey=" + drattachment["FLDDTKEY"].ToString();
            }
        }
    }
    ...
    protected void gvInvoice_ItemCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
            ...
            {
                Response.Redirect("../Accounts/AccountsInvoiceAttachmentsRWAForPurchase.aspx?qinvoicecode=" + ((RadTextBox)e.Item.FindControl("txtInvoiceCode")).Text + "&qfrom=invoiceforpurchase");
            }
