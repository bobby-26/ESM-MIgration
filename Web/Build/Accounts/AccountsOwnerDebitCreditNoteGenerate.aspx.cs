...
public partial class AccountsOwnerDebitCreditNoteGenerate : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                ddlSubtype.DataBind();
                ddlVessel.DataBind();
                ViewState["VesselAccountMapid"] = string.Empty;
                ViewState["PAGENUMBERLINE"] = 1;
                ViewState["VesselId"] = "";
                ViewState["PRINCIPAL"] = "";
                ViewState["BudgetCode"] = "Dummy";
                ViewState["Accountid"] = string.Empty;
    ...
    protected void MenuDebitCreditNote_TabStripCommand(object sender, EventArgs e)
    {
        ...
        try
        {
            if (CommandName.ToUpper().Equals("SAVE"))
            {
                ...
                if (!IsValidAccount(lblCAccounts.Text))
                {
                    ucError.Visible = true;
                    return;
                }
    ...
    public void OwnerDebitCreditNoteEdit()
    {
        try
        {
            if (ViewState["OwnerDebitCreditNoteId"] != null && ViewState["OwnerDebitCreditNoteId"].ToString() != string.Empty)
            {
                ...
                //ddlBank.DataSource = PhoenixRegistersAccount.ListBankAccount(null, null, General.GetNullableInteger(dr["FLDBILLTOCOMPANY"].ToString()));
                ddlBank.DataBind();
				...
				lblCAccounts.Text = dr["FLDACTIVEC"].ToString();
                txtaddressed.Text = dr["FLDOWNERFUNDADDRESS"].ToString();
                if (dr["FLDOWNERFUNDADDRESS"].ToString() == "")
                {
                    lbladdresses.Visible = false;
                    txtaddressed.Visible = false;
                    lbl1.Visible = false;
                    lbl2.Visible = false;
    ...
    private bool IsValidLineItem(string description, string amount)
    {
	 ...
	 private bool IsValidAccount(string companyaccount)
    {
        ucError.HeaderMessage = "Information";

        if (General.GetNullableInteger(companyaccount) != 1)
            ucError.ErrorMessage = "Account code is Inactive in Company.";
    ...
    protected void ddlVessel_Changed(object sender, EventArgs e)
    {
        ...
        if (ds1.Tables[0].Rows.Count > 0)
        {
            ...
            ViewState["Accountid"] = ds1.Tables[0].Rows[0]["FLDACCOUNTID"].ToString();
    ...
    public void Gettemplate(int accountid)
    {
        try
        {
            ...
            if (dstemplate.Tables[0].Rows.Count > 0)
            {
                ...
                lblCAccounts.Text = dstemplate.Tables[0].Rows[0]["FLDACTIVEC"].ToString();
                txtaddressed.Text = dstemplate.Tables[0].Rows[0]["FLDOWNERFUNDADDRESS"].ToString();
                ViewState["ADDRESSYN"] = dstemplate.Tables[0].Rows[0]["ADDRESSYN"].ToString();
				if (dstemplate.Tables[0].Rows[0]["FLDACTIVEYN"].ToString() != "0")
                {
				 ...
				 if (dstemplate.Tables[0].Rows[0]["ADDRESSYN"].ToString() != "1")
                {
                    lbladdresses.Visible = false;
                    txtaddressed.Visible = false;
                    lbl1.Visible = false;
