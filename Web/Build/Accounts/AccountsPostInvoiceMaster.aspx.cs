...
public partial class AccountsPostInvoiceMaster : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbargrid.AddImageButton("../Accounts/AccountsPostInvoiceMaster.aspx", "Export to Excel", "icon_xls.png", "Excel");
            toolbargrid.AddImageLink("javascript:CallPrint('gvInvoice')", "Print Grid", "icon_print.png", "PRINT");
            toolbargrid.AddImageButton("../Accounts/AccountsInvoiceBasicFilter.aspx?qcalfrom=POSTINVOICE", "Find", "search.png", "FIND");
            ...
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["qinvoicecode"] != null)
                {
                    ...
                    ifMoreInfo.Attributes["src"] = "../Accounts/AccountsPostInvoice.aspx?qinvoicecode=" + ViewState["invoicecode"];
                }
            }
    ...
    protected void MenuOrderFormMain_TabStripCommand(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                Response.Redirect("../Accounts/AccountsInvoiceAttachments.aspx?qinvoicecode=" + ViewState["invoicecode"].ToString() + "&qfrom=PR");
            }
            if (CommandName.ToUpper().Equals("HISTORY") && ViewState["invoicecode"] != null && ViewState["invoicecode"].ToString() != string.Empty)
            {
                Response.Redirect("../Accounts/AccountsInvoiceHistory.aspx?qinvoicecode=" + ViewState["invoicecode"].ToString() + "&qfrom=PR", false);
            }
            if (CommandName.ToUpper().Equals("QUERIES") && ViewState["invoicecode"] != null && ViewState["invoicecode"].ToString() != string.Empty)
            {
                Response.Redirect("../Accounts/AccountsInvoiceQueryMaster.aspx?qinvoicecode=" + ViewState["invoicecode"].ToString() + "&qcallfrom=PR", false);
            }
            else
                MenuOrderFormMain.SelectedMenuIndex = 0;
        }
    ...
    protected void ShowExcel()
    {
        ...
        int? iInvoiceStatus = 243;
        NameValueCollection nvc = Filter.CurrentInvoiceSelection;

        if (nvc != null && General.GetNullableInteger(nvc.Get("ucInvoiceStatus")) != null)
            iInvoiceStatus = General.GetNullableInteger(nvc.Get("ucInvoiceStatus"));
        ...
        {
            ds = PhoenixAccountsInvoice.InvoicePostDefaultSearch(
                                                      nvc != null ? General.GetNullableInteger(nvc.Get("ddlInvoiceType")) : null
    ...
    private void BindData()
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            {
                ifMoreInfo.Attributes["src"] = "../Accounts/AccountsPostInvoice.aspx?qinvoicecode=" + ViewState["invoicecode"].ToString();
            }
            SetRowSelection();
        }
        else
        {
            if (ViewState["PAGEURL"] == null)
            {
                ifMoreInfo.Attributes["src"] = "../Accounts/AccountsPostInvoice.aspx?voucherid=";
            }
        }
    }
    protected void ReconcileInvoiceWithPurchaseOrder(string strInvoiceCode)
    {
        try
        {
    ...
    protected void gvInvoice_ItemCommand(object sender, GridCommandEventArgs e)
    {
        ...
        else if (e.CommandName.ToUpper().Equals("CREATEPR"))
        {
            ReconcileInvoiceWithPurchaseOrder(((RadTextBox)e.Item.FindControl("txtInvoiceCode")).Text);
        }
        else if (e.CommandName.ToUpper().Equals("ATTACHMENT"))
        {
            Response.Redirect("../Accounts/AccountsInvoiceAttachments.aspx?qinvoicecode=" + ((RadTextBox)e.Item.FindControl("txtInvoiceCode")).Text + "&qfrom=PR");
        }
        //else if (e.CommandName.ToUpper().Equals("HISTORY"))
        //{
        //   Response.Redirect("../Accounts/AccountsInvoiceIncommingInvoiceList.aspx");
