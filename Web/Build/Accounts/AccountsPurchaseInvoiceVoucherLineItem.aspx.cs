...
public partial class AccountsPurchaseInvoiceVoucherLineItem : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        // txtAccountCode.Attributes.Add("onkeydown", "return false;");
        if (!IsPostBack)
        {
            Reset();
    ...
    {
        int? accountid = General.GetNullableInteger(txtAccountId.Text);
        int? budgetid = General.GetNullableInteger("");
        ...
        if (CommandName.ToUpper().Equals("SAVE"))
        {
            ...
            if (ViewState["VOUCHERLINEITEMCODE"] == null)
            {
                try
                {
                    ...
                    InsertVoucherLineItem(int.Parse(ViewState["VOUCHERID"].ToString()),
                                            ...
                                            , txtRemarks.Text
                                            , General.GetNullableInteger(ucProjectcode.SelectedValue)
                                           );
            ...
            else
            {
                try
                {
                    ...
                    //if (dr["FLDBALANCE"].ToString() == "0.00")
                    //{
    ...
    protected void VoucherLineEdit(Guid gLineId)
    {
        ...
        if (ds.Tables.Count > 0)
        {
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                txtUpdatedBy.Text = Convert.ToString(dr["FLDUPDATEDBYUSERNAME"]) + " On " + Convert.ToString(dr["FLDUPDATEDDATE"]);
    ...
    public bool IsValidLineItem()
    {
        ...
        if (General.GetNullableDecimal(txtExchangeRate.Text) == null)
            ucError.ErrorMessage = "Invalid Base ExchangeRate .";
        if (General.GetNullableDecimal(txt2ndExchangeRate.Text) == null)
            ucError.ErrorMessage = "Invalid Report ExchangeRate .";
        if (General.GetNullableDecimal(txtPrimeAmoutEdit.Text) == null)
            ucError.ErrorMessage = "Invalid Prime Amout .";
