...
public partial class AccountsReconcilationStaging : PhoenixBasePage
{
    PhoenixToolbar toolbarmain;
    PhoenixToolbar toolbar;
    PhoenixToolbar toolbarSubmit;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            toolbargrid.AddImageButton("../Accounts/AccountsReconcilationStaging.aspx", "Export to Excel", "icon_xls.png", "Excel");
            ...
            toolgrid.AddImageButton("../Accounts/AccountsReconcilationStaging.aspx", "Export to Excel", "icon_xls.png", "Excel");
            toolgrid.AddImageLink("javascript:CallPrint('gvTax')", "Print Grid", "icon_print.png", "PRINT");
            //toolgrid.AddImageLink("../Accounts/AccountsReconcilationStaging.aspx", "Add Gst", "add.png", "GST");
            ...
            if ((ViewState["INVOICESTATUS"].ToString() == "242" || ViewState["INVOICESTATUS"].ToString() == "632") && ViewState["POPIC"].ToString() == "")
            {

                toolbarSubmit = new PhoenixToolbar();
                toolbarSubmit.AddButton("Submit For Approval", "SUBMITFORAPPROVAL", ToolBarDirection.Right);
                MenuSubmitForApproval.AccessRights = this.ViewState;
                MenuSubmitForApproval.MenuList = toolbarSubmit.Show();
                MenuSubmitForApproval.Visible = true;
            }
    ...
    private void BindVendorInfo()
    {
        ...
        if (dsVendor.Tables[0].Rows.Count > 0)
        {
            ...
            txtPICName.Text = dr["FLDPOPICNAME"].ToString();
            txtpicid.Text = dr["FLDPOPIC"].ToString();
            ViewState["POPIC"] = dr["FLDPOPIC"].ToString();
    ...
    protected void ApproverHistory(object sender, ImageClickEventArgs arg)
    {
        String scriptpopup = String.Format("javascript:parent.openNewWindow('codehelp2','','" + Session["sitepath"] + "/Accounts/AccountsRWAApprovalHistory.aspx?orderID=" + ViewState["ORDERID"].ToString() + "');");
    ...
    protected void MenuSubmitForApproval_TabStripCommand(object sender, EventArgs e)
    {
        ...
        try
        {
            if (CommandName.ToUpper().Equals("SUBMITFORAPPROVAL"))
            {
                if (ViewState["ORDERID"] != null)
                {
                    PhoenixAccountsPOStaging.OrderFormStagingPOPicsubmitforapprovalUpdate(PhoenixSecurityContext.CurrentSecurityContext.UserCode
                                                                                    , new Guid(ViewState["ORDERID"].ToString())
                                                                                   );

                    BindVendorInfo();
                    if (MenuSubmitForApproval.Visible == true)
                    {
                        MenuSubmitForApproval.Visible = false;
                    }
    ...
    protected void MenuLineItem_TabStripCommand(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                Response.Redirect("../Accounts/AccountsInvoiceLineItemDetails.aspx?qinvoicecode=" + ViewState["invoicecode"] + "&qcallfrom=" + ViewState["callfrom"].ToString());
            }
        ...
        if (ViewState["ORDERID"] != null)
        {
            if (txtExchangeRateEdit.Text != string.Empty)
            {
                try
                {
                    ...
                    Response.Redirect("AccountsReconcilationStaging.aspx?ORDERID=" + ViewState["ORDERID"].ToString() + "&qinvoicecode=" + ViewState["invoicecode"].ToString() + "&qcallfrom=" + ViewState["callfrom"].ToString() + "&qfrom=" + ViewState["callfrom"].ToString());
                }
                catch (Exception ex)
                {
                    ucError.ErrorMessage = ex.Message;
    ...
    protected void gvTax_ItemDataBound(Object sender, GridItemEventArgs e)
    {
        try
        {
            ...
            if (e.Item is GridFooterItem)
            {
                ...
                {
                    btnShowOwnerBudget.Attributes.Add("onclick", "return showPickList('spnPickListOwnerBudget', 'codehelp1', '', '../Common/CommonPickListOwnerBudget.aspx?iframignore=true&Ownerid=" + ViewState["Ownerid"].ToString() + "&budgetdate=" + DateTime.Now.ToString() + "&budgetid=" + txtBudgetIdEdit.Text + "', true); ");          //+ "&budgetid=" + txtBudgetIdEdit.Text       
    ...
    protected void POPicUpdate(object sender, ImageClickEventArgs arg)
    {
        try
        {
            if (ViewState["ORDERID"] != null)
            {
                if (txtPICName.Text != string.Empty)
                {
                    PhoenixAccountsPOStaging.OrderFormStagingPOPicUpdate(PhoenixSecurityContext.CurrentSecurityContext.UserCode
                                                                                    , new Guid(ViewState["ORDERID"].ToString())
                                                                                    , General.GetNullableInteger(txtpicid.Text));
                    ucStatus.Text = "PIC Name updated Successfully.";
                }
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(),
                                 "BookMarkScript", "fnReloadList(null,'ifMoreInfo','keepopen');", true);
            }
