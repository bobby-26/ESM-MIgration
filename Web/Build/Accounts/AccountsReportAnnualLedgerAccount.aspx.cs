...
public partial class AccountsReportAnnualLedgerAccount : PhoenixBasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbarmain.AddButton("Ledger Account Detailed", "ANNUALLEDGERDETAILED", ToolBarDirection.Right);
    ...
    protected void MenuMainFilter_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                Response.Redirect("../Accounts/AccountsReportAnnualLedgerDetailed.aspx", false);
            }
        }
        catch (Exception ex)
        {
        ...
        try
        {
            ...
            if (CommandName.ToUpper().Equals("ANNUALEDGER"))
            {
                ...
                if (IsValidAccountFilter(ViewState["ACCOUNT"].ToString(), txtFromDate.Text, txtToDate.Text))
                {
                    ...
                    {
                        Response.Redirect("../Reports/ReportsView.aspx?applicationcode=5&reportcode=ANNUALEDGER&account=" + criteria.Get("cblAccount") + "&fromdate=" + criteria.Get("txtFromDate") + "&todate=" + criteria.Get("txtToDate") + "&fromAccount=" + criteria.Get("txtFromAccount") + "&toAccount=" + criteria.Get("txtToAccount"), false);
                    }
                    else if (ddlType.SelectedValue.ToString().Equals("2"))
                    {
                        Response.Redirect("../Reports/ReportsView.aspx?applicationcode=5&reportcode=ANNUALEDGERREPORTINGCURRENCY&account=" + criteria.Get("cblAccount") + "&fromdate=" + criteria.Get("txtFromDate") + "&todate=" + criteria.Get("txtToDate") + "&fromAccount=" + criteria.Get("txtFromAccount") + "&toAccount=" + criteria.Get("txtToAccount"), false);
                    }
