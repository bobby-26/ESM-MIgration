...
public partial class AccountsReportAnnualLedgerBaseCurrencyForPCL : System.Web.UI.Page
{
    ...
    protected void AnnualReport_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            if (dce.CommandName.ToUpper().Equals("ANNUALEDGER"))
            {
                ...
                {
                    Response.Redirect("../Reports/ReportsView.aspx?applicationcode=5&reportcode=ANNUALEDGERBASECURRENCYFORPCL&account=" + ViewState["ACCOUNT"].ToString() + "&fromdate=" + txtFromDate.Text + "&todate=" + txtToDate.Text, false);
                }
                else
                {
                    ucError.Visible = true;
