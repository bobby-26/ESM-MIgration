...
public partial class AccountsReportsSupplierSubsidairyLedgerPurchaseInvoice : PhoenixBasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbargrid.AddFontAwesomeButton("../Accounts/AccountsReportsSupplierSubsidairyLedgerPurchaseInvoice.aspx", "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "Excel");
            toolbargrid.AddFontAwesomeButton("javascript:CallPrint('gvSupplierLedger')", "Print Grid", "<i class=\"fas fa-print\"></i>", "PRINT");
            toolbargrid.AddFontAwesomeButton("../Accounts/AccountsReportsSupplierSubsidairyLedgerPurchaseInvoice.aspx", "Find", "<i class=\"fas fa-search\"></i>", "FIND");
            ...
            MenuSubsidiaryLedger.SelectedMenuIndex = 2;
    ...
    public void BindData()
    {
        try
        {
            string[] alColumns = { "FLDREFERENCEDOCUMENTNO", "FLDVESSELNAME", "FLDPURCHASEORDERNUMBER", "FLDCURRENCYCODE", "FLDACCOUNTNUMBER", "FLDINVOICEAMOUNT", "FLDHARDNAME" };
            string[] alCaptions = { "Reference Number", "Vessel Name", "PO Number", "Currency", "Bank Entry Number", "Prime Amount", "Status" };

            DataSet ds = new DataSet();
            if (txtSupplierId.Text != string.Empty)
            {
                ds = PhoenixAccountsSupplierSubsidiaryLedger.PurchaseInvoiceSubsidiaryLedger(General.GetNullableInteger(txtSupplierId.Text),
                                                                                                txtVoucherNumber.Text);
            }
            else
            {
                ds = PhoenixAccountsSupplierSubsidiaryLedger.PurchaseInvoiceSubsidiaryLedger(null, txtVoucherNumber.Text);
            }

            General.SetPrintOptions("gvSupplierLedger", "PurchaseInvoice", alCaptions, alColumns, ds);
    ...
    protected void ShowExcel()
    {
        ...
        Response.Write("<td><h3>Purchase Invoice</h3></td>");
