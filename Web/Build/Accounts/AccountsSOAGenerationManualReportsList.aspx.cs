...
public partial class AccountsSOAGenerationManualReportsList : PhoenixBasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["subreportcode"] != null)
            {
                ViewState["subreportcode"] = Request.QueryString["subreportcode"].ToString();
            }
            ...
            if (!string.IsNullOrEmpty(attachmentcode))
            {
                ...
                try
                {
                    ...
                    if (Request.QueryString["mod"].ToString().ToUpper() == "ACCOUNTS")
                    {
                        ...
                        Guid OutputGuid = new Guid();
                        if (ViewState["AllowedMIMEType"] != null)
                        {
                            OutputGuid = PhoenixCommonFileAttachment.InsertAttachment(Request.Files, new Guid(attachmentcode), module,
                                    null, ViewState["AllowedMIMEType"].ToString(), string.Empty, General.GetNullableString(type), vesselid);
                        }
                        else
                        {
                            OutputGuid = PhoenixCommonFileAttachment.InsertAttachment(Request.Files, new Guid(attachmentcode), module, null, string.Empty, string.Empty, General.GetNullableString(type), vesselid);
                        }

                        ScriptManager.RegisterClientScriptBlock(this, this.GetType(),
                                 "BookMarkScript", "fnReloadList(null,'ifMoreInfo','keepopen');", true);

                        PhoenixAccountsSOAGeneration.InsertSOAGenerationManualReportsVerification(new Guid(ViewState["debitnotereferenceid"].ToString()), new Guid(attachmentcode), OutputGuid, ViewState["subreportcode"].ToString());
    ...
    private void BindData()
    {
        ...
        if (attachmentcode != string.Empty)
        {
            ...
            //{
            ds = PhoenixAccountsSOAGeneration.SOAGenerationManualReportsAttachmentSearch(new Guid(attachmentcode), null, type, sortexpression, sortdirection,
        ...
        if (e.Item is GridDataItem)
        {
            if (!e.Item.Equals(DataControlRowState.Alternate | DataControlRowState.Edit) && !e.Item.Equals(DataControlRowState.Edit))
            {
                ...
                if (db != null)
                {
                    ...
                    ed.Visible = (module.ToString() == "ACCOUNTS" && Request.QueryString["Status"] == "Pending") ? true : false;
        ...
        try
        {
            if (Request.QueryString["adjustmentamount"] != null && Request.QueryString["adjustmentamount"] != string.Empty)
            {
                if (Convert.ToDouble(ViewState["adjustmentamount"]) == 0.00)
                {
                    ...
                    RadLabel lblFilePath = (RadLabel)e.Item.FindControl("lblFilePath");
                    string filepath = PhoenixGeneralSettings.CurrentGeneralSetting.AttachmentPath;
                    if (!filepath.EndsWith("/"))
                        filepath = filepath + "/";
                    String path = filepath + lblFilePath.Text;
            ...
            {
                string dtkey = ((RadLabel)e.Item.FindControl("lblDTKey")).Text;
                string reportverificationid = ((RadLabel)e.Item.FindControl("lblReportVericationID")).Text;
                if (General.GetNullableGuid(ViewState["debitnotereferenceid"].ToString()) != null)
                    ...
                    PhoenixAccountsSOAGeneration.DeleteSOAGenerationManualReportsVerification(new Guid(reportverificationid));
                PhoenixCommonFileAttachment.AttachmentDelete(new Guid(dtkey));
                RadLabel lblFilePath = (RadLabel)e.Item.FindControl("lblFilePath");
                string filepath = PhoenixGeneralSettings.CurrentGeneralSetting.AttachmentPath;
                if (!filepath.EndsWith("/"))
    ...
    protected void gvAttachment_ItemCommand(object sender, GridCommandEventArgs e)
    {
        ...
        try
        {
            if (e.CommandName.ToUpper().Equals("VIEW"))
            {
                ...
                string reportverificationid = "";
                string filepath = "";
                string dtkey = string.Empty;
                reportverificationid = ((RadLabel)e.Item.FindControl("lblReportVericationID")).Text;
                filepath = ((RadLabel)e.Item.FindControl("lblFilePath")).Text;
                dtkey = ((RadLabel)e.Item.FindControl("lblDTKey")).Text;
                if (reportverificationid != "" && filepath != "")
                    ifMoreInfo.Attributes["src"] = "../Accounts/AccountsSOAGenerationManualReportsDetails.aspx?reportverificationid=" + reportverificationid + "&dtkey=" + dtkey;

                //Session["sitepath"] + "/attachments/" +e.CommandArgument;
            }
