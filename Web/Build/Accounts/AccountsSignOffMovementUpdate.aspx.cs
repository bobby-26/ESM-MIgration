...
public partial class AccountsSignOffMovementUpdate : PhoenixBasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["SIGNOFFID"] != null && Request.QueryString["SIGNOFFID"].ToString() != string.Empty)
            {
                ViewState["SIGNONOFFID"] = Request.QueryString["SIGNOFFID"].ToString();
    ...
    {
        DataSet ds = PhoenixAccountsSignonoffConfirm.AccountssignoffmovementEdit(int.Parse(ViewState["SIGNONOFFID"].ToString()));
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            txtsignoffdate.Text = string.Format("{0:dd/MMM/yyyy}", dr["FLDSIGNOFFDATE"].ToString());
