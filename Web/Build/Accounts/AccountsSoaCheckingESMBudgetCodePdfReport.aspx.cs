...
public partial class Accounts_AccountsSoaCheckingESMBudgetCodePdfReport : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            string src, filepath;
            filepath = HttpContext.Current.Request.MapPath("~/Attachments/Accounts/" + ViewState["dtkey"].ToString() + "_ESM.pdf");
            src = Session["sitepath"] + "/attachments/Accounts/" + ViewState["dtkey"].ToString() + "_ESM.pdf";
    ...
    private void Bind()
    {
        ...
        string[] alColumns = { "FLDVOUCHERDATE", "FLDVOUCHERROW", "FLDDESCRIPTION", "FLDDEBIT", "FLDCREDIT", "FLDAMOUNT" };
                ...
                if (DtVL.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow DrVL in DtVL.Rows) // Loop over the rows.
                    {
                        ...
                        if (ViewState["VOUCHERDTKEY"].ToString() != DrVL["FLDVOUCHERDTKEY"].ToString())
                        {
                            ...
                            {
                                strFilePath = HttpContext.Current.Request.MapPath("~/Attachments/") + DrVLFA["FLDFILEPATH"].ToString();
        ...
        CombineAndSavePdf(HttpContext.Current.Request.MapPath("~/Attachments/Accounts/" + ViewState["dtkey"].ToString() + "_ESM.pdf"), lstPdf);
