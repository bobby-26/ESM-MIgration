...
public partial class AccountsStatementOfAccountsSubledger : PhoenixBasePage
{
    ...
    protected void MenuReports_TabStripCommand(object sender, EventArgs e)
    {
        ...
        try
        {
            if (CommandName.ToUpper().Equals("SHOW"))
            {
                ...
                Response.Redirect("../Reports/ReportsViewWithSubReport.aspx?accountcode=" + txtAccountId.Text + "&fromdate=" + ucfromDate.Text + "&todate=" + uctoDate.Text + "&fromsubaccount=" + txtFromAccount.Text + "&tosubaccount=" + txtToAccount.Text + "&applicationcode=5&reportcode=STATEMENTOFACCOUNTSUBLEDGER", false);
            }
        }
        catch (Exception ex)
        {
