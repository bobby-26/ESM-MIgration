...
public partial class AccountsSundryPurchaseRequirementItem : PhoenixBasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        MenuStockItem.Title = "Store Item List";
    ...
    private void BindData()
    {
        try
        {
            ...
            DataTable dt = PhoenixAccountsSundryPurchase.SearchSundryPurchaseRequirementStoreItem (new Guid(Request.QueryString["REQUIREMENTID"]),
                PhoenixSecurityContext.CurrentSecurityContext.VesselID,txtNumberSearch.Text, txtStockItemNameSearch.Text,null, 
                null, General.GetNullableInteger(ddlStockClass.SelectedHard), null,sortexpression, sortdirection,
    ...
    protected void gvStoreItem_ItemDataBound(object sender, GridItemEventArgs e)
    {
        try
        {
            ...
            {
                UserControlMaskNumber qty = (UserControlMaskNumber)e.Item.FindControl("txtQuantity");
                if (qty == null)
                    e.Item.Attributes["onclick"] = Page.ClientScript.GetPostBackEventReference(gvStoreItem, "Edit$" + e.Item.RowIndex.ToString(), false);
    ...
    protected void gvStoreItem_ItemCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
            ...
            {
              //  Guid id = Guid.Parse(((RadLabel)e.Item.FindControl("lblStoreitemid")).Text);
                string quantity = ((UserControlMaskNumber)e.Item.FindControl("txtQuantity")).Text;
             
                if (General.GetNullableDecimal(quantity).HasValue)
                {
                    PhoenixAccountsSundryPurchase.InsertSundryPurchaseRequirementLineItem(
                        new Guid(Request.QueryString["REQUIREMENTID"]),Guid.Parse (((RadLabel)e.Item.FindControl("lblStoreitemid")).Text).ToString(), quantity);
    ...
    //        {
    //            PhoenixAccountsSundryPurchase.InsertSundryPurchaseRequirementLineItem(
    //                new Guid(Request.QueryString["REQUIREMENTID"]), _gridView.DataKeys[nCurrentRow].Value.ToString(), quantity);
    //            string Script = "<script language='javaScript' id='BookMarkScript'>" + "\n";
    //            Script += "fnReloadList('codehelp1','ifMoreInfo','keepopen');";
    //            Script += "</script>" + "\n";
    //            Page.ClientScript.RegisterStartupScript(typeof(Page), "BookMarkScript", Script);              
    ...
    protected void gvStoreItem_SortCommand(object sender, GridSortCommandEventArgs e)
    {
