...
public partial class Accounts_AccountsTrialBalanceMonthlyReport : PhoenixBasePage
{
    ...
    protected void MenuReports_TabStripCommand(object sender, EventArgs e)
    {
        ...
        try
        {
            if (CommandName.ToUpper().Equals("SHOW"))
            {
                ...
                Response.Redirect("../Reports/ReportsViewWithSubReport.aspx?AccountId=" + txtAccountId.Text + "&fromdate=" + ucfromDate.Text + "&Todate=" + uctoDate.Text + "&applicationcode=5&reportcode=ERMTRIALBALANCEMONTHLY", false);
            }
        }
        catch (Exception ex)
        {
