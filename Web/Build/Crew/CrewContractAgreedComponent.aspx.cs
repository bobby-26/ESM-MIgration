...
public partial class CrewContractAgreedComponent : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbar1.AddImageButton("../Crew/CrewContractAgreedComponent.aspx?Contractid=" + Request.QueryString["Contractid"].ToString(), "Export to Excel", "icon_xls.png", "Excel");
    ...
    protected void Menuexport_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                string[] alColumns = { "FLDCOMPONENTNAME", "FLDCALCULATIONBASISNAME", "FLDPAYABLEBASISNAME", "FLDINCLUDEDONBOARDYNNAME", "FLDEARNINGDEDUCTIONNAME" };
                string[] alCaptions = { "Component", "Calculation Basis", "Payable Basis", "Included Onboard", "Earning /Deduction" };
                DataTable dt = PhoenixCrewManagement.CrewContractAgreedCompnent(new Guid(Request.QueryString["Contractid"].ToString()));
                General.ShowExcel("Crew Agreed", dt, alColumns, alCaptions, null, null);
            }
        }
        catch (Exception ex)
        {
    ...
    private void BindData()
    {
        ...
        General.SetPrintOptions("gvCrew", "Crew Agreed", alCaptions, alColumns, ds);
    }
    private void EditContractDetails()
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                ViewState["SENIORITYSCALEID"] = dt.Rows[0]["FLDSENIORITYSCALEID"].ToString();
                txtLastName.Text = dt.Rows[0]["FLDLASTNAME"].ToString();
                txtFirstName.Text = dt.Rows[0]["FLDFIRSTNAME"].ToString();
                txtMiddlename.Text = dt.Rows[0]["FLDMIDDLENAME"].ToString();
                txtRankMonth.Text = dt.Rows[0]["FLDRANKEXPERIENCE"].ToString();
