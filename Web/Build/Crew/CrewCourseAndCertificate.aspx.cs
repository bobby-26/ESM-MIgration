...
public partial class CrewCourseAndCertificate : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbar.AddFontAwesomeButton("../Crew/CrewCourseAndCertificate.aspx", "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "Excel");
            toolbar.AddFontAwesomeButton("javascript:CallPrint('gvCrewCourseCertificate')", "Print Grid", "<i class=\"fas fa-print\"></i>", "PRINT");
            toolbar.AddFontAwesomeButton("javascript:parent.openNewWindow('NAFA','','" + Session["sitepath"] + "/Crew/CrewCourseandCertificateArchived.aspx?empid=" + Filter.CurrentCrewSelection + "&type=1&t=p'); return false;", "Show Archived", "<i class=\"fas fa-archive\"></i>", "ARCHIVED");
            toolbar.AddFontAwesomeButton("../Crew/CrewCourseAndCertificate.aspx", "Email", "<i class=\"fas fa-envelope\"></i>", "CEMAIL");

            if (Filter.CurrentCrewLaunchedFrom != null && Filter.CurrentCrewLaunchedFrom.ToString() != "")
                toolbar.AddFontAwesomeButton("javascript:openNewWindow('chml','','" + Session["sitepath"] + "/CrewOffshore/CrewOffshoreCourseMissing.aspx?empid=" + Filter.CurrentCrewSelection + "&t=p'); return false;", "Initiate Course Request", "<i class=\"fas fa-book\"></i>", "COURSEREQUEST");
            else
                toolbar.AddFontAwesomeButton("javascript:openNewWindow('chml','','" + Session["sitepath"] + "/Crew/CrewCourseMissing.aspx?empid=" + Filter.CurrentCrewSelection + "&t=p'); return false;", "Initiate Course Request", "<i class=\"fas fa-book\"></i>", "COURSEREQUEST");
    ...
    {
        gvCrewCourseCertificate.SelectedIndexes.Clear();
        gvCrewCourseCertificate.EditIndexes.Clear();
        gvCrewCourseCertificate.DataSource = null;
    ...
    protected void CrewCourseCertificate_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            else if (CommandName.ToUpper().Equals("CEMAIL"))
            {
                ...
                scriptRefreshDontClose += "parent.openNewWindow('NAFA','','" + Session["sitepath"] + "/Crew/CrewCourseEmail.aspx?csvcourseid=" + stbcourseid.ToString() + "&empid=" + Filter.CurrentCrewSelection + "&course=1')";
                scriptRefreshDontClose += "</script>" + "\n";
                ScriptManager.RegisterClientScriptBlock(this, typeof(Page), "CrewCourseEmail", scriptRefreshDontClose, false);
            }
        }
        catch (Exception ex)
        {
    ...
    protected void gvCrewCourseCertificate_ItemCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
            ...
            {
                String scriptpopup = String.Format("javascript:parent.openNewWindow('codehelp1', '', '" + Session["sitepath"] + "/Common/CommonDiscussion.aspx?');");
