...
public partial class CrewEmployeeDocumentEmail : PhoenixBasePage
{
    ...
    protected void SendMail()
    {
        ...
        try
        {
            PhoenixMail.SendMail(txtTO.Text.Trim(), txtCC.Text.Trim(), null, txtSubject.Text.Trim(), edtBody.Content, true, (MailPriority)Convert.ToInt32(ddlPriority.SelectedValue.ToString()), ViewState["mailsessionid"].ToString());
        }
        catch (Exception ex)
        {
    ...
    private void DocumentRequestMail()
    {
        ...
        if (nvc == null)
        {
            ...
            nvc.Add("DocumentDtkey", string.Empty);
            nvc.Add("DocumentType", string.Empty);
            nvc.Add("EmpId", string.Empty);
        }

        string StrDocumentDtkeylist = General.GetNullableString(nvc != null ? nvc.Get("DocumentDtkey") : string.Empty);
        string StrDocumentTypeList = General.GetNullableString(nvc != null ? nvc.Get("DocumentType") : string.Empty);
        int? EmpId = General.GetNullableInteger(nvc != null ? nvc.Get("EmpId") : string.Empty);
        ...
        DataSet ds = PhoenixCrewEmployeeDocument.ListCrewDocumentEmail(StrDocumentDtkeylist, StrDocumentTypeList, EmpId);
        ViewState["UserName"] = "";
        ViewState["EmpName"] = "";
        ...
        {
            sbemailbody.AppendLine("Find the below documents link for the employee -  " + ds.Tables[0].Rows[0]["FLDEMPLOYEENAME"].ToString());
            sbemailbody.AppendLine("<br/>");
            sbemailbody.AppendLine("<br/>");
            sbemailbody.AppendLine("<br/>");
            sbemailbody.AppendLine("<table style='border: 1px solid black;border-collapse:collapse; font-size:12px' width=60%>");
            sbemailbody.AppendLine("<tr>");
            sbemailbody.AppendLine("<th style='border: 1px solid black'>Document Type</th>");
            sbemailbody.AppendLine("<th style='border: 1px solid black'>Document Name</th>");
            sbemailbody.AppendLine("<th style='border: 1px solid black'>Download</th>");
            sbemailbody.AppendLine("</tr>");
            
            foreach (DataRow dt1 in ds.Tables[0].Rows)
            {
                sbemailbody.AppendLine("<tr>");
                sbemailbody.AppendLine("<td style='border: 1px solid black'>" + dt1["FLDDOCUMENTTYPE"].ToString()+"</td>");
                sbemailbody.AppendLine("<td style='border: 1px solid black'>" + dt1["FLDFILENAME"].ToString() + "</td>");
                sbemailbody.AppendLine("<td style='border: 1px solid black'>" + "<a href =" + Session["sitepath"] + "/Options/OptionDownload.aspx?dtkey=" + dt1["FLDDTKEY"].ToString() + ">Link to Download</a>" + "</td>");
                sbemailbody.AppendLine("</tr>");             
            }
        ...
        DataTable dtEmail = PhoenixCrewEmployeeDocument.CrewDocumentEmailUser(EmpId);

        if (dtEmail.Rows.Count > 0)
        {
            DataRow dr = dtEmail.Rows[0];

            if (rblEmailTo.SelectedValue == "1")
            {
                txtTO.Text = dr["FLDUSEREMAIL"].ToString().Replace(";", ",").TrimEnd(',');
                txtCC.Text = "";
            }
            else
            {
                txtTO.Text = dr["FLDEMPLOYEEEMAIL"].ToString().Replace(";", ",").TrimEnd(',');
                txtCC.Text = dr["FLDUSEREMAIL"].ToString().Replace(";", ",").TrimEnd(',');
            }

            txtSubject.Text = dr["FLDEMPLOYEENAME"].ToString() + " - Document List";

            ViewState["EmpName"] = dr["FLDEMPLOYEENAME"].ToString();
            ViewState["UserName"] = dr["FLDUSERNAME"].ToString();

        }

        // pdform as an attachment 

        if (ViewState["pdform"] != null)
        {
            string Tmpfilelocation = string.Empty; string[] reportfile = new string[0];

            DataSet dspdform = PhoenixCrewReports.PdFormDetails(EmpId, PhoenixSecurityContext.CurrentSecurityContext.UserCode);

            if (dspdform.Tables[0].Rows.Count > 0 && PhoenixGeneralSettings.CurrentGeneralSetting.ReportType == "1")
            {
                NameValueCollection nvcReport = new NameValueCollection();
                nvcReport.Add("applicationcode", "4");
                nvcReport.Add("reportcode", "PDFORM");
                nvcReport.Add("empid", EmpId.ToString());
                nvcReport.Add("CRITERIA", "");
                Session["PHOENIXREPORTPARAMETERS"] = nvcReport;

                string filename = "PDForm_" + ViewState["EmpName"].ToString() + ".pdf";
                Tmpfilelocation = dirpath + "/" + filename;

                PhoenixSSRSReportClass.ExportSSRSReport(reportfile, dspdform, PhoenixSSRSReportClass.ExportFileFormat.PDF, null, ref Tmpfilelocation);
        ...
        sbemailbody.AppendLine("<br/>" + ViewState["UserName"].ToString());
