...
public partial class CrewMissingLicence : PhoenixBasePage
{
    ...
    protected void BindData()
    {
        try
        {
            DataTable dt = PhoenixCrewLicence.ListLicenceMissing(int.Parse(strVesselId), int.Parse(strEmployeeId), int.Parse(strRankId),DateTime.Parse(strJoinDate));
            if (dt.Rows.Count > 0)
            {
