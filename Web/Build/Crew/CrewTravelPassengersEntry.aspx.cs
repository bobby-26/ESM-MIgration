...
public partial class CrewTravelPassengersEntry : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["travelrequestedit"].ToString() == "1")
        {
    ...
    private void Binddetails()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            txtStaffName.Text = dr["FLDFIRSTNAME"].ToString();
    ...
    protected void CrewMenu_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                Response.Redirect("CrewTravelPassengersList.aspx?" + Request.QueryString, false);
            }
            if (CommandName.ToUpper().Equals("PASSENGERSENTRY"))
            {
                Response.Redirect("CrewTravelPassengersEntry.aspx?travelid=" + ViewState["TRAVELID"] + "&travelrequestedit=" + ViewState["EDITTRAVELREQUEST"].ToString() + "&requestid=" + ViewState["REQUESTID"], false);
            }
            if (CommandName.ToUpper().Equals("BREAKUP"))
            {
                Response.Redirect("CrewTravelPassengerBreakupdetails.aspx?travelid=" + ViewState["TRAVELID"] + "&travelrequestedit=" + ViewState["EDITTRAVELREQUEST"].ToString() + "&requestid=" + ViewState["REQUESTID"] + "&name=" + txtStaffName.Text + " " + txtlastname.Text, false);
            }
    ...
    protected void MenuCrewTraveladd_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            else if (CommandName.ToUpper().Equals("SAVE"))
            {
                ...
                {
                    if (!IsValidTravelRequest(txtStaffName.Text.Trim(), txtOrigin.SelectedValue, txtDestination.SelectedValue,
                    ...
                    PhoenixCrewTravelRequest.InsertPassengersOfficeStaffDetails(
                               PhoenixSecurityContext.CurrentSecurityContext.UserCode
                               , int.Parse(txtStaffId.Text)
                               , General.GetNullableGuid(ViewState["TRAVELID"] == null ? null : ViewState["TRAVELID"].ToString())
                ...
                else
                {
                    ...
                    PhoenixCrewTravelRequest.UpdatePassengersOfficeStaffDetails(
                               PhoenixSecurityContext.CurrentSecurityContext.UserCode
                               , General.GetNullableGuid(ViewState["TRAVELID"] == null ? null : ViewState["TRAVELID"].ToString())
                               , General.GetNullableGuid(ViewState["REQUESTID"] == null ? null : ViewState["REQUESTID"].ToString())
                               , int.Parse(txtOrigin.SelectedValue)
    ...
    {
        txtStaffName.Text = txtmiddlename.Text = txtlastname.Text = txtdob.Text = txtcdcno.Text = txtDestinationdate.Text = "";
                                                ...
                                                string arrivaldate, string paymentmode, string depdateoly, string arrdateoly)
    {
        ...
        if (string.IsNullOrEmpty(fname.Trim()))
            ucError.ErrorMessage = "Staff is required.";
