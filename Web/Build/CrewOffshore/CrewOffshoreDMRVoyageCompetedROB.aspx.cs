...
public partial class CrewOffshore_CrewOffshoreDMRVoyageCompetedROB : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            MenuVoyageTap.SelectedMenuIndex = 3;
    ...
    private void BindDataTradingArea()
    {
        ...
        dt = PhoenixCrewOffshoreDMRVoyageData.VoyageCharterCargoROBList(new Guid(Filter.CurrentVPRSVoyageSelection), int.Parse(ViewState["VESSELID"].ToString()), 1);
        gvTradingArea.DataSource = dt;
        gvTradingArea.DataBind();
        ...
        if (CommandName.ToUpper().Equals("REPORT"))
        {
            if (Filter.CurrentVPRSVoyageSelection != null)
            {
                String scriptpopup = String.Format(
                       ...
                       + "&VesselID=" + int.Parse(ViewState["VESSELID"].ToString()) + "&Flag=1&showmenu=0');");
                ...
                //                                + "&VesselID=" + int.Parse(ViewState["VESSELID"].ToString()) + "&Flag=" + 1, false);

            }
            else
            {
                ucError.ErrorMessage = "Chartrer is not yet Created.";
                ucError.Visible = true;
            }
        }
    }
    ...
    private void UpdateCargoROBData()
    {
        foreach (GridDataItem item in gvTradingArea.Items)
        {
               ...
               RadLabel OilTypeID = (RadLabel)item.FindControl("lblOilTypeCode");
                ...
                if (OilTypeID != null)
                {
                    PhoenixCrewOffshoreDMRVoyageData.UpdateCharterCargoROBUpdate(new Guid(Filter.CurrentVPRSVoyageSelection), int.Parse(ViewState["VESSELID"].ToString()), new Guid(OilTypeID.Text.ToString()), 1, General.GetNullableDecimal(txtROB.Text),General.GetNullableInteger(ucUnit.SelectedUnit),General.GetNullableString(txtOilType.Text));                    
                }
            }
        
        PhoenixCrewOffshoreDMRVoyageData.UpdateCharterDeckConditionUpdate(new Guid(Filter.CurrentVPRSVoyageSelection), int.Parse(ViewState["VESSELID"].ToString()), txtDeckConditionRemarks.Text, 1, chkAtSea.Checked.Equals(true) ? 1 : 0, txtLocation.Text);
    }
    ...
    private void BindDeckConditionData()
    {
        if (Filter.CurrentVPRSVoyageSelection != null)
        {
            ...
            {
                txtDeckConditionRemarks.Text = dt.Rows[0]["FLDCOMPLETEDECKCONDREMARKS"].ToString();
                chkAtSea.Checked = dt.Rows[0]["FLDCOMPLETIONATSEAYN"].ToString().Equals("1") ? true : false;
                txtLocation.Text = dt.Rows[0]["FLDCOMPLETEDLOCATION"].ToString();
            }
        }
    }
}