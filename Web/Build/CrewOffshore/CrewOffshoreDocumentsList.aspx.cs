...
public partial class CrewOffshoreDocumentsList : PhoenixBasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        toolbarMDI.AddFontAwesomeButton("javascript:parent.openNewWindow('codehelp1','','" + Session["sitepath"] + "/Crew/CrewMedicalAdd.aspx?empid=" + ViewState["empid"].ToString() + "')", "Add", "<i class=\"fa fa-plus-circle\"></i>", "ADDMEDICAL");
        ...
        PhoenixToolbar toolbargridDAT = new PhoenixToolbar();
        toolbargridDAT.AddFontAwesomeButton("../CrewOffShore/CrewOffshoreDocumentsList.aspx?empid=" + ViewState["empid"].ToString(), "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "Excel");
        toolbargridDAT.AddFontAwesomeButton("javascript:CallPrint('gvAlcoholTest')", "Print Grid", "<i class=\"fas fa-print\"></i>", "PRINT");
        MenuDAT.AccessRights = this.ViewState;
        MenuDAT.MenuList = toolbargridDAT.Show();
        ...
        if (!IsPostBack)
        {
            ...
            ViewState["PAGENUMBERDAT"] = 1;
            ViewState["SORTEXPRESSIONDAT"] = null;
            ViewState["SORTDIRECTIONDAT"] = null;
            gvAlcoholTest.PageSize = int.Parse(PhoenixGeneralSettings.CurrentGeneralSetting.Records);
        ...
        toolbarmain.AddButton("Send Email", "SENDEMAIL", ToolBarDirection.Right);
    ...
    protected void CrewPassPort_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            //    }
            //}
            if (CommandName.ToUpper() == "SENDEMAIL")
            {

                String scriptpopup = String.Format("javascript:openNewWindow('codehelp2','','" + Session["sitepath"] + "/CrewOffshore/CrewOffshoreEmployeeDocumentList.aspx?empid="+ ViewState["empid"].ToString() + "');");
    ...
    protected void gvCrewLicence_ItemDataBound(object sender, GridItemEventArgs e)
    {
        ...
        if (e.Item is GridDataItem)
        {
            ...
            ImageButton cmdUnlock = (ImageButton)e.Item.FindControl("cmdUnlock");
            RadLabel lblNumber = (RadLabel)e.Item.FindControl("lblNumber");
            LinkButton lnkLicenceName = (LinkButton)e.Item.FindControl("lnkLicenceName");
            ImageButton cmdAuthenticate = (ImageButton)e.Item.FindControl("cmdAuthenticate");
            ImageButton cmdCrossCheck = (ImageButton)e.Item.FindControl("cmdCrossCheck");
            RadLabel lblAuthenticationRequYN = (RadLabel)e.Item.FindControl("lblAuthenticationRequYN");
            DataRowView drv = (DataRowView)e.Item.DataItem;
        ...
        try
        {
            ...
            else if (e.CommandName.ToString().ToUpper() == "UPDATE")
            {
                try
                {
                    ...
                    string certificatenumber = ((TextBox)e.Item.FindControl("txtCourseNumberEdit")).Text;
                    string dateofissue = ((UserControlDate)e.Item.FindControl("txtIssueDateEdit")).Text;
                    string placeofissue = ((TextBox)e.Item.FindControl("txtPlaceOfIssueEdit")).Text;
                    UserControlDate dateofexpiry = ((UserControlDate)e.Item.FindControl("txtExpiryDateEdit"));
                    ...
                    string authority = ((TextBox)e.Item.FindControl("txtAuthorityEdit")).Text;
                    string verificationmethod = ((UserControlQuick)e.Item.FindControl("ucVerificationMethodEdit")).SelectedQuick;
    ...
    protected void gvCrewCourseCertificate_ItemDataBound(object sender, GridItemEventArgs e)
    {
        ...
        if (e.Item is GridDataItem)
        {
              ...
              && !e.Item.Equals(DataControlRowState.Selected | DataControlRowState.Edit) && !e.Item.Equals(DataControlRowState.Alternate | DataControlRowState.Selected | DataControlRowState.Edit))
            {
                ...
                System.Web.UI.WebControls.Image imgFlag = e.Item.FindControl("imgFlag") as System.Web.UI.WebControls.Image;
                DateTime? d = null;
                if (expdate != null) General.GetNullableDateTime(expdate.Text);
                if (d.HasValue)
                {
                    TimeSpan t = d.Value - DateTime.Now;
                    if (t.Days >= 0 && t.Days < 120)
    ...
    protected void gvOtherDocument_ItemCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName.ToString().ToUpper() == "ADD")
            {
                ...
                InsertTravelDocument(documenttype, documentnumber, dateofissue, placeofissue, dateofexpiry.Text, issuingauthority, verificationmethod);
                BindDataOTH();
    ...
    public void InsertTravelDocument(string documenttype, string documentnumber, string dateofissue, string placeofissue, string dateofexpiry, string issuingauthority, string verificationmethod)
    {
    ...
    protected void gvOtherDocument_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
    {
        try
        {
            ViewState["PAGENUMBEROTH"] = ViewState["PAGENUMBEROTH"] != null ? ViewState["PAGENUMBEROTH"] : gvOtherDocument.CurrentPageIndex + 1;
            BindDataOTH();
        }
        catch (Exception ex)
        {
    ...
    protected void MenuDAT_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            RadToolBarEventArgs dce = (RadToolBarEventArgs)e;
            ...
            if (CommandName.ToUpper().Equals("EXCEL"))
            {
                ...
                string[] alColumns = { "FLDALCOHOLTESTDATE", "FLDPORTNAME", "FLDINSPECTORNAME", "FLDCOMPANYNAME", "FLDSAMPLETAKEN", "FLDRESULT" };
                string[] alCaptions = { "Test Date", "Port Name", "Inspector", "Company", "Sample Taken/Remarks", "Result" };

                string sortexpression;
                int? sortdirection = null;

                sortexpression = (ViewState["SORTEXPRESSIONDAT"] == null) ? null : (ViewState["SORTEXPRESSIONDAT"].ToString());
                if (ViewState["SORTDIRECTIONDAT"] != null)
                    sortdirection = Int32.Parse(ViewState["SORTDIRECTIONDAT"].ToString());

                DataSet ds = PhoenixCrewMedicalDocuments.SearchEmployeeDrugAndAlcoholTest(Convert.ToInt32(ViewState["empid"].ToString())
                                                                             , sortexpression, sortdirection,
                                                                             1,
                                                                             gvAlcoholTest.PageSize,
                                                                             ref iRowCount,
                                                                             ref iTotalPageCount
                                                                             );


                General.ShowExcel("D&A Testt", ds.Tables[0], alColumns, alCaptions, sortdirection, sortexpression);
            }
        }
        catch (Exception ex)
        {
    ...
    private void BindDataDAT()
    {
        try
        {
            ...
            string sortexpression = (ViewState["SORTEXPRESSIONDAT"] == null) ? null : (ViewState["SORTEXPRESSIONDAT"].ToString());
            int? sortdirection = null;
            if (ViewState["SORTDIRECTIONDAT"] != null)
                sortdirection = Int32.Parse(ViewState["SORTDIRECTIONDAT"].ToString());
            ...
            DataSet ds = PhoenixCrewMedicalDocuments.SearchEmployeeDrugAndAlcoholTest(Convert.ToInt32(ViewState["empid"].ToString())
                                                                               , sortexpression, sortdirection,
                                                                               int.Parse(ViewState["PAGENUMBERDAT"].ToString()),
                                                                               gvAlcoholTest.PageSize,
                                                                               ref iRowCount,
                                                                               ref iTotalPageCount
                                                                               );

            General.SetPrintOptions("gvAlcoholTest", "D&A Test", alCaptions, alColumns, ds);

            gvAlcoholTest.DataSource = ds;
            gvAlcoholTest.VirtualItemCount = iRowCount;
    ...
    protected void gvAlcoholTest_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
    {
        try
        {
            ViewState["PAGENUMBERDAT"] = ViewState["PAGENUMBERDAT"] != null ? ViewState["PAGENUMBERDAT"] : gvAlcoholTest.CurrentPageIndex + 1;
            BindDataDAT();
        }
        catch (Exception ex)
        {
    ...
    protected void gvAlcoholTest_ItemCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
            if (e.CommandName == "Page")
            {
                ViewState["PAGENUMBERDAT"] = null;
            }
    ...
    protected void gvAlcoholTest_ItemDataBound(object sender, GridItemEventArgs e)
    {
        if (e.Item is GridEditableItem)
        {
            LinkButton att = (LinkButton)e.Item.FindControl("cmdAtt");
            ...
            if (att != null)
            {
                ...
                att.Attributes.Add("onclick", "javascript:parent.openNewWindow('NAFA','','" + Session["sitepath"] + "/Common/CommonFileAttachment.aspx?dtkey=" + lblDTKey.Text + "&mod="
                        + PhoenixModule.CREW + "&type=" + PhoenixCrewAttachmentType.MEDICAL + "&cmdname=DRUGANDTESTUPLOAD'); return false;");
            }
        }
    }
