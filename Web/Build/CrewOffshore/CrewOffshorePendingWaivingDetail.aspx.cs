...
public partial class CrewOffshorePendingWaivingDetail : PhoenixBasePage
{
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
               ...
               // ViewState["Waivedyn"]="";
                ...
                //if (Request.QueryString["Waivedyn"] != null && Request.QueryString["Waivedyn"].ToString() != "")
                //{
                //    ViewState["Waivedyn"] = Request.QueryString["Waivedyn"].ToString();
                //}
    ...
    protected void CrewMenu_TabStripCommand(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                Response.Redirect("CrewOffshorePendingWaivingRequest.aspx");
            }
        }
        catch (Exception ex)
        {
    ...
    private void BindData()
    {
        try
        {
            ...
            DataSet ds = PhoenixCrewOffshoreWaivingRequest.CrewWaivingDocumentList(
                ...
                //,General.GetNullableInteger(Request.QueryString["Waivedyn"].ToString())
                );

            if (ds.Tables[0].Rows.Count > 0)
            {
                gvSuitability.DataSource = ds.Tables[0];
                if (gvSuitability.Items.Count > 0)
                {
                    //GridDecorator.MergeRows(gvSuitability);
                    string planstatus = ((RadLabel)gvSuitability.Items[0].FindControl("lblPlanStatus")).Text;
            ...
            if (ds.Tables[1].Rows.Count > 0)
            {
                ...
                if (gvRankExp.Items.Count > 0)
                {
                    // GridDecorator.MergeRowsExperience(gvRankExp);
                    string planstatus = ((RadLabel)gvRankExp.Items[0].FindControl("lblPlanStatus")).Text;
                    if (!string.IsNullOrEmpty(planstatus))
                        ViewState["planstatus"] = planstatus;
                    if (string.IsNullOrEmpty(planstatus))
            ...
            if (ds.Tables[2].Rows.Count > 0)
            {
                ...
                if (gvVesselTypeExp.Items.Count > 0)
                {
                    //GridDecorator.MergeRowsExperience(gvVesselTypeExp);
                    string planstatus = ((RadLabel)gvVesselTypeExp.Items[0].FindControl("lblPlanStatus")).Text;
                    if (!string.IsNullOrEmpty(planstatus))
                        ViewState["planstatus"] = planstatus;
                    if (string.IsNullOrEmpty(planstatus))
        ...
        if (e.Item is GridDataItem)
        {
            try
            {
                ...
                if (!e.Item.Equals(DataControlRowState.Alternate | DataControlRowState.Edit) && !e.Item.Equals(DataControlRowState.Edit))
                {
                    ...
                    //chkWaivedconfirmYN.Attributes.Add("onmouseover", "showTooltip(ev, '" + ucWaiveToolTipDate.ToolTip + "', 'visible');");
                    //chkWaivedconfirmYN.Attributes.Add("onmouseout", "showTooltip(ev, '" + ucWaiveToolTipDate.ToolTip + "', 'hidden');");
                    ...
                    if (lblStageid != null && lblProposalStageid != null)
                    {
                        if (lblStageid.Text.Equals(lblProposalStageid.Text) && drv["FLDWAIVEDYN"].ToString().Equals("1")) // enabled only for proposal stage and if the document can be waived.
                        {
                            ...
                            {
                                chkCanbeWaivedYN.Visible = false;
                            }
                        }
                    }
    ...
    protected void gvRankExp_ItemCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName.ToUpper().Equals("APPROVERANK"))
            {
    ...
    protected void gvVesselTypeExp_ItemCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName.ToUpper().Equals("APPROVETYPE"))
            {
