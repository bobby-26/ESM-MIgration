...
public partial class DataTransferBrowseFolder : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            //  toolbar.AddImageButton("../DataTransfer/DataTransferBrowseFolder.aspx", "Up One Folder", "folder-up.png", "UPONEFOLDER");
            toolbar.AddImageButton("../DataTransfer/DataTransferBrowseFolder.aspx", "Reset Service Broker", "process.png", "RESETSERVICEBROKER");
            toolbar.AddImageButton("../DataTransfer/DataTransferBrowseFolder.aspx", "Clear Pending Jobs", "72.png", "CLEARPENDINGJOBS");
            //  toolbar.AddImageLink("javascript:Openpopup('Upload','','DataTransferFileUpload.aspx'); return false;", "Upload", "uploadimage.png", "UPLOADFILE");
            toolbar.AddImageButton("../DataTransfer/DataTransferBrowseFolder.aspx", "Clean Import Folder", "te_del.png", "CLEANIMPORTFOLDER");

            DataTransferFolder.MenuList = toolbar.Show();

            DataTable dt = PhoenixDataTransferImport.DataTransferFolderUpOneLevel(string.Empty);
            Session["FOLDERPATH"] = dt.Rows[0]["FLDANCHORFOLDER"].ToString();
            ...
            if (!IsPostBack)
            {
                ...
                FileExplorer1.InitialPath = Session["FOLDERPATH"].ToString();
                FileExplorer1.Configuration.ContentProviderTypeName = typeof(CustomFileSystemProvider).AssemblyQualifiedName;
            }
            RadMenuItem customMenuOption = new RadMenuItem("Open File");
            customMenuOption.Value = "OPENFILE";
            FileExplorer1.GridContextMenu.Items.Add(customMenuOption);
            customMenuOption = new RadMenuItem("Move To Inbox");
            customMenuOption.Value = "MOVETOINBOX";
            FileExplorer1.GridContextMenu.Items.Add(customMenuOption);
            customMenuOption = new RadMenuItem("Mail To Vessel");
            customMenuOption.Value = "MAILTOVESSEL";
            FileExplorer1.GridContextMenu.Items.Add(customMenuOption);
            customMenuOption = new RadMenuItem("Mail To User");
            customMenuOption.Value = "MAILTOUSER";
            FileExplorer1.GridContextMenu.Items.Add(customMenuOption);
            customMenuOption = new RadMenuItem("Archive/Restore");
            customMenuOption.Value = "ARCHIVERESTORE";
            FileExplorer1.GridContextMenu.Items.Add(customMenuOption);
            customMenuOption = new RadMenuItem("Delete");
            customMenuOption.Value = "DELETE";
            FileExplorer1.GridContextMenu.Items.Add(customMenuOption);
            ...
            gridTemplateColumn1.HeaderStyle.Width = Unit.Parse("150px");
    ...
    protected void DataTransferFolder_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            RadToolBarEventArgs dce = (RadToolBarEventArgs)e;
            string CommandName = ((RadToolBarButton)dce.Item).CommandName;
            if (CommandName.ToUpper().Equals("RESETSERVICEBROKER"))
            {
                PhoenixDataTransferImport.ResetServiceBroker();
            }

            if (CommandName.ToUpper().Equals("CLEARPENDINGJOBS"))
            {
                PhoenixDataTransferImport.ClearPendingJobs();
            }

            if (CommandName.ToUpper().Equals("CLEANIMPORTFOLDER"))
            {
                PhoenixDataTransferCommon.DataTransferCleanImportFolder(1); //Delete Folder
            }
        }
        catch (Exception ex)
        {
  ...
  <Paths>
    <genericHandlerPath>DataTransferXML.aspx?currentpath=</genericHandlerPath>
  </Paths>
  <Mappings>
    <Mapping>
      <PhysicalPath>
            ...
            {// The path is a physical path
                ...
                foreach (KeyValuePair<string, string> mappedPath in MappedPaths)
                {
                    ...
                    {// Exists 
                        ...
                        //  virtualPath = mappedPath.Key + restOfPhysicalPath.Replace("/", "\\").Replace("//", "\\\\");
    ...
    protected void BtnClick_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string path = args[0];
            string name = args[1];
            //string sourcedirname = args[2];
            if (CmdName == "MOVETOINBOX")
            {
                string inboxpath = FileExplorer1.InitialPath + @"\INBOX\" + name;
                string folderpath = path;
                if (folderpath.Contains("/MERGE/"))
                {
                    PhoenixDataTransferImport.DataTransferImportFileMove(folderpath, inboxpath);
                }
                else
                {
                    ucError.ErrorMessage = "You can delete only folders immediately under IMPORT folder. Other folders cannot be deleted";
                    ucError.Visible = true;
                }
            }
            else if (CmdName == "MAILTOVESSEL")
            {
                string currentpath = FileExplorer1.CurrentFolder.Replace("/", "\\").Replace("//", "\\\\"); ;
               // currentpath.Replace("/", "\\").Replace("//", "\\\\");
                string filename = name;
                string folderpath = path;
                if (currentpath.EndsWith("MERGE\\") || currentpath.EndsWith("MAIL\\OUT") || currentpath.EndsWith("MAIL\\TRASH"))
                {
                    folderpath = folderpath.Replace("/", "\\");
                    PhoenixDataTransferImport.DataTransferMail2Vessel(folderpath, filename);
                }
                else
                {
                    ucError.ErrorMessage = "You can mail only Zip Files under MERGE, MAIL\\OUT, MAIL\\TRASH folders. Other files cannot be mailed";
                    ucError.Visible = true;
                }
            }
            else if (CmdName == "MAILTOUSER")
            {
               ...
               // currentpath.Replace("/", "\\").Replace("//", "\\\\");
                ...
                {
                    folderpath = folderpath.Replace("/", "\\");
                    PhoenixDataTransferImport.DataTransferMail2User(folderpath, filename);
                }
                else
                {
                    ucError.ErrorMessage = "You can mail only Zip Files under MERGE, MAIL\\OUT, MAIL\\TRASH folders. Other files cannot be mailed";
        ...
        //    {
        //        string currentpath = FileExplorer1.CurrentFolder;
        //        string folderpath = path;
        //        if (currentpath.EndsWith("IMPORT"))
        //        {
        //            PhoenixDataTransferImport.DataTransferImportFolderDelete(folderpath);
        //        }
        //        else
        //        {
        //            ucError.ErrorMessage = "You can delete only folders immediately under IMPORT folder. Other folders cannot be deleted";
        ...
        //        }
        //    }
        //    else if (CmdName == ("ARCHIVE") || CmdName == ("RESTORE"))
        //    {
        //        string currentpath = FileExplorer1.CurrentFolder;
        //        string sourcename = sourcedirname;

        //        if (currentpath.EndsWith("IMPORT"))
        //        {
        //            sourcedirname = currentpath + '\\' + sourcedirname;
        ...
        //        else
        //        {
        //            ucError.ErrorMessage = "You can archive/restore only folders immediately under IMPORT folder. Other folders cannot be archived";
        //            ucError.Visible = true;
        //        }
        //    }
        //}
        ...
        //private void ShowVessel(string currentpath)
        //{
        //    if (currentpath.EndsWith(@"\0\IMPORT"))
        //    {
