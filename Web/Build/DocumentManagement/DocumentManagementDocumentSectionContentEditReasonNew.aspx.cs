...
public partial class DocumentManagement_DocumentManagementDocumentSectionContentEditReasonNew : PhoenixBasePage
{
    ...
    protected void MenuOfficeComment_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ViewState["DOCUMENTID"].ToString() != null && ViewState["DOCUMENTID"].ToString() != "" && ViewState["SECTIONID"].ToString() != null && ViewState["SECTIONID"].ToString() != "")
            {
                ...
                {
                    int Grammatical = (cbGrammaticalyn.Checked == true ? 1 : 0);
                ...
                Response.Redirect("../DocumentManagement/DocumentManagementDocumentSectionContentGeneralnNew.aspx?DOCUMENTID=" + ViewState["DOCUMENTID"] + "&SECTIONID=" + ViewState["SECTIONID"], false);
            }
            BindOfficeComments();
        }
        catch (Exception ex)
    ...
    protected void cbGrammaticalyn_CheckedChanged(object sender, EventArgs e)
    {
        try
        {
            if (cbGrammaticalyn.Checked == true)
            {
                txtOfficeComments.Text = "Grammatical correction";
            }
            else
            {
                txtOfficeComments.Text = "";
