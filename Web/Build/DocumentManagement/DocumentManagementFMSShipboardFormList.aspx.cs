...
public partial class DocumentManagementFMSShipboardFormList : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbar.AddFontAwesomeButton("../DocumentManagement/DocumentManagementFMSShipboardFormList.aspx", "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "Excel");
            toolbar.AddFontAwesomeButton("javascript:CallPrint('gvForm')", "Print Grid", "<i class=\"fas fa-print\"></i>", "PRINT");
            toolbar.AddFontAwesomeButton("../DocumentManagement/DocumentManagementFMSShipboardFormList.aspx", "Find", "<i class=\"fas fa-search\"></i>", "SEARCH");
            toolbar.AddFontAwesomeButton("../DocumentManagement/DocumentManagementFMSShipboardFormList.aspx", "Clear Filter", "<i class=\"fas fa-eraser\"></i>", "CLEAR");
            ...
            if (!IsPostBack)
            {
                ...
                btnShowCategory.Attributes.Add("onclick", "return showPickList('spnPickListCategory', 'codehelp1', '', 'Common/CommonPickListFMSDocumentCategory.aspx?CATEGORYID=2', true); ");
                gvForm.PageSize = int.Parse(PhoenixGeneralSettings.CurrentGeneralSetting.Records);

                if (Request.QueryString["Callfrom"] != null)
                {
                    Filter.FMSShipFormFilterCriteria = null;
                }

                if (Filter.FMSShipFormFilterCriteria != null)
                {
                    NameValueCollection nvc = Filter.FMSShipFormFilterCriteria;
    ...
    protected void RegistersCountry_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            if (CommandName.ToUpper().Equals("SEARCH"))
            {
                ...
                Filter.FMSShipFormFilterCriteria = nvc;
                gvForm.Rebind();
            }
    ...
    protected void gvForm_ItemDataBound(Object sender, Telerik.Web.UI.GridItemEventArgs e)
    {
        if (e.Item is GridDataItem)
        {
            ...
            if (cmdUpload != null)
            {
                ...
                cmdUpload.Attributes.Add("onclick", "javascript:parent.openNewWindow('formList','','" + Session["sitepath"] + "/DocumentManagement/DocumentManagementFormUpload.aspx?formid=" + dr["FLDFORMID"].ToString() + "&formrevisionid=" + dr["FLDDRAFTREVISIONID"].ToString() + "&formrevisiondtkey=" + dr["FLDFORMREVISIONDTKEY"].ToString() + "','medium'); return false;");
            ...
            if (hlnkfilename != null && dr["FLDFORMBUILDERID"].ToString() != null)
                ...
                hlnkfilename.Attributes.Add("onclick", "javascript:return openNewWindow('formList', '', '" + Session["sitepath"] + "/DocumentManagement/DocumentManagementFMSSubCategoryReports.aspx?typeid="+ lblType.Text + "&FORMID=" + dr["FLDFORMID"].ToString() + "&type=Form&VESSELID=" + ddlvessel.SelectedVessel + "&FormName=" + dr["FLDCAPTION"] + "'); return false;");
