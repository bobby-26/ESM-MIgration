...
public partial class DocumentManagementFMSSubCategoryReports : PhoenixBasePage
{
    //int num = 1;
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (ViewState["FTYPE"].ToString() == "1")
                {
                    ...
                    toolbar.AddLinkButton("javascript:openNewWindow('formUpload','','DocumentManagement/DocumentManagementFMSShipboardFormsAttachmentUpload.aspx?FORMID=" + Request.QueryString["FORMID"].ToString() + "')", "Upload File", "UPLOAD", ToolBarDirection.Right);
    ...
    private void BindData()
    {
        try
        {
            ...
            DataSet ds = PhoenixFormBuilder.FMSReportSearch(Formid
                                                        , sortexpression
                                                        , sortdirection
            ...
            if (ViewState["FTYPE"].ToString() == "1")
                gvFormList.Columns[4].Visible = true;
            else
                gvFormList.Columns[4].Visible = false;
        }
        catch (Exception ex)
        {
            ucError.ErrorMessage = ex.Message;
    ...
    protected void gvFormList_ItemCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
            ...
            {
                RadLabel reportId = (RadLabel)e.Item.FindControl("lblReportId");
                DataTable dt = PhoenixCommonFileAttachment.AttachmentList(new Guid(reportId.Text));
                if (dt.Rows.Count > 0)
                {
                    DataRow drRow = dt.Rows[0];
                    PhoenixCommonFileAttachment.AttachmentDelete(new Guid(drRow["FLDDTKEY"].ToString()));
                    PhoenixFormBuilder.FMSFormUploadDelete(new Guid(reportId.Text));
                    ucStatus.Text = "Attachment is deleted.";
                    BindData();
                }
                //    RadRadLabel lblattachmentcode = (RadRadLabel)e.Item.FindControl("lblattachmentcode");
    ...
    protected void gvFormList_ItemDataBound(Object sender, GridItemEventArgs e)
    {
        try
        {
            if (e.Item is GridDataItem)
            {
                ...
                string statusname = ((RadLabel)e.Item.FindControl("lblStatusname")).Text;
                HyperLink lnkfile = (HyperLink)e.Item.FindControl("lnkReportName");

                if (lnkfile != null)
                {
                    if (ViewState["FTYPE"].ToString() != null && ViewState["FTYPE"].ToString() == "1")
                    {
