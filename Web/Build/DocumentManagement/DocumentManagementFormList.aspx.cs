...
public partial class DocumentManagementFormList : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbar.AddFontAwesomeButton("../DocumentManagement/DocumentManagementFormList.aspx", "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "Excel");
            toolbar.AddFontAwesomeButton("javascript:CallPrint('gvForm')", "Print Grid", "<i class=\"fas fa-print\"></i>", "PRINT");
            toolbar.AddFontAwesomeButton("../DocumentManagement/DocumentManagementFormList.aspx", "Find", "<i class=\"fas fa-search\"></i>", "SEARCH");
            toolbar.AddFontAwesomeButton("../DocumentManagement/DocumentManagementFormList.aspx", "Clear Filter", "<i class=\"fas fa-eraser\"></i>", "CLEAR");
            ...
            toolbar.AddFontAwesomeButton("../DocumentManagement/DocumentManagementFormListAdd.aspx", "Add", "<i class=\"fa fa-plus-circle\"></i>", "ADDFORM");
            ...
            if (e.CommandName == "RowClick" || (e.CommandName == RadGrid.ExpandCollapseCommandName) && (!e.Item.Expanded))
            {
                ...
                if (e.CommandName == RadGrid.ExpandCollapseCommandName || e.CommandName == "RowClick")
                {
                    ...
                    if (item.Expanded == false)
                    {
                        ...
                        DataSet ds = PhoenixDocumentManagementForm.FormNestEdit(PhoenixSecurityContext.CurrentSecurityContext.UserCode, new Guid(ViewState["FORMID"].ToString()));
                        if (ds.Tables[0].Rows.Count > 0)
                        {
                            ...
                            RadLabel txtfunction = ((gvForm.MasterTableView.Items[0].ChildItem as GridNestedViewItem).FindControl("lbltFunction") as RadLabel);
                            txtfunction.Text = dr["FLDFUNCTION"].ToString();
    ...
    private void BindPageURL(int rowindex)
    {
        try
        {
            ...
            ViewState["CATEGORYID"] = ((RadLabel)gvForm.Items[rowindex].FindControl("lblCategoryId")).Text;
