...
public partial class DocumentManagementFormRevisionListNew : PhoenixBasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbar.AddFontAwesomeButton("../DocumentManagement/DocumentManagementFormRevisionListNew.aspx", "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "Excel");
            ...
            toolbar.AddButton("Form", "FORM", ToolBarDirection.Right);
            ...
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["FORMTYPE"] != null && Request.QueryString["FORMTYPE"].ToString() != "" && Request.QueryString["FORMTYPE"].ToString() == "1")
                {
                    ViewState["FORMTYPE"] = Request.QueryString["FORMTYPE"].ToString();
                }
                else
                {
                    ViewState["FORMTYPE"] = "0";
            ...
            if (General.GetNullableInteger(ViewState["FORMTYPE"].ToString()) == 1) //1 -> Upload, 0 ->Design
            {
                gvFormField.Visible = false;
            }
            else
            {
                gvFormField.Visible = false;
    ...
    private void GetFormDetails()
    {
        if (ViewState["FORMID"] != null && ViewState["FORMID"].ToString() != "")
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                MenuDocument.Title = dr["FLDCAPTION"].ToString() + " - " + "Revisions";
            }
        }
    }
    ...
    protected void MenuDocument_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            if (CommandName.ToUpper().Equals("CATEGORY"))
            {
            ...
            {
                Response.Redirect("../DocumentManagement/DocumentManagementFormListNew.aspx?FORMID=" + ViewState["FORMID"] + "&CATEGORYID=" + ViewState["CATEGORYID"].ToString());
            }
        }
        catch (Exception ex)
        {
    ...
    protected void gvFormRevision_ItemDataBound(Object sender, Telerik.Web.UI.GridItemEventArgs e)
    {
        if (e.Item is GridEditableItem)
        {
            ...
            if (hlnkAddedDate != null)
            {
                ...
                {
                    hlnkAddedDate.Attributes.Add("onclick", "javascript:parent.openNewWindow('codehelp','','" + Session["sitepath"] + "/StandardForm/StandardFormFBformView.aspx?FormId=" + dr["FLDFORMID"].ToString() + "&FORMREVISIONID=" + dr["FLDFORMREVISIONID"].ToString() + "&FORMTYPE=DMSForm'); return false;");
                ...
                else
                {
                    if (dr["FLDDTKEY"] != null && dr["FLDDTKEY"].ToString() != "")
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            hlnkAddedDate.NavigateUrl = "../Common/download.aspx?dtkey=" + drRow["FLDDTKEY"].ToString();
    ...
    private void SetRowSelection()
    {
        ...
        foreach (GridDataItem item in gvFormRevision.Items)
        {
            ...
            if (item.GetDataKeyValue("FLDFORMREVISIONID").ToString().Equals(ViewState["REVISIONID"].ToString()))
            {
