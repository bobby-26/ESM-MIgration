...
public partial class FMSDownload : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                DataTable dt = PhoenixDocumentManagementFMSReports.FMSMailAttachmentDownloadEdit(new Guid(dtkey));
                if (dt.Rows.Count > 0)
                {
                    string path = PhoenixGeneralSettings.CurrentGeneralSetting.AttachmentPath;
                    if (!path.EndsWith("/"))
                        path = path + "/";
                    fileDownload(dt.Rows[0]["FLDORGINALFILENAME"].ToString(), dt.Rows[0]["FLDFILENAME"].ToString());
                }
            }
            else
            {
