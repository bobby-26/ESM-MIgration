...
public partial class PayRoll_PayRollDeductionsChapterviaindia : PhoenixBasePage
{
    int usercode = 0;
    int vesselId = 0;
    Guid Id = Guid.Empty;
    ...
    private void GetEditData()
    {
        if (Id != Guid.Empty)
        {
            DataSet ds = PhoenixPayRollIndia.DeductionsChapterviaindiaDetail(usercode, Id);
    ...
    protected void gvTabStrip_TabStripCommand(object sender, EventArgs e)
    {
    	try
        {
            ...
            if (CommandName.ToUpper().Equals("SAVE"))
            {
                ...
                PhoenixPayRollIndia.DeductionsChapterviaindiaInsert(usercode, payRollId, employerId, employeeId, txtsection.Text,
                    txthead.Text, txtdescription.Text,  decimal.Parse(txtamt.Text));
            }
            if (CommandName.ToUpper().Equals("UPDATE"))
            {
                PhoenixPayRollIndia.DeductionsChapterviaindiaUpdate(usercode, Id, payRollId, employerId, employeeId, txtsection.Text,
                    txthead.Text,  txtdescription.Text, decimal.Parse(txtamt.Text));
            }
