...
public partial class PayRoll_PayRollExemptionSection10 : PhoenixBasePage
{
    int usercode = 0;
    int vesselId = 0;
    Guid Id = Guid.Empty;
    ...
    private void GetEditData()
    {
        if (Id != Guid.Empty)
        {
            DataSet ds = PhoenixPayRollIndia.ExemptionSection10Detail(usercode, Id);
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                ...
                txtNatureOfAllowance.Text = dr["FLDNATUREOFALLOWANCE"].ToString();
    ...
    protected void gvTabStrip_TabStripCommand(object sender, EventArgs e)
    {
    	try
        {
            ...
            {
                PhoenixPayRollIndia.ExemptionSection10Insert(usercode, payRollId, employerId, employeeId, txtNatureOfAllowance.Text, txtDescription.Text, amount);
            }

            if (CommandName.ToUpper().Equals("UPDATE"))
            {
                PhoenixPayRollIndia.ExemptionSection10Update(usercode, payRollId, Id, employerId, employeeId, txtNatureOfAllowance.Text, txtDescription.Text, amount);
            }
    ...
    private bool IsValidReport()
    {
        ...
        if (string.IsNullOrWhiteSpace(txtNatureOfAllowance.Text))
        {
            ucError.ErrorMessage = "Nature of Allowance is mandatory";
        }
