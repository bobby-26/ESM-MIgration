...
public partial class PayRoll_PayRollTDSindia : PhoenixBasePage
{
    int usercode = 0;
    int vesselId = 0;
    Guid Id = Guid.Empty;
    ...
    private void GetEditData()
    {
        if (Id != Guid.Empty)
        {
            DataSet ds = PhoenixPayRollIndia.PayRollTDSindiaDetail(usercode, Id);
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                DataRow dr = ds.Tables[0].Rows[0];

                ddltax.SelectedValue = dr["FLDPAYROLLTAXID"].ToString();
                ddladvancetax.SelectedValue = dr["FLDPAYROLLADVANCETAXID"].ToString();
                ddlemployer.SelectedValue = dr["FLDPAYROLLEMPLOYERID"].ToString();
                ddlemploye.SelectedValue = dr["FLDEMPLOYEEID"].ToString();
                txtincomeamt.Text = dr["FLDINCOMEAMOUNT"].ToString();
                txtttaxamt.Text = dr["FLDTTOTALTAXAMOUNT"].ToString();
    ...
    private void bindadvancetax()
    {
      
        ddladvancetax.DataTextField = "FLDBSRCODE";
        ddladvancetax.DataValueField = "FLDPAYROLLADVANCETAXID";
        ddladvancetax.DataSource = PhoenixPayRollIndia.PayRollAdavanceTaxList();
        ddladvancetax.DataBind();
    }
    protected void gvTabStrip_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            Guid advancetaxId = new Guid(ddladvancetax.SelectedItem.Value);
            ...
            if (CommandName.ToUpper().Equals("SAVE"))
            {
                ...
                PhoenixPayRollIndia.PayRollTDSindiaInsert(usercode, payRollId, advancetaxId, employerId, employeeId, 
                decimal.Parse(txtincomeamt.Text), decimal.Parse(txtttaxamt.Text));
            }
            if (CommandName.ToUpper().Equals("UPDATE"))
            {
                PhoenixPayRollIndia.PayRollTDSindiaUpdate(usercode, Id, payRollId, advancetaxId, employerId, employeeId,
                    decimal.Parse(txtincomeamt.Text), decimal.Parse(txtttaxamt.Text));
            }
    ...
    private bool IsValidReport()
    {
        ...
        if (string.IsNullOrWhiteSpace(ddladvancetax.SelectedValue))
        {
            ucError.ErrorMessage = "Advance Tax is mandatory";
        }
        ...
        if (string.IsNullOrWhiteSpace(txtincomeamt.Text))
        {
            ucError.ErrorMessage = "Income amt is mandatory";
        }
        if (string.IsNullOrWhiteSpace(txtttaxamt.Text))
        {
