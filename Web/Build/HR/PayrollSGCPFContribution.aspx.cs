...
public partial class HR_PayrollSGCPFContribution : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        SessionUtil.PageAccessRights(this.ViewState);
        PhoenixToolbar menu = new PhoenixToolbar();
        menu.AddFontAwesomeButton("../HR/PayrollSGCPFContribution.aspx", "Search", "<i class=\"fa fa-search\"></i>", "SEARCH");
        gvmenu.MenuList = menu.Show();
        if (!IsPostBack)
        {
            ddlyear.SelectedYear = DateTime.Now.Year;
            gvcpf.PageSize = int.Parse(PhoenixGeneralSettings.CurrentGeneralSetting.Records);
            Loaddata();
    ...
    protected void gvcpf_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
    {
        ViewState["PAGENUMBER"] = ViewState["PAGENUMBER"] != null ? ViewState["PAGENUMBER"] : gvcpf.CurrentPageIndex + 1;


        int iRowCount = 0;
        int iTotalPageCount = 0;

        DataTable dt = PhoenixPayRollSingapore.CPFContributionSearch(General.GetNullableInteger(ddlyear.SelectedYear.ToString())
                                                                   ...
                                                                   ,(int)ViewState["PAGENUMBER"], gvcpf.PageSize, ref iRowCount, ref iTotalPageCount);
        gvcpf.DataSource = dt;
        gvcpf.VirtualItemCount = iRowCount;
    }

    protected void gvcpf_ItemDataBound(object sender, Telerik.Web.UI.GridItemEventArgs e)
    {

    }

    protected void gvcpf_ItemCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
    {
