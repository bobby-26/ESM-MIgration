...
public partial class InspectionAuditInterfaceReport : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbar.AddFontAwesomeButton("../Inspection/InspectionAuditInterfaceReport.aspx", "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "Excel");
            ...
            {
                toolbar.AddFontAwesomeButton("../Inspection/InspectionAuditInterfaceReport.aspx", "Upload Deficiency", "<i class=\"fas fa-upload\"></i>", "UPLOAD");
            }
    ...
    protected void gvReport_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
    {
        try
        {
            ViewState["PAGENUMBER"] = ViewState["PAGENUMBER"] != null ? ViewState["PAGENUMBER"] : gvReport.CurrentPageIndex + 1;
            BindData();
        }
        catch (Exception ex)
        {
    ...
    private void BindData()
    {
        try
        {
            ...
            DataSet ds = PhoenixInspectionAuditInterfaceDetails.InspectionAuditInterfaceReport(
                    General.GetNullableGuid(ViewState["ReviewScheduleId"].ToString()),
            ...
            string[] alCaptions = { "S.No", "Comp Code", "CheckListCode", "Item", "Complies", "Deficiency/Observation", "Action Required", "NC/Obs", "Assigned TO", "Due Date", "Verification Level" };
    ...
    protected void CheckAll(Object sender, EventArgs e)
    {
        ...
        if (ctl != null && ctl[0].ToString() == "gvReport$ctl00$ctl02$ctl01$chkUploadHeader")
        {
            GridHeaderItem headerItem = gvReport.MasterTableView.GetItems(GridItemType.Header)[0] as GridHeaderItem;
            RadCheckBox chkAll = (RadCheckBox)headerItem.FindControl("chkUploadHeader");
            foreach (GridDataItem row in gvReport.MasterTableView.Items)
            {
    ...
    protected void gvReport_ItemDataBound(object sender, GridItemEventArgs e)
    {
        try
        {
            if (e.Item is GridDataItem)
            {
                ...
                {                    
                    condition.Attributes["style"] = "color:Green !important";
                    condition.Font.Bold = true;
                }
                else if (drv["FLDCODE"].ToString().Equals("1"))
                {
                    condition.Attributes["style"] = "color:Red !important";
                    condition.Font.Bold = true;
                }
                else if (drv["FLDCODE"].ToString().Equals("2"))
                {
                    condition.Attributes["style"] = "color:darkorange !important";
                    condition.Font.Bold = true;
                }
                else if (drv["FLDCODE"].ToString().Equals("3"))
                {
                    condition.Attributes.Add("style", "font-weight:bold;");
                    condition.BackColor = System.Drawing.Color.White;
                }
    ...
    protected void MenuReport_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                PhoenixInspectionAuditInterfaceDetails.InterfaceInterfaceDeficiencyupload(
    ...
    protected void SaveCheckedValues(Object sender, EventArgs e)
    {
        ...
        foreach (GridDataItem gvrow in gvReport.Items)
        {
            if (((RadCheckBox)(gvrow.FindControl("ChkItem"))).Checked == true)
            {
                ViewState["MappingIDList"] += gvrow.GetDataKeyValue("FLDMAPPINGID").ToString() + ',';
            }
            else
