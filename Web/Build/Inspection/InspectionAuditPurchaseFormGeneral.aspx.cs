...
public partial class InspectionAuditPurchaseFormGeneral : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
	{
		...
		try
		{
            ...
            MenuFormGeneral.Title = "General ( " + PhoenixInspectionAuditPurchaseForm.FormNumber + " )";
            if (!IsPostBack)
			{				
	...
	private void BindData(string orderid)
	{
		...
		if (ds.Tables[0].Rows.Count > 0)
		{
			...
			{
				btnShowBudget.Attributes.Add("onclick", "return showPickList('spnPickListMainBudget', 'codehelp1', '', '../Common/CommonPickListBudget.aspx?budgetgroup=106&hardtypecode=30&vesselid=" + Filter.CurrentInspectionPurchaseVesselSelection + "&budgetdate=" + txtBugetDate.Text + "&framename=ifMoreInfo', true); ");
			}
			else if (Filter.CurrentInspectionPurchaseStockType.ToUpper().Equals("SERVICE"))
			{
				btnShowBudget.Attributes.Add("onclick", "return showPickList('spnPickListMainBudget', 'codehelp1', '', '../Common/CommonPickListBudget.aspx?budgetgroup=107&hardtypecode=30&vesselid=" + Filter.CurrentInspectionPurchaseVesselSelection + "&budgetdate=" + txtBugetDate.Text + "&framename=ifMoreInfo', true); ");
			}
			else
			{
				btnShowBudget.Attributes.Add("onclick", "return showPickList('spnPickListMainBudget', 'codehelp1', '','../Common/CommonPickListBudget.aspx?budgetgroup=105&hardtypecode=30&vesselid=" + Filter.CurrentInspectionPurchaseVesselSelection + "&budgetdate=" + txtBugetDate.Text + "&framename=ifMoreInfo', true); ");
			}
			cmdPicPartPaid.Attributes.Add("onclick", "return showPickList('spnPicPartPaid', 'codehelp1', '', '../Inspection/InspectionPurchaseOrderPartPaid.aspx?orderid=" + ViewState["orderid"].ToString() + "', true); ");
	...
	protected void MenuFormGeneral_TabStripCommand(object sender, EventArgs e)
	{
        ...
        try
		{
			...
			{
				Response.Redirect("../Inspection/InspectionAuditPurchaseFormType.aspx?REVIEWDNC="+ViewState["REVIEWDNC"]);
			}
