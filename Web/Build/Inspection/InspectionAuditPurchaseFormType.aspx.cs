...
public partial class InspectionAuditPurchaseFormType : PhoenixBasePage
{
    ...
    private void SetVesselIdForOffice()
    {
        ...
        if (ViewState["REVIEWDNC"] != null && ViewState["REVIEWDNC"].ToString() != "")
        {
            ...
            ds = PhoenixInspectionAuditDirectNonConformity.EditObservation(new Guid(ViewState["REVIEWDNC"].ToString()));
            if (ds.Tables[0].Rows.Count > 0)
            {
                if (PhoenixSecurityContext.CurrentSecurityContext.VesselID == 0)
                {
			...
			if (CommandName.ToUpper().Equals("SAVE"))
			{
                ...
                else
                {
                    if (Request.QueryString["callfrom"] != null && Request.QueryString["callfrom"].ToString() == "nonconformity")
                        ucError.Text = "First record the 'Non Conformity' details to create a requisition";
                    else if (Request.QueryString["callfrom"] != null && Request.QueryString["callfrom"].ToString() == "ncobservation")
                        ucError.Text = "First record the 'Observation' details to create a requisition";
                    else
                        ucError.Text = "First record the 'Direct Non Conformity' details to create a requisition";                    
                    ucError.Visible = true;
                }
			}
