...
public partial class Inspection_InspectionBulkReviewOfficeRemarks : PhoenixBasePage
{
            ...
            if (CommandName.ToUpper().Equals("SAVE"))
            {
                if (Filter.CurrentSelectedComments != null)
                {
                    ...
                    if (selectedcomments != null && selectedcomments.Count > 0)
                    {
                        ...
                        {
                            PhoenixInspectionRiskAssessmentGenericExtn.RACommentReviewUpdate(
    ...
    private void BindComments()
    {
        if (Filter.CurrentSelectedComments != null)
        {
            ...
            if (selectedcomments != null && selectedcomments.Count > 0)
            {
                foreach (Guid commentid in selectedcomments)
                {
                    DataTable dt = PhoenixInspectionRiskAssessmentGenericExtn.RADocumentSectionRevisonRemarks(new Guid(commentid.ToString()));
