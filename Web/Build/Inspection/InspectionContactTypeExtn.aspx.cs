...
public partial class InspectionContactTypeExtn : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbar.AddFontAwesomeButton("../Inspection/InspectionContactTypeExtn.aspx", "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "EXCEL");
    ...
    protected void ShowExcel()
    {
        ...
        ds = PhoenixInspectionContractTypeExtn.ContactTypeSearch(General.GetNullableInteger(ddlcause.SelectedHard), sortexpression, sortdirection,
           1,
           iRowCount,
           ref iRowCount,
           ref iTotalPageCount);
    ...
    protected void gvContactType_ItemCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
            ...
            {
                PhoenixInspectionContractTypeExtn.DeleteContactType(PhoenixSecurityContext.CurrentSecurityContext.UserCode
                    , General.GetNullableGuid(((RadLabel)e.Item.FindControl("lblContactTypeId")).Text));
                ucStatus.Text = "Contact type deleted";
                Rebind();
            }           

            else if (e.CommandName.ToUpper().Equals("WSMAPPING"))
            {
                string contacttypeid = ((RadLabel)e.Item.FindControl("lblContactTypeId")).Text;
                string contacttype = ((RadLabel)e.Item.FindControl("lblContactType")).Text;
                Response.Redirect("../Inspection/InspectionUndesirableEventWorstCase.aspx?contacttypeid=" + contacttypeid + "&contacttype=" + contacttype);
                Rebind();
            }
    ...
    {
        PhoenixInspectionContractTypeExtn.InsertContactType(PhoenixSecurityContext.CurrentSecurityContext.UserCode
            , contacttype
            , serialnumber
            );
    }
    ...
    {
        PhoenixInspectionContractTypeExtn.UpdateContactType(PhoenixSecurityContext.CurrentSecurityContext.UserCode
            , contacttypeid
            , contacttype
            , serialnumber
            );
