    ...
    protected void MenuOfficeToCopy_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ViewState["JOBHAZARDID"] != null && ViewState["TYPE"].ToString() == "JHA")
            {
                ...
                {
                    PhoenixInspectionRiskAssessmentJobHazardExtn.RiskAssessmentJobHazardCopyByCompany(PhoenixSecurityContext.CurrentSecurityContext.UserCode
            ...
            else if (ViewState["RAPROCESSID"] != null && ViewState["TYPE"].ToString() == "RAPROCESS")
            {
                if (CommandName.ToUpper().Equals("SAVE"))
                {
                    PhoenixInspectionRiskAssessmentProcessExtn.CopyRiskAssessmentProcessByCompany(PhoenixSecurityContext.CurrentSecurityContext.UserCode, new Guid(ViewState["RAPROCESSID"].ToString()), 0, General.GetNullableInteger(ucCompanySelect.SelectedCompany));
                }
            }
