...
public partial class InspectionDeficiencyCARList : PhoenixBasePage
{
        ...
        try
        {
            ...
            if (e.Item is GridDataItem)
            {
                ...
                if (ev != null)
                {
                    ...
                    if (ev != null)
                        ev.Attributes.Add("onclick", "javascript:openNewWindow('codehelp1','', '" + Session["sitepath"] + "/Common/CommonFileAttachment.aspx?dtkey=" + lblDtkey.Text
    ...
    protected void gvPreventiveAction_ItemDataBound(Object sender, GridItemEventArgs e)
    {
        try
        {
            ...
            if (e.Item is GridDataItem)
            {
                ...
                {
                    lnkWorkOrder.Attributes.Add("onclick", "javascript:Openpopup('source','','../Inspection/InspectionLongTermActionWorkOrderDetails.aspx?WORKORDERID=" + lblWorkOrderID.Text + "&viewonly=1'); return true;");
                }
                ...
                if (ParEdit != null)
                {
                    if ((drv["FLDDEPARTMENTTYPEID"].ToString() == "1") && (drv["FLDOPENEDYN"].ToString() == "0"))
                    {
                        ...
                        ParEdit.Attributes.Add("onclick", "javascript:parent.Openpopup('codehelp1','','../Inspection/InspectionPreventiveTasksDetails.aspx?CallFrom=incident&preventiveactionid=" + drv["FLDINSPECTIONPREVENTIVEACTIONID"].ToString() + "'); return true;");

                    }
                    else if ((drv["FLDDEPARTMENTTYPEID"].ToString() == "2") && (drv["FLDOPENEDYN"].ToString() == "0"))
                    {
                        ParEdit.Visible = SessionUtil.CanAccess(this.ViewState, ParEdit.CommandName);
                        ParEdit.Attributes.Add("onclick", "javascript:parent.Openpopup('codehelp1','','../Inspection/InspectionOfficeTasksDetails.aspx?CallFrom=incident&preventiveactionid=" + drv["FLDINSPECTIONPREVENTIVEACTIONID"].ToString() + "'); return true;");
                    }
                    else
                    {
    ...
    protected void gvCorrectiveAction_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
    {
        try
        {
