...
public partial class Inspection_InspectionDirectNCDefectWorkRequest : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        SessionUtil.PageAccessRights(this.ViewState);
        PhoenixToolbar toolbar = new PhoenixToolbar();
        toolbar.AddFontAwesomeButton("../Inspection/InspectionDirectNCDefectWorkRequest.aspx", "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "Excel");
    ...
    protected void SaveDefect()
    {
        if ((ViewState["DefectId"] == null) || (ViewState["DefectId"].ToString() == ""))
        {
            ...
            PhoenixInspectionWorkOrder.InspectionDefectJobMap(defectJobId, new Guid(ViewState["REVIEWDNC"].ToString()), int.Parse(ViewState["DEFICIENCYTYPE"].ToString()), null, int.Parse(ViewState["VESSELID"].ToString()), General.GetNullableString("INS"));
    ...
    private void GetVesselId()
    {
        if (ViewState["REVIEWDNC"] != null && ViewState["REVIEWDNC"].ToString() != "")
        {
            ...
            ds = PhoenixInspectionObservation.EditDirectObservation(new Guid(ViewState["REVIEWDNC"].ToString()));
            if (ds.Tables[0].Rows.Count > 0)
            {
                ViewState["VESSELID"] = ds.Tables[0].Rows[0]["FLDVESSELID"].ToString();
