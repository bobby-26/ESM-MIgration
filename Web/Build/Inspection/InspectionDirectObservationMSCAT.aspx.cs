...
public partial class InspectionDirectObservationMSCAT : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            VesselConfiguration();
            if (Request.QueryString["OBSID"] != null && Request.QueryString["OBSID"].ToString() != string.Empty)
            {
                ViewState["OBSID"] = Request.QueryString["OBSID"].ToString();
                EditNonConformity(new Guid(ViewState["OBSID"].ToString()));
            }
        }
        ...
        toolbar.AddFontAwesomeButton("javascript:openNewWindow('codehelp1','', '" + Session["sitepath"] + "/Inspection/InspectionDirectObservationMSCATAdd.aspx?OBSID=" + (ViewState["OBSID"] == null ? null : ViewState["OBSID"].ToString()) + "')", "Add New", "<i class=\"fa fa-plus-circle\"></i>", "ADD");
    ...
    protected void gvMSCAT_ItemDataBound(Object sender, GridItemEventArgs e)
    {
        if (e.Item is GridDataItem)
        {
            ...
            if (ce != null)
            {
                ...
                {
                    ce.Attributes.Add("onclick", "javascript:openNewWindow('codehelp1', '', '" + Session["sitepath"] + "/Inspection/InspectionMSCATEdit.aspx?mscatid=" + new Guid(lblMscatid) + "&DEFICIENCYID=" + (ViewState["OBSID"] == null ? null : ViewState["OBSID"].ToString()) + "');return true;");
                }
            }
        }
    }
    ...
    protected void BindData()
    {
        ...
        ds = PhoenixInspectionIncidentMSCAT.MSCATItemList(ViewState["OBSID"] == null ? null : General.GetNullableGuid((ViewState["OBSID"].ToString())));
    ...
    protected void ShowExcel()
    {
        try
        {
            ...
            DataSet ds = PhoenixInspectionIncidentMSCAT.MSCATItemList(ViewState["OBSID"] == null ? null : General.GetNullableGuid((ViewState["OBSID"].ToString())));
