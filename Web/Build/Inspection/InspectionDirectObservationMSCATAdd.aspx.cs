...
public partial class InspectionDirectObservationMSCATAdd : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (Request.QueryString["OBSID"] != null && Request.QueryString["OBSID"].ToString() != string.Empty)
            {
                ...
                EditObservation(new Guid(ViewState["OBSID"].ToString()));
            }
    ...
    {
        DataSet ds = PhoenixInspectionCorrectiveAction.ListCorrectiveAction(General.GetNullableGuid(ViewState["OBSID"].ToString()));
    ...
    protected void EditObservation(Guid observationid)
    {
        DataSet ds = PhoenixInspectionObservation.EditDirectObservation(observationid);
    ...
    protected void MenuMSCAT_TabStripCommand(object sender, EventArgs e)
    {
        ...
        if (CommandName.ToUpper().Equals("SAVE"))
        {
            ...
            {
                ucError.ErrorMessage = "Please record Observation details before starting M-SCAT Analysis.";
            ...
            for (int i = 0; i < chkCAR.Items.Count; i++)
            {
                if (chkCAR.Items[i].Selected)
                {
                    ...
                    //contact type
                    //PhoenixInspectionIncidentMSCAT.MSCATItemInsert(new Guid(ViewState["OBSID"].ToString()),
    ...
    private bool IsValidAdd()
    {
        ...
        //if (General.GetNullableGuid(rblCAN.SelectedValue) == null)
        //    ucError.ErrorMessage = "Conrol action is required.";
