...
public partial class Inspection_InspectionIncidentDefectWorkRequest : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        SessionUtil.PageAccessRights(this.ViewState);
        PhoenixToolbar toolbar = new PhoenixToolbar();
        toolbar.AddFontAwesomeButton("../Inspection/InspectionIncidentDefectWorkRequest.aspx", "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "Excel");
        ...
        if (!IsPostBack)
        {
            ...
            ViewState["REVIEWDNC"] = Filter.CurrentIncidentID;
    ...
    private void BindFields()
    {
        ...
        if ((ViewState["DefectId"] != null) && (ViewState["DefectId"].ToString() != ""))
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                rdType.SelectedValue = dr["FLDTYPE"].ToString();
    ...
    private bool IsValidDefectjob(string Details, string Duedate)
    {
        ...
        if (string.IsNullOrEmpty(txtWorkDetails.Text))
            ucError.ErrorMessage = "Work Details is required.";
    ...
    protected void SaveDefect()
    {
        if ((ViewState["DefectId"] == null) || (ViewState["DefectId"].ToString() == ""))
        {
            ...
            PhoenixInspectionWorkOrder.InspectionDefectJobMap(defectJobId, new Guid(ViewState["REVIEWDNC"].ToString()), int.Parse(ViewState["DEFICIENCYTYPE"].ToString()), null, int.Parse(ViewState["VESSELID"].ToString()), General.GetNullableString("INC"));
    ...
    protected void gvWorkOrder_ItemDataBound(object sender, GridItemEventArgs e)
    {
        if (e.Item is GridDataItem)
        {
            ...
            if (verify != null)
            {
                ...
                verify.Attributes.Add("onclick", "$modalWindow.modalWindowUrl = '../Inspection/InspectionDefectJobVerification.aspx?defectjobId=" + drv["FLDDEFECTJOBID"].ToString() + " &defectno=" + drv["FLDDEFECTNO"].ToString() + "'; showDialog('Review - "+ drv["FLDDEFECTNO"].ToString() + "'); return false;");
            }
