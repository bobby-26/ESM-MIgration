...
public partial class InspectionIncidentSecurityList : PhoenixBasePage
{
    protected override void Render(HtmlTextWriter writer)
    {
        foreach (GridViewRow r in gvIncidentSecurity.Rows)
        {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                toolbar.AddImageButton("../Inspection/InspectionIncidentSecurityList.aspx", "Export to Excel", "icon_xls.png", "Excel");
                toolbar.AddImageLink("javascript:CallPrint('gvIncidentSecurity')", "Print Grid", "icon_print.png", "PRINT");
                ...
                MenuIncidentSecuritySearch.AccessRights = this.ViewState;
                MenuIncidentSecuritySearch.MenuList = toolbar.Show();
                ...
                MenuIncidentReportGeneral.SelectedMenuIndex = 3;
    ...
    protected void ShowExcel()
    {
        ...
        string[] alColumns = { "FLDTYPEOFSECURITYNAME", "FLDESTIMATEDCOST", "FLDCATEGORY" };
        string[] alCaptions = { "Type", "Estimated Cost in USD", "Consequence Category" };
        ...
        DataSet ds = PhoenixInspectionIncidentSecurity.IncidentSecuritySearch(
        ...
        Response.AddHeader("Content-Disposition", "attachment; filename=IncidentSecurityList.xls");
        Response.ContentType = "application/vnd.msexcel";
        Response.Write("<TABLE BORDER='0' CELLPADDING='2' CELLSPACING='0' width='100%'>");
        Response.Write("<tr>");
        Response.Write("<td><img src='http://" + Request.Url.Authority + Session["images"] + "/esmlogo4_small.png" + "' /></td>");
        Response.Write("<td><h3>Incident Security List</h3></td>");
    ...
    protected void IncidentSecuritySearch_TabStripCommand(object sender, EventArgs e)
    {
        DataListCommandEventArgs dce = (DataListCommandEventArgs)e;
    ...
    private void BindData()
    {
        ...
        General.SetPrintOptions("gvIncidentSecurity", "Incident Security List", alCaptions, alColumns, ds);

        if (ds.Tables[0].Rows.Count > 0)
        {
            gvIncidentSecurity.DataSource = ds;
            gvIncidentSecurity.DataBind();

            if (ViewState["SECURITYID"] == null)
            {
                gvIncidentSecurity.SelectedIndex = 0;
                ViewState["SECURITYID"] = ((Label)gvIncidentSecurity.Rows[0].FindControl("lblIncidentSecurityId")).Text;
            }

            if (ViewState["PAGEURL"] == null)
            {
                ViewState["PAGEURL"] = "../Inspection/InspectionIncidentSecurityGeneral.aspx";
                ifMoreInfo.Attributes["src"] = ViewState["PAGEURL"].ToString() + "?securityid=" + ViewState["SECURITYID"].ToString();
            }
            else
            {
                ifMoreInfo.Attributes["src"] = ViewState["PAGEURL"].ToString() + "?securityid=" + ViewState["SECURITYID"].ToString();
        ...
        else
        {
            ...
            ShowNoRecordsFound(dt, gvIncidentSecurity);

            ViewState["SECURITYID"] = null;
            ViewState["PAGEURL"] = "../Inspection/InspectionIncidentSecurityGeneral.aspx";
            ifMoreInfo.Attributes["src"] = "../Inspection/InspectionIncidentSecurityGeneral.aspx";
        }
    ...
    protected void gvIncidentSecurity_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            {
                ViewState["SECURITYID"] = ((Label)_gridView.Rows[nCurrentRow].FindControl("lblIncidentSecurityId")).Text;

                BindPageURL(nCurrentRow);
            }
            if (e.CommandName.ToUpper().Equals("DELETE"))
            {
                string inspectionincidentid = ((Label)_gridView.Rows[nCurrentRow].FindControl("lblIncidentSecurityId")).Text;
                ViewState["SECURITYID"] = null;
                DeleteIncidentSecurity(inspectionincidentid);
            }
            else
            {
    ...
    {
        gvIncidentSecurity.SelectedIndex = -1;
        for (int i = 0; i < gvIncidentSecurity.Rows.Count; i++)
        {
            if (gvIncidentSecurity.DataKeys[i].Value.ToString().Equals(ViewState["SECURITYID"].ToString()))
            {
                gvIncidentSecurity.SelectedIndex = i;
                ViewState["SECURITYID"] = ((Label)gvIncidentSecurity.Rows[i].FindControl("lblIncidentSecurityId")).Text;
                break;
            }
        }
    }
    ...
    private void BindPageURL(int rowindex)
    {
        try
        {
            ViewState["SECURITYID"] = ((Label)gvIncidentSecurity.Rows[rowindex].FindControl("lblIncidentSecurityId")).Text;
    ...
    private void DeleteIncidentSecurity(string incidentsecurityid)
    {
        PhoenixInspectionIncidentSecurity.DeleteIncidentSecurity(
            PhoenixSecurityContext.CurrentSecurityContext.UserCode,
            new Guid(incidentsecurityid));
    }

    protected void gvIncidentSecurity_RowDeleting(object sender, GridViewDeleteEventArgs de)
    {
        BindData();
        SetPageNavigator();
    }

    protected void gvIncidentSecurity_RowEditing(object sender, GridViewEditEventArgs de)
    {

    }

    protected void gvIncidentSecurity_Sorting(object sender, GridViewSortEventArgs se)
    {
        gvIncidentSecurity.SelectedIndex = -1;
        gvIncidentSecurity.EditIndex = -1;
    ...
    protected void gvIncidentSecurity_ItemDataBound(Object sender, System.Web.UI.WebControls.GridViewRowEventArgs e)
    {
