...
public partial class InspectionLongTermActionAttachment : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                ifMoreInfo.Attributes["src"] = "../Common/CommonFileAttachment.aspx?DTKEY=" + ViewState["DTKEY"] + "&MOD=" + PhoenixModule.QUALITY;
            }
        }
        catch (Exception ex)
        {
    ...
    protected void InspectionGeneral_TabStripCommand(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("../Inspection/InspectionLongTermActionWorkOrderList.aspx?WORKORDERID=" + ViewState["WORKORDERID"] + "&DTKEY=" + ViewState["DTKEY"]);
        }
        else if (dce.CommandName.ToUpper().Equals("ATTACHEMENTS"))
        {
            Response.Redirect("../Inspection/InspectionLongTermActionAttachment.aspx");
        }
