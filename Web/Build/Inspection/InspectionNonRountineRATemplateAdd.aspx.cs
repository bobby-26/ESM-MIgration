...
public partial class Inspection_InspectionNonRountineRATemplateAdd : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                imgShowRAEdit.Attributes.Add("onclick", "return showPickList('spnRAEdit', 'codehelp1', '', '../Common/CommonPickListRA.aspx?opt=M&vesselid=" + PhoenixSecurityContext.CurrentSecurityContext.VesselID.ToString() + "&activity=" + "', true); ");
    ...
    protected void MenuMachinery_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            if (CommandName.ToUpper().Equals("LIST"))
            {
                ...
                {
                    Response.Redirect("../Inspection/InspectionStandardTemplateList.aspx", false);
                }
                else
                {
            ...
            if (CommandName.ToUpper().Equals("BACK"))
            {
                ...
                if (ViewState["FromWorkorder"].ToString() == "1")
                {
            ...
            //              {
            //                  string scriptpopup = string.Format("javascript:Openpopup('codehelp1','','../Inspection/InspectionVesselRAApproval.aspx?RATEMPLATEID=" + ViewState["RISKASSESSMENTMACHINERYID"].ToString() + "&TYPE=3','large');");
    ...
    protected void gvMachinerySafety_ItemCommand(object sender, GridCommandEventArgs gce)
    {
        try
        {
            ...
            if (gce.CommandName.ToUpper().Equals("SUPDATE"))
            {
    ...
    protected void BindComponents()
    {
        ...
        if (categoryshortcode.ToUpper().Equals("MCE")) // Maintenance of critical equipment
        {
            if (ViewState["VESSELID"] != null && ViewState["VESSELID"].ToString() != "")
                imgComponent.Attributes.Add("onclick", "return showPickList('spnPickListComponent', 'codehelp1', '', '../Common/CommonPickListComponent.aspx?vesselid=" + ViewState["VESSELID"].ToString() + "&showcritical=1', true); ");
            else
                imgComponent.Attributes.Add("onclick", "return showPickList('spnPickListComponent', 'codehelp1', '', '../Common/CommonPickListComponent.aspx?showcritical=1', true); ");
        }
        else //if (categoryshortcode.ToUpper().Equals("MCF") || categoryshortcode.Equals("")) // Machinery failure
        {
            if (ViewState["VESSELID"] != null && ViewState["VESSELID"].ToString() != "")
                imgComponent.Attributes.Add("onclick", "return showPickList('spnPickListComponent', 'codehelp1', '', '../Common/CommonPickListComponent.aspx?vesselid=" + ViewState["VESSELID"].ToString() + "', true); ");
            else
                imgComponent.Attributes.Add("onclick", "return showPickList('spnPickListComponent', 'codehelp1', '', '../Common/CommonPickListComponent.aspx?', true); ");
        }
    }
}
