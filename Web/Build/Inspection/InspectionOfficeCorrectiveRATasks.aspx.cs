...
public partial class Inspection_InspectionOfficeCorrectiveRATasks : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        toolbar.AddFontAwesomeButton("../Inspection/InspectionOfficeCorrectiveRATasks.aspx", "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "EXCEL");
        toolbar.AddFontAwesomeButton("javascript:CallPrint('gvOfficeRATask')", "Print Grid", "<i class=\"fas fa-print\"></i>", "PRINT");
        toolbar.AddFontAwesomeButton("javascript:openNewWindow('codehelp1','', '" + Session["sitepath"] + "/Inspection/InspectionOfficeRATaskFilter.aspx')", "Find", "<i class=\"fas fa-search\"></i>", "FIND");
        toolbar.AddFontAwesomeButton("../Inspection/InspectionOfficeCorrectiveRATasks.aspx", "Clear Filter", "<i class=\"fas fa-eraser\"></i>", "CLEAR");

        MenuOfficeRATasks.AccessRights = this.ViewState;
        MenuOfficeRATasks.MenuList = toolbar.Show();
        ...
        if (!IsPostBack)
        {
            ...
            gvOfficeRATask.PageSize = int.Parse(PhoenixGeneralSettings.CurrentGeneralSetting.Records);
    ...
    protected void MenuOfficeRATasks_TabStripCommand(object sender, EventArgs e)
    {
        RadToolBarEventArgs dce = (RadToolBarEventArgs)e;
        string CommandName = ((RadToolBarButton)dce.Item).CommandName;
        ...
        {
            InspectionFilter.CurrentOfficeCorrectiveRATaskFilter = null;
            BindData();
            gvOfficeRATask.Rebind();
        }
    }
    ...
    protected void ShowExcel()
    {
        ...
        NameValueCollection nvc = InspectionFilter.CurrentOfficeCorrectiveRATaskFilter;
        ...
        if (InspectionFilter.CurrentOfficeCorrectiveRATaskFilter == null)
        {
        ...
        DataSet ds = PhoenixInspectionLongTermAction.OfficeRATasksSearch(
                                                                  null
                                                                , vesselid
                                                                ...
                                                                , sortexpression
                                                                , sortdirection
                                                                , gvOfficeRATask.CurrentPageIndex + 1
                                                                , gvOfficeRATask.PageSize
                                                                , ref iRowCount
                                                                , ref iTotalPageCount
                                                                );

        Response.AddHeader("Content-Disposition", "attachment; filename=OfficeRATaskList.xls");
        Response.ContentType = "application/vnd.msexcel";
        Response.Write("<TABLE BORDER='0' CELLPADDING='2' CELLSPACING='0' width='100%'>");
        Response.Write("<tr>");
        Response.Write("<td><img src='http://" + Request.Url.Authority + Session["images"] + "/esmlogo4_small.png" + "' /></td>");
        Response.Write("<td><h3>Office RA Tasks</h3></td>");
    ...
    private void BindData()
    {
        ...
        General.SetPrintOptions("gvOfficeRATask", "Office RA Tasks", alCaptions, alColumns, ds);

        gvOfficeRATask.DataSource = ds;
        gvOfficeRATask.VirtualItemCount = iRowCount;
    }

    protected void gvOfficeRATask_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
    {
        try
        {
            ViewState["PAGENUMBER"] = ViewState["PAGENUMBER"] != null ? ViewState["PAGENUMBER"] : gvOfficeRATask.CurrentPageIndex + 1;
            BindData();
        }
        catch (Exception ex)
        {
    ...
    protected void gvOfficeRATask_ItemDataBound(object sender, GridItemEventArgs e)
    {
        ...
        if (eb != null)
            eb.Attributes.Add("onclick", "javascript:openNewWindow('codehelp1','','" + Session["sitepath"] + "/Inspection/InspectionOfficeCorrectiveRATasksEdit.aspx?RAMACHINERYID=" + lblRaMachinaryId.Text + "'); return false;");
    }

    protected void gvOfficeRATask_ItemCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
