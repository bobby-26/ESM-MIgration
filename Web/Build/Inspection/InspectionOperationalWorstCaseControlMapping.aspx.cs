...
public partial class InspectionOperationalWorstCaseControlMapping : PhoenixBasePage
{
    ...
    {
        DataSet dss = PhoenixInspectionOperationalRiskControls.OperationalUndesirableFormPosterList(ViewState["UNIQUEID"] == null ? null : General.GetNullableGuid(ViewState["UNIQUEID"].ToString()));
        int number = 1;
    ...
    protected void lnkFormAdd_Click(object sender, EventArgs e)
    {
        if (General.GetNullableGuid(txtDocumentId.Text) != null)
        {
            ...
            PhoenixInspectionOperationalRiskControls.UpdateOperationalUndesirableForms(new Guid(ViewState["UNIQUEID"].ToString()), new Guid(txtDocumentId.Text));
    ...
    void cb_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (c.Checked == false)
        {
            PhoenixInspectionOperationalRiskControls.OperationalUndesirableeventFormsDelete(new Guid(ViewState["UNIQUEID"].ToString()), new Guid(c.ID));
    ...
    protected void MenuMapping_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            if (CommandName.ToUpper().Equals("SAVE"))
            {
                ...
                PhoenixInspectionOperationalRiskControls.InsertOperationalundesirableeventMapping(new Guid(ViewState["UNIQUEID"].ToString()), General.GetNullableString(recommendedppe));
    ...
    {
        DataSet ds = PhoenixInspectionOperationalRiskControls.OperationalUndesirableeventedit(new Guid(ViewState["UNIQUEID"].ToString()));
        DataTable dt = ds.Tables[0];
        if (dt.Rows.Count > 0)
        {
            txtelement.Text = dt.Rows[0]["FLDELEMENT"].ToString();
            txtAspect.Text = dt.Rows[0]["FLDASPECT"].ToString();
            txtRiskofEscalation.Text = dt.Rows[0]["FLDRISKESCALATION"].ToString();
            txtevent.Text = dt.Rows[0]["FLDEVENTNAME"].ToString();
            txthazard.Text = dt.Rows[0]["FLDWORSTCASENAME"].ToString();
            BindCheckBoxList(cblRecomendedPPE, dt.Rows[0]["FLDPPE"].ToString());
    ...
    protected void lnkComponentAdd_Click(object sender, EventArgs e)
    {
        if (General.GetNullableGuid(txtComponentId.Text) != null)
        {
            ...
            PhoenixInspectionOperationalRiskControls.UpdateOperationalUndesirableEventComponents(new Guid(ViewState["UNIQUEID"].ToString()), new Guid(txtComponentId.Text));
    ...
    {
        DataSet dss = PhoenixInspectionOperationalRiskControls.OperationalUndesirableEventComponentList(ViewState["UNIQUEID"] == null ? null : General.GetNullableGuid(ViewState["UNIQUEID"].ToString()));
        int number = 1;
        if (dss.Tables[0].Rows.Count > 0)
        {
    ...
    void com_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (c.Checked == false)
        {
            PhoenixInspectionOperationalRiskControls.OperationalUndesirableEventComponentDelete(new Guid(ViewState["UNIQUEID"].ToString()), new Guid(c.ID));
