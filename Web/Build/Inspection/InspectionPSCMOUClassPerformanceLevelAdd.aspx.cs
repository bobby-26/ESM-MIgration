...
public partial class Inspection_InspectionPSCMOUClassPerformanceLevelAdd : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            ViewState["FLDCLASSPERFORMANCELEVEL"] = null;
            ...
            if (Request.QueryString["classperformancelevel"] != null)
                ViewState["FLDCLASSPERFORMANCELEVEL"] = Request.QueryString["classperformancelevel"].ToString();
            else
                ViewState["FLDCLASSPERFORMANCELEVEL"] = null;
    ...
    protected void CreateDynamicTextBox()
    {
        if (ViewState["NOOFCOLUMNS"].ToString() != "")
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (ViewState["FLDCLASSPERFORMANCELEVEL"] != null && ViewState["ACTION"].ToString() == "edit")
                {
                    DataTable dt = new DataTable();
                    dt = PhoenixInspectionPSCMOUMatrix.PSCMOUClassPerformanceLevelEdit(General.GetNullableInteger(ViewState["FLDCLASSPERFORMANCELEVEL"].ToString()));
                    if (dt.Rows.Count > 0)
                    {
                        MyTextBox.Text = dt.Rows[i]["FLDPSCMOUWEIGHTAGE"].ToString();
                        ddlclassperf.SelectedValue = dt.Rows[i]["FLDCLASSPERFORMANCELEVEL"].ToString();
    ...
    protected void MenuDocumentCategoryMain_TabStripCommand(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (CommandName.ToUpper().Equals("SAVE"))
            {
                ...
                if (ddlclassperf.SelectedValue == "")
                {
                    ucError.ErrorMessage = "Class Performance Level is required";
                    ...
                    if (n > 0)
                    {
                        ...
                        for (int i = 0; i < n; i++)
                        {
                            ...
                            if (MyTextBox != null)
                            {
                                ...
                                if (MyTextBox != null)
                                {
                                    PhoenixInspectionPSCMOUMatrix.PSCMOUShipClassPerformanceLevelInsert(
                                                                        PhoenixSecurityContext.CurrentSecurityContext.UserCode,
                                                                         General.GetNullableInteger(ddlclassperf.SelectedValue),
                    ...
                    if (n > 0)
                    {
                        ...
                        for (int i = 0; i < n; i++)
                        {
                            ...
                            if (MyTextBox != null)
                            {
                                ...
                                if (MyTextBox != null)
                                {
                                    PhoenixInspectionPSCMOUMatrix.PSCMOUShipClassPerformanceLevelUpdate(
                                                                        PhoenixSecurityContext.CurrentSecurityContext.UserCode,
                                                                         General.GetNullableInteger(ddlclassperf.SelectedValue),
                                                                         General.GetNullableGuid(mou),
                                                                         General.GetNullableInteger(ss)
