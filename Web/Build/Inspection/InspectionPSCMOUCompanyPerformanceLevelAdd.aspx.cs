...
public partial class Inspection_InspectionPSCMOUCompanyPerformanceLevelAdd : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            ViewState["FLDCOMPANYPERFORMANCELEVEL"] = null;
            ...
            if (Request.QueryString["companyperformancelevel"] != null)
                ViewState["FLDCOMPANYPERFORMANCELEVEL"] = Request.QueryString["companyperformancelevel"].ToString();
            else
                ViewState["FLDCOMPANYPERFORMANCELEVEL"] = null;
    ...
    {
        ddlcompanyperf.DataSource = PhoenixInspectionPSCMOUMatrix.ListCompanyPerformanceList();
        ddlcompanyperf.DataTextField = "FLDNAME";
        ddlcompanyperf.DataValueField = "FLDCOMPANYPERFORMANCEID";
        ddlcompanyperf.DataBind();
        ddlcompanyperf.Items.Insert(0, new RadComboBoxItem("--Select--", "Dummy"));
    }
    ...
    protected void CreateDynamicTextBox()
    {
        if (ViewState["NOOFCOLUMNS"].ToString() != "")
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (ViewState["FLDCOMPANYPERFORMANCELEVEL"] != null && ViewState["ACTION"].ToString() == "edit")
                {
                    DataTable dt = new DataTable();
                    dt = PhoenixInspectionPSCMOUMatrix.PSCMOUCompanyPerformanceLevelEdit(General.GetNullableInteger(ViewState["FLDCOMPANYPERFORMANCELEVEL"].ToString()));
                    if (dt.Rows.Count > 0)
                    {
                        MyTextBox.Text = dt.Rows[i]["FLDPSCMOUWEIGHTAGE"].ToString();
                        ddlcompanyperf.SelectedValue = dt.Rows[i]["FLDCOMPANYPERFORMANCELEVEL"].ToString();
                        ddlcompanyperf.Enabled = false;
                    }
                }
    ...
    protected void MenuDocumentCategoryMain_TabStripCommand(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (CommandName.ToUpper().Equals("SAVE"))
            {
                ...
                if (ddlcompanyperf.SelectedValue == "")
                {
                    ucError.ErrorMessage = "Company Performance Level is required";
                    ...
                    if (n > 0)
                    {
                        ...
                        for (int i = 0; i < n; i++)
                        {
                            ...
                            if (MyTextBox != null)
                            {
                                ...
                                if (MyTextBox != null)
                                {
                                    PhoenixInspectionPSCMOUMatrix.PSCMOUShipCompanyPerformanceLevelInsert(
                                                                        PhoenixSecurityContext.CurrentSecurityContext.UserCode,
                                                                         General.GetNullableInteger(ddlcompanyperf.SelectedValue),
                    ...
                    if (n > 0)
                    {
                        ...
                        for (int i = 0; i < n; i++)
                        {
                            ...
                            if (MyTextBox != null)
                            {
                                ...
                                if (MyTextBox != null)
                                {
                                    PhoenixInspectionPSCMOUMatrix.PSCMOUShipCompanyPerformanceLevelUpdate(
                                                                        PhoenixSecurityContext.CurrentSecurityContext.UserCode,
                                                                         General.GetNullableInteger(ddlcompanyperf.SelectedValue),
                                                                         General.GetNullableGuid(mou),
                                                                         General.GetNullableInteger(ss)
