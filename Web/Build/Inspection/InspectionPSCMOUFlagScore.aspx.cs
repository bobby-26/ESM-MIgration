...
public partial class Inspection_InspectionPSCMOUFlagScore : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbar.AddFontAwesomeButton("../Inspection/InspectionPSCMOUFlagScore.aspx", "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "Excel");
            toolbar.AddFontAwesomeButton("javascript:CallPrint('gvMenuPSCMOU')", "Print Grid", "<i class=\"fas fa-print\"></i>", "PRINT");
            toolbar.AddFontAwesomeButton("javascript:openNewWindow('codehelpactivity','Flag','Inspection/InspectionPSCMOUFlagPerformanceScore.aspx?action=add')", "Add", "<i class=\"fa fa-plus-circle\"></i>", "ADDADDRESS");
    ...
    protected void ShowExcel()
    {
        ...
        ds = PhoenixInspectionPSCMOUMatrix.PSCMOUFlagScoreSearch(
    ...
    protected void gvMenuPSCMOU_ItemCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
            if (e.Item is GridFooterItem)
            {
                ...
                {
                    if (!IsValidFlag(
                        ((UserControlFlag)e.Item.FindControl("ucFlagAdd")).SelectedFlag,
                        ((RadComboBox)e.Item.FindControl("ddlpscmouregionNameAdd")).SelectedValue,
                        ((RadComboBox)e.Item.FindControl("ddlflagperfAdd")).SelectedValue,
                        ((UserControlMaskNumber)e.Item.FindControl("ucScoreAdd")).Text))
                    {
                        ucError.Visible = true;
                        return;
                    }
                    PhoenixInspectionPSCMOUMatrix.PSCMOUShipFlagScoreInsert(
                        PhoenixSecurityContext.CurrentSecurityContext.UserCode,
                        General.GetNullableInteger(((UserControlFlag)e.Item.FindControl("ucFlagAdd")).SelectedFlag),
                        "",
                        General.GetNullableInteger(((RadComboBox)e.Item.FindControl("ddlflagperfAdd")).SelectedValue),                        
                        (((RadCheckBox)e.Item.FindControl("chkIMOAuditAdd")).Checked == true) ? 1 : 0,
                        (((RadCheckBox)e.Item.FindControl("chkCertificatesIssuedAdd")).Checked == true) ? 1 : 0,
            ...
            if (e.Item is GridEditableItem)
            {
                if (e.CommandName.ToUpper().Equals("UPDATE"))
                {
                    ...
                    if (!IsValidFlag(
                        ((UserControlFlag)e.Item.FindControl("ucFlagEdit")).SelectedFlag,
                        ((RadComboBox)e.Item.FindControl("ddlpscmouregionNameEdit")).SelectedValue,
                        ((RadComboBox)e.Item.FindControl("ddlflagperfedit")).SelectedValue,
                        ((UserControlMaskNumber)e.Item.FindControl("ucScoreEdit")).Text))
                    {
                        ucError.Visible = true;
                        return;
                    }
                    PhoenixInspectionPSCMOUMatrix.PSCMOUFlagScoreUpdate(
                            PhoenixSecurityContext.CurrentSecurityContext.UserCode,
                            General.GetNullableInteger(((RadLabel)e.Item.FindControl("lblflagEdit")).Text),
                            General.GetNullableInteger(((UserControlFlag)e.Item.FindControl("ucFlagEdit")).SelectedFlag),
                            "",
                            General.GetNullableInteger(((RadComboBox)e.Item.FindControl("ddlflagperfedit")).SelectedValue),
                        (((RadCheckBox)e.Item.FindControl("chkIMOAuditEdit")).Checked == true) ? 1 : 0,
                        (((RadCheckBox)e.Item.FindControl("chkCertificatesIssuedEdit")).Checked == true) ? 1 : 0,
                ...
                {
                    PhoenixInspectionPSCMOUMatrix.PSCMOUFlagScoreDelete(
                        PhoenixSecurityContext.CurrentSecurityContext.UserCode,
                        General.GetNullableInteger(((RadLabel)e.Item.FindControl("lblflagId")).Text));
                    gvMenuPSCMOU.Rebind();
    ...
    protected void gvMenuPSCMOU_ItemDataBound(object sender, GridItemEventArgs e)
    {
        if (e.Item is GridDataItem)
        {
            ...
            UserControlFlag ucFlagEdit = (UserControlFlag)e.Item.FindControl("ucFlagEdit");
            RadComboBox ddlpscmouregionNameedit = (RadComboBox)e.Item.FindControl("ddlpscmouregionNameedit");
            RadComboBox ddlflagperfedit = (RadComboBox)e.Item.FindControl("ddlflagperfedit");

            DataRowView drv = (DataRowView)e.Item.DataItem;
            if (ucFlagEdit != null) ucFlagEdit.SelectedFlag = DataBinder.Eval(e.Item.DataItem, "FLDFLAGID").ToString();
            ...
            if (ddlflagperfedit != null)
            {
                ddlflagperfedit.DataSource = PhoenixInspectionPSCMOUMatrix.ListPSCMOUFlagPerformance();
                ddlflagperfedit.DataTextField = "FLDNAME";
                ddlflagperfedit.DataValueField = "FLDFLAGPERFORMANCEID";
                ddlflagperfedit.DataBind();
                ddlflagperfedit.Items.Insert(0, new RadComboBoxItem("--Select--", "Dummy"));
                if (drv["FLDFLAGPERFORMANCE"] != null) ddlflagperfedit.SelectedValue = drv["FLDFLAGPERFORMANCE"].ToString();
            }
        }
        ...
        if (e.Item is GridFooterItem)
        {
            ...
            RadComboBox ddlflagperfAdd = (RadComboBox)footer.FindControl("ddlflagperfAdd");
            if (db != null)
            {
            ...
            if (ddlpscmouregionNameAdd != null)
            {
                ddlflagperfAdd.DataSource = PhoenixInspectionPSCMOUMatrix.ListPSCMOUFlagPerformance();
                ddlflagperfAdd.DataTextField = "FLDNAME";
                ddlflagperfAdd.DataValueField = "FLDFLAGPERFORMANCEID";
                ddlflagperfAdd.DataBind();
                ddlflagperfAdd.Items.Insert(0, new RadComboBoxItem("--Select--", "Dummy"));
            }
        }
    }

    private bool IsValidFlag(string flag, string pscmou, string flagper, string score)
    {
    ...
    protected void ucFlag_TextChangedEvent(object sender, EventArgs e)
    {
        gvMenuPSCMOU.Rebind();
    }
}