...
public partial class Inspection_InspectionPSCMOUShipAgeScoreAdd : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["shipage"] != null)
                ViewState["FLDSHIPAGEID"] = Request.QueryString["shipage"].ToString();
            else
                ViewState["FLDSHIPAGEID"] = null;
    ...
    protected void CreateDynamicTextBox()
    {
        if (ViewState["NOOFCOLUMNS"].ToString() != "")
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (ViewState["FLDSHIPAGEID"] != null && ViewState["ACTION"].ToString() == "edit")
                {
                    DataTable dt = new DataTable();
                    dt = PhoenixInspectionPSCMOUMatrix.PSCMOUShipAgeScoresEdit(General.GetNullableInteger(ViewState["FLDSHIPAGEID"].ToString()));
                    if (dt.Rows.Count > 0)
                    {
                        MyTextBox.Text = dt.Rows[i]["FLDPSCMOUWEIGHTAGE"].ToString();
                        ddlshipage.SelectedValue = dt.Rows[i]["FLDSHIPAGEID"].ToString();
                    ...
                    if (n > 0)
                    {
                        ...
                        for (int i = 0; i < n; i++)
                        {
                            ...
                            if (MyTextBox != null)
                            {
                                ...
                                if (MyTextBox != null)
                                {
                                    PhoenixInspectionPSCMOUMatrix.PSCMOUShipAgeScoresInsert(
                                                                        PhoenixSecurityContext.CurrentSecurityContext.UserCode,
                                                                         General.GetNullableInteger(ddlshipage.SelectedValue),
                                                                         General.GetNullableString(ddlshipage.SelectedItem.Text),
                                                                         "",
                    ...
                    if (n > 0)
                    {
                        ...
                        for (int i = 0; i < n; i++)
                        {
                            ...
                            if (MyTextBox != null)
                            {
                                ...
                                if (MyTextBox != null)
                                {
                                    PhoenixInspectionPSCMOUMatrix.PSCMOUShipAgeScoresUpdate(
                                                                        PhoenixSecurityContext.CurrentSecurityContext.UserCode,
                                                                        General.GetNullableInteger(ddlshipage.SelectedValue),
                                                                        General.GetNullableString(ddlshipage.SelectedItem.Text),
                                                                         "",
