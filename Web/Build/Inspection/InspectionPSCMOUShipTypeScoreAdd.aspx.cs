...
public partial class Inspection_InspectionPSCMOUShipTypeScoreAdd : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["shiptype"] != null)
                ViewState["FLDSHIPTYPE"] = Request.QueryString["shiptype"].ToString();
            else
                ViewState["FLDSHIPTYPE"] = null;
    ...
    protected void CreateDynamicTextBox()
    {
        if (ViewState["NOOFCOLUMNS"].ToString() != "")
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (ViewState["FLDSHIPTYPE"] != null && ViewState["ACTION"].ToString() == "edit")
                {
                    DataTable dt = new DataTable();
                    dt = PhoenixInspectionPSCMOUMatrix.PSCMOUShipTypeScoresEdit(General.GetNullableInteger(ViewState["FLDSHIPTYPE"].ToString()));
                    if (dt.Rows.Count > 0)
                    {
                        MyTextBox.Text = dt.Rows[i]["FLDPSCMOUWEIGHTAGE"].ToString();
                        ucVesselType.SelectedHard = dt.Rows[i]["FLDSHIPTYPE"].ToString();
                    ...
                    if (n > 0)
                    {
                        ...
                        for (int i = 0; i < n; i++)
                        {
                            ...
                            if (MyTextBox != null)
                            {
                                ...
                                if (MyTextBox != null)
                                {
                                    PhoenixInspectionPSCMOUMatrix.PSCMOUShipTypeScoreInsert(
                    ...
                    if (n > 0)
                    {
                        ...
                        for (int i = 0; i < n; i++)
                        {
                            ...
                            if (MyTextBox != null)
                            {
                                ...
                                if (MyTextBox != null)
                                {
                                    PhoenixInspectionPSCMOUMatrix.PSCMOUShipTypeScoreUpdate(
                                                                        PhoenixSecurityContext.CurrentSecurityContext.UserCode,
                                                                         General.GetNullableInteger(ucVesselType.SelectedHard),
                                                                         "",
                                                                         General.GetNullableGuid(mou),
