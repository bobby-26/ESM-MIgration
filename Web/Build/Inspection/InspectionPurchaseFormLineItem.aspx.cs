...
public partial class InspectionPurchaseFormLineItem : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            toolbargrid.AddFontAwesomeButton("../Inspection/InspectionPurchaseFormLineItem.aspx", "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "Excel");
            ...
            if (Filter.CurrentInspectionPurchaseStockType.ToUpper().Equals("SERVICE"))
            {
                if (Filter.CurrentInspectionMenu == null)
                    toolbargrid.AddFontAwesomeButton("javascript:openNewWindow('Filter','', '" + Session["sitepath"] + "/Inspection/InspectionPurchaseOrderLineItemServiceSelection.aspx?orderid=" + ViewState["orderid"].ToString() + "');return false;", "Add", "<i class=\"fas fa-plus-circle\"></i>", "ADD");
            ...
            {
                if (Filter.CurrentInspectionMenu == null)
                    toolbargrid.AddFontAwesomeButton("javascript:openNewWindow('Filter','', '" + Session["sitepath"] + "/Inspection/InspectionPurchaseOrderLineItemSelection.aspx?orderid=" + ViewState["orderid"].ToString() + "');return false;", "Add", "<i class=\"fas fa-plus-circle\"></i>", "ADD");
                else if (Filter.CurrentInspectionMenu == "directobs")
                {
                    if (ViewState["DIRECTOBJ"] != null)
                    {
            ...
            if (Filter.CurrentInspectionMenu == null)
                toolbargrid.AddFontAwesomeButton("../Inspection/InspectionPurchaseFormLineItem.aspx", "Delete", "<i class=\"fas fa-trash\"></i>", "BulkDelete");
            else if (Filter.CurrentInspectionMenu == "directobs")
            {
                if (ViewState["DIRECTOBJ"] != null)
                {
            ...
            toolbargrid.AddFontAwesomeButton("javascript:openNewWindow('Report','', '" + Session["sitepath"] + "/Reports/ReportsView.aspx?applicationcode=3&reportcode=REQUISTIONFORM&orderid=" + ViewState["orderid"].ToString() + "');return false;", "Requisition Form", "<i class=\"fas fa-file-alt\"></i>", "REQUISTION");
    ...
    private void BindDataEdit(string orderid, string orderlineid)
    {
        if (orderlineid != null && orderlineid != "")
        {
            ...
            lineitemdataset = PhoenixInspectionAuditPurchaseForm.EditOrderLine(new Guid(orderlineid));
    ...
    private void BindData()
    {
        ...
        DataSet ds = PhoenixInspectionPurchaseForm.OrderLineSearch(
    ...
    //protected void gvLineItem_SelectedIndexChanging(object sender, GridViewSelectEventArgs e)
    //{
    //    gvLineItem.SelectedIndex = e.NewSelectedIndex;
    //    ViewState["orderlineid"] = ((RadLabel)gvLineItem.Rows[e.NewSelectedIndex].FindControl("lblLineid")).Text;
    //    ViewState["DTKEY"] = ((RadLabel)gvLineItem.Rows[e.NewSelectedIndex].FindControl("lbldtkey")).Text;

    //    if (gvLineItem.EditIndex > -1)
    //        gvLineItem.UpdateRow(gvLineItem.EditIndex, false);
    ...
    protected void gvLineItem_ItemDataBound(Object sender, GridItemEventArgs e)
    {
        if (e.Item is GridDataItem)
        {
            ...
            if (img != null)
                img.Attributes.Add("onclick", "javascript:return Openpopup('Component', '', 'PurchaseFormItemComponentDetails.aspx?COMPONENTID=" + lblcomponentid.Text + "&VESSELID=" + lblvesselid.Text + "'); return false;");
            ...
            LinkButton img1 = (LinkButton)e.Item.FindControl("cmdDetail");
            RadLabel lblisitemdetails = (RadLabel)e.Item.FindControl("lblIsItemDetails");
            if (img1 != null)
            {
                if (Filter.CurrentInspectionPurchaseStockType.ToUpper().Equals("SERVICE"))
                    img1.Attributes.Add("onclick", "javascript:return Openpopup('Component', '', 'PurchaseServiceDetail.aspx?WORKORDERID=" + lblcomponentid.Text + "&VESSELID=" + lblvesselid.Text + "&STOCKTYPE=" + Filter.CurrentInspectionPurchaseStockType.ToString() + "'); return false;");
                else
                    img1.Attributes.Add("onclick", "javascript:return Openpopup('Component', '', 'PurchaseSpareItemDetail.aspx?SPAREITEMID=" + lblcomponentid.Text + "&VESSELID=" + lblvesselid.Text + "&STOCKTYPE=" + Filter.CurrentInspectionPurchaseStockType.ToString() + "'); return false;");
                ...
                if (lblisitemdetails.Text.ToUpper().Equals("1"))
                {
                    ...
                    img1.Controls.Add(html);
                }
                else
                {
                    img1.Visible = false;
                }
                //img1.ImageUrl = lblisitemdetails.Text.ToUpper().Equals("1") ? Session["images"] + "/part-detail.png" : Session["images"] + "/spacer.gif";
            }
        ...
        if (ViewState["orderlineid"] != null)
        {
            ...
            for (int i = 0; i < gvLineItem.Items.Count; i++)
            {
                if (gvLineItem.MasterTableView.Items[i].GetDataKeyValue("FLDORDERLINEID").ToString().Equals(ViewState["orderlineid"].ToString()))
                {
                    ...
                    PhoenixInspectionPurchaseForm.OrderLinePartNumber = ((LinkButton)gvLineItem.Items[i].FindControl("lnkStockItemName")).Text;
    ...
    protected void gvLineItem_UpdateCommand(object sender, GridCommandEventArgs e)
    {
        ...
        //ucStatus.Text = "Quantity is updated.";
        Rebind();
    }
