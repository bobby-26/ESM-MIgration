...
public partial class Inspection_InspectionRAGeneric : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                imgShowRAEdit.Attributes.Add("onclick", "return showPickList('spnRAEdit', 'codehelp1', '', '../Common/CommonPickListRA.aspx?opt=G&vesselid=" + PhoenixSecurityContext.CurrentSecurityContext.VesselID.ToString() + "&activity=" + "', true); ");
    ...
    protected void BindHealthSafetyRiskGrid()
    {
    ...
    protected void BindgvEconomicRiskGrid()
    {
    ...
    protected void MenuGeneric_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            if (CommandName.ToUpper().Equals("REAPPROVE"))
            {
                ...
                {
                    PhoenixInspectionRiskAssessmentGeneric.RequestApprovalGeneric(new Guid(ViewState["RISKASSESSMENTGENERICID"].ToString()));
            ...
            if (CommandName.ToUpper().Equals("CONFIRM"))
            {
                ...
                if (ViewState["COPYTYPE"].ToString() == "2")
                {
                    if (ViewState["RISKASSESSMENTGENERICID"] != null && ViewState["RISKASSESSMENTGENERICID"].ToString() != "")
                    {
                        PhoenixInspectionRiskAssessmentGeneric.ProposeGenericTemplate(PhoenixSecurityContext.CurrentSecurityContext.UserCode,
                            new Guid(ViewState["RISKASSESSMENTGENERICID"].ToString()), PhoenixSecurityContext.CurrentSecurityContext.VesselID);
    ...
    //protected void gvGenericSafety_RowEditing(object sender, GridViewEditEventArgs de)
    //{
    //    try
    //    {
    //        GridView _gridView = (GridView)sender;
    ...
    //    }
    //}
    //protected void gvGenericSafety_RowCancelingEdit(object sender, GridViewCancelEditEventArgs e)
    //{
    //    GridView _gridView = (GridView)sender;
    ...
    protected void gvGenericSafety_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
    {
        BindGridGenericSafety();
    }
}
