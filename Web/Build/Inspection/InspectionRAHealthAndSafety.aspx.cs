...
public partial class InspectionRAHealthAndSafety : PhoenixBasePage
{
	protected void Page_Load(object sender, EventArgs e)
	{
		try
		{
			...
			toolbargrid.AddImageButton("../Inspection/InspectionRAHealthAndSafety.aspx", "Export to Excel", "icon_xls.png", "Excel");
			toolbargrid.AddImageLink("javascript:CallPrint('gvHealthAndSafety')", "Print Grid", "icon_print.png", "PRINT");
			MenuGridHealthAndSafety.AccessRights = this.ViewState;
			MenuGridHealthAndSafety.MenuList = toolbargrid.Show();
			...
			if (!IsPostBack)
			{
				...
				MenuHealthAndSafety.AccessRights = this.ViewState;
				MenuHealthAndSafety.MenuList = toolbar.Show();
	...
	protected void ShowExcel()
	{
		...
		string[] alCaptions = { "Activity", "Risk", "Hazard", "Probabilty of Occurance", "Likelyhood of Harm", "Level of Control", "Level of Risk" };
		...
		ds = PhoenixInspectionRiskAssessment.RiskAssessmentSearch(Convert.ToInt32(ViewState["subcategoryid"]),
				1, sortexpression, sortdirection,
				Int32.Parse(ViewState["PAGENUMBER"].ToString()),
				iRowCount,
				ref iRowCount,
				ref iTotalPageCount);

		Response.AddHeader("Content-Disposition", "attachment; filename=HealthAndSafety.xls");
		Response.ContentType = "application/vnd.msexcel";
		Response.Write("<TABLE BORDER='0' CELLPADDING='2' CELLSPACING='0' width='100%'>");
		Response.Write("<tr>");
		Response.Write("<td><img src='http://" + Request.Url.Authority + Session["images"] + "/esmlogo4_small.png" + "' /></td>");
		Response.Write("<td><h3> Health and Safety</h3></td>");
	...
	protected void MenuHeader_TabStripCommand(object sender, EventArgs e)
	{
		...
		if (dce.CommandName.ToUpper().Equals("ENVIRONMENTAL"))
		{
			Response.Redirect("../Inspection/InspectionRAEnvironmental.aspx?categoryid=" + Request.QueryString["categoryid"] + "&subcategoryid=" + ViewState["subcategoryid"], false);
		}
	...
	protected void HealthAndSafety_TabStripCommand(object sender, EventArgs e)
	{
		DataListCommandEventArgs dce = (DataListCommandEventArgs)e;
		if (dce.CommandName.ToUpper().Equals("SAVE"))
		{
			if (!IsValidHealthandSafety(txtActivity.Text, txtRisk.Text, ucHazardType.SelectedHazardType, ucFrequency.SelectedFrequency, ucActivityDuration.SelectedFrequency, ucRAControl.SelectedFrequency))
			{
				ucError.Visible = true;
				return;
			}
			if (ViewState["assessmentid"].ToString() == "")
			{
				...
				PhoenixInspectionRiskAssessment.InsertRiskAssessment(PhoenixSecurityContext.CurrentSecurityContext.UserCode,
					...
					General.GetNullableInteger(ucRAControl.SelectedFrequency), 1);
				Reset();
				BindData();
			}
			else
	...
	protected void Reset()
	{
		...
		txtLevelOfRisk.Text = "";
		ViewState["assessmentid"] = "";
		gvHealthAndSafety.SelectedIndex = -1;
	}
	private bool IsValidHealthandSafety(string activity,string risk,string hazardtype,string noofpeople,string activityduration,string racontrol)
	{
		...
		if (General.GetNullableInteger(noofpeople) == null)
			ucError.ErrorMessage = "No of people is required.";
	...
	private void BindData()
	{
		try
		{
			...
			General.SetPrintOptions("gvHealthAndSafety", "Health And Safety List", alCaptions, alColumns, ds);

			if (ds.Tables[0].Rows.Count > 0)
			{
				gvHealthAndSafety.DataSource = ds;
			...
			else
			{
				...
				ShowNoRecordsFound(dt, gvHealthAndSafety);
				ViewState["batchid"] = "";
			}
	...
	{
		gvHealthAndSafety.SelectedIndex = -1;
		for (int i = 0; i < gvHealthAndSafety.Rows.Count; i++)
		{
			if (gvHealthAndSafety.DataKeys[i].Value.ToString().Equals(ViewState["assessmentid"]))
			{
				gvHealthAndSafety.SelectedIndex = i;
				ViewState["DTKEY"] = ((Label)gvHealthAndSafety.Rows[gvHealthAndSafety.SelectedIndex].FindControl("lbldtkey")).Text;

			}
		}
	}
	protected void GridHealthAndSafety_TabStripCommand(object sender, EventArgs e)
	{
		DataListCommandEventArgs dce = (DataListCommandEventArgs)e;
	...
	protected void gvHealthAndSafety_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
	...
	protected void gvHealthAndSafety_RowDeleting(object sender, GridViewDeleteEventArgs e)
	{
		BindData();
		SetPageNavigator();
	}
	protected void gvHealthAndSafety_RowEditing(object sender, GridViewEditEventArgs e)
	{
		GridView _gridView = (GridView)sender;
	...
	protected void gvHealthAndSafety_ItemDataBound(Object sender, System.Web.UI.WebControls.GridViewRowEventArgs e)
	{
	...
	protected void gvHealthAndSafety_Sorting(object sender, GridViewSortEventArgs se)
	{
		gvHealthAndSafety.SelectedIndex = -1;
		gvHealthAndSafety.EditIndex = -1;
