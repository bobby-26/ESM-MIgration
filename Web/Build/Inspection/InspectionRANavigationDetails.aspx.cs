...
public partial class InspectionRANavigationDetails : PhoenixBasePage
{
	...
	protected void gvPersonal_ItemDataBound(object sender, GridItemEventArgs e)
	{
        ...
        {
            //LinkButton del = (LinkButton)e.Row.FindControl("cmdDelete");
            //if (del != null) del.Visible = SessionUtil.CanAccess(this.ViewState, del.CommandName);

            //LinkButton eb = (LinkButton)e.Row.FindControl("cmdEdit");
            //if (eb != null) eb.Visible = SessionUtil.CanAccess(this.ViewState, eb.CommandName);

            //LinkButton sb = (LinkButton)e.Row.FindControl("cmdSave");
            //if (sb != null) sb.Visible = SessionUtil.CanAccess(this.ViewState, sb.CommandName);

            //LinkButton cb = (LinkButton)e.Row.FindControl("cmdCancel");
    ...
    protected void gvNavigationSafety_ItemDataBound(object sender, GridItemEventArgs e)
    {
        ...
        if (e.Item is GridDataItem)
        {
            ...
            //RadTextBox txtActualFinishTimeEdit = (RadTextBox)e.Row.FindControl("txtActualFinishTimeEdit");            
    ...
    protected void gvNavigationSafety_ItemCommand(object sender, GridCommandEventArgs gce)
    {
        try
        {
            ...
            {                
                ...
                DateTime actualfinishtime = Convert.ToDateTime(((RadTimePicker)gce.Item.FindControl("txtActualFinishTimeEdit")).SelectedTime);
    ...
    public void rblOptions_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            GridItem gvrow = (GridItem)rblOptions.Parent.Parent;
    ...
    protected void gvPersonal_UpdateCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
            if (!IsValidSpareTools(((RadRadioButtonList)e.Item.FindControl("rblOptionEdit")).SelectedValue))
            {
                ucError.Visible = true;
                return;
            }
            ...
            RadLabel lblItemid = (RadLabel)e.Item.FindControl("lblItemid");
            if (lblId != null && lblId != "")
            {
                PhoenixInspectionRiskAssessmentNavigation.UpdateNavigationControl(PhoenixSecurityContext.CurrentSecurityContext.UserCode,
                                                            new Guid(lblId), new Guid(lblItemid.Text),
                                                            int.Parse(((RadRadioButtonList)e.Item.FindControl("rblOptionEdit")).SelectedValue));
            }
            else
            {
                PhoenixInspectionRiskAssessmentNavigation.InsertNavigationControl(PhoenixSecurityContext.CurrentSecurityContext.UserCode,
                                                            new Guid(lblItemid.Text), int.Parse(((RadRadioButtonList)e.Item.FindControl("rblOptionEdit")).SelectedValue),
                                                            new Guid(ViewState["RISKASSESSMENTNAVIGATIONID"].ToString()),
                                                            int.Parse(ViewState["VESSELID"].ToString()));
            }
            BindGridRiskQuestions();
