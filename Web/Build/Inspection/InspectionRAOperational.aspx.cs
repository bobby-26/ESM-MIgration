...
public partial class InspectionRAOperational : PhoenixBasePage
{
	protected void Page_Load(object sender, EventArgs e)
	{
		try
		{
			...
			toolbargrid.AddImageButton("../Inspection/InspectionRAOperational.aspx", "Export to Excel", "icon_xls.png", "Excel");
			toolbargrid.AddImageLink("javascript:CallPrint('gvOperational')", "Print Grid", "icon_print.png", "PRINT");
			MenuGridOperational.AccessRights = this.ViewState;
			MenuGridOperational.MenuList = toolbargrid.Show();
	...
	protected void ShowExcel()
	{
		...
		ds = PhoenixInspectionRiskAssessment.RiskAssessmentSearch(Convert.ToInt32(ViewState["subcategoryid"]),
				3, sortexpression, sortdirection,
				Int32.Parse(ViewState["PAGENUMBER"].ToString()),
				iRowCount,
				ref iRowCount,
				ref iTotalPageCount);

		Response.AddHeader("Content-Disposition", "attachment; filename=Operational.xls");
		Response.ContentType = "application/vnd.msexcel";
		Response.Write("<TABLE BORDER='0' CELLPADDING='2' CELLSPACING='0' width='100%'>");
		Response.Write("<tr>");
		Response.Write("<td><img src='http://" + Request.Url.Authority + Session["images"] + "/esmlogo4_small.png" + "' /></td>");
		Response.Write("<td><h3> Operational</h3></td>");
	...
	protected void Operational_TabStripCommand(object sender, EventArgs e)
	{
		...
		{
			if (!IsValidOperational(txtActivity.Text, txtRisk.Text, ucHazardType.SelectedHazardType, ucFrequency.SelectedFrequency, ucActivityDuration.SelectedFrequency, ucRAControl.SelectedFrequency))
			{
				ucError.Visible = true;
				return;
			}
			if (ViewState["assessmentid"].ToString() == "")
			{
				...
				PhoenixInspectionRiskAssessment.InsertRiskAssessment(PhoenixSecurityContext.CurrentSecurityContext.UserCode,
					...
					General.GetNullableInteger(ucRAControl.SelectedFrequency), 3);
				Reset();
				BindData();
			}
			else
	...
	protected void Reset()
	{
		...
		txtLevelOfRisk.Text = "";
		ViewState["assessmentid"] = "";
		gvOperational.SelectedIndex = -1;
	}
	private bool IsValidOperational(string activity, string risk, string hazardtype, string noofpeople, string activityduration, string racontrol)
	{
	...
	private void BindData()
	{
		try
		{
			...
			General.SetPrintOptions("gvOperational", "Operational List", alCaptions, alColumns, ds);

			if (ds.Tables[0].Rows.Count > 0)
			{
				gvOperational.DataSource = ds;
				gvOperational.DataBind();

			}
			else
			{
				DataTable dt = ds.Tables[0];
				ShowNoRecordsFound(dt, gvOperational);
				ViewState["batchid"] = "";
			}
	...
	{
		gvOperational.SelectedIndex = -1;
		for (int i = 0; i < gvOperational.Rows.Count; i++)
		{
			if (gvOperational.DataKeys[i].Value.ToString().Equals(ViewState["assessmentid"]))
			{
				gvOperational.SelectedIndex = i;
				ViewState["DTKEY"] = ((Label)gvOperational.Rows[gvOperational.SelectedIndex].FindControl("lbldtkey")).Text;

			}
		}
	}
	protected void GridOperational_TabStripCommand(object sender, EventArgs e)
	{
		DataListCommandEventArgs dce = (DataListCommandEventArgs)e;
	...
	protected void gvOperational_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
	...
	protected void gvOperational_RowDeleting(object sender, GridViewDeleteEventArgs e)
	{
		BindData();
		SetPageNavigator();
	}
	protected void gvOperational_RowEditing(object sender, GridViewEditEventArgs e)
	{
		GridView _gridView = (GridView)sender;
	...
	protected void gvOperational_ItemDataBound(Object sender, System.Web.UI.WebControls.GridViewRowEventArgs e)
	{
	...
	protected void gvOperational_Sorting(object sender, GridViewSortEventArgs se)
	{
		gvOperational.SelectedIndex = -1;
		gvOperational.EditIndex = -1;
