...
public partial class InspectionRAOtherRisk : PhoenixBasePage
{
	protected void Page_Load(object sender, EventArgs e)
	{
		try
		{
			...
			toolbargrid.AddImageButton("../Inspection/InspectionRAOtherRisk.aspx", "Export to Excel", "icon_xls.png", "Excel");
			toolbargrid.AddImageLink("javascript:CallPrint('gvOtherRisk')", "Print Grid", "icon_print.png", "PRINT");
			MenuGridOtherRisk.AccessRights = this.ViewState;
			MenuGridOtherRisk.MenuList = toolbargrid.Show();
			...
			if (!IsPostBack)
			{
				...
				MenuHeader.SelectedMenuIndex = 4;

				toolbar = new PhoenixToolbar();
				toolbar.AddButton("New", "NEW");
				toolbar.AddButton("Save", "SAVE");
				MenuOtherRisk.AccessRights = this.ViewState;
				MenuOtherRisk.MenuList = toolbar.Show();
	...
	protected void ShowExcel()
	{
		...
		string[] alCaptions = { "Activity", "Risk", "Consequences", "Probabilty of Occurance", "Likelyhood of Harm", "Level of Control", "Level of Risk" };
		...
		ds = PhoenixInspectionRiskAssessment.RiskAssessmentSearch(Convert.ToInt32(ViewState["subcategoryid"]),
				4, sortexpression, sortdirection,
				Int32.Parse(ViewState["PAGENUMBER"].ToString()),
				iRowCount,
				ref iRowCount,
				ref iTotalPageCount);

		Response.AddHeader("Content-Disposition", "attachment; filename=OtherRisk.xls");
		Response.ContentType = "application/vnd.msexcel";
		Response.Write("<TABLE BORDER='0' CELLPADDING='2' CELLSPACING='0' width='100%'>");
		Response.Write("<tr>");
		Response.Write("<td><img src='http://" + Request.Url.Authority + Session["images"] + "/esmlogo4_small.png" + "' /></td>");
		Response.Write("<td><h3> Other Risk</h3></td>");
	...
	protected void OtherRisk_TabStripCommand(object sender, EventArgs e)
	{
		DataListCommandEventArgs dce = (DataListCommandEventArgs)e;
		if (dce.CommandName.ToUpper().Equals("SAVE"))
		{
			if (!IsValidOtherRisk(txtActivity.Text, txtRisk.Text, ucHazardType.SelectedHazardType, ucFrequency.SelectedFrequency, ucActivityDuration.SelectedFrequency, ucRAControl.SelectedFrequency))
			{
				ucError.Visible = true;
				return;
			}
			if (ViewState["assessmentid"].ToString() == "")
			{
				...
				PhoenixInspectionRiskAssessment.InsertRiskAssessment(PhoenixSecurityContext.CurrentSecurityContext.UserCode,
					...
					General.GetNullableInteger(ucRAControl.SelectedFrequency), 4);
				Reset();
				BindData();
			}
			else
	...
	protected void Reset()
	{
		...
		txtLevelOfRisk.Text = "";
		ViewState["assessmentid"] = "";
		gvOtherRisk.SelectedIndex = -1;
	}
	private bool IsValidOtherRisk(string activity, string risk, string hazardtype, string activityfrequency, string activityduration, string racontrol)
	{
		...
		if (General.GetNullableInteger(hazardtype) == null)
			ucError.ErrorMessage = "Consequences is required.";
	...
	private void BindData()
	{
		try
		{
			...
			General.SetPrintOptions("gvOtherRisk", "Other Risk List", alCaptions, alColumns, ds);

			if (ds.Tables[0].Rows.Count > 0)
			{
				gvOtherRisk.DataSource = ds;
				gvOtherRisk.DataBind();

			}
			else
			{
				DataTable dt = ds.Tables[0];
				ShowNoRecordsFound(dt, gvOtherRisk);
				ViewState["batchid"] = "";
			}
	...
	{
		gvOtherRisk.SelectedIndex = -1;
		for (int i = 0; i < gvOtherRisk.Rows.Count; i++)
		{
			if (gvOtherRisk.DataKeys[i].Value.ToString().Equals(ViewState["assessmentid"]))
			{
				gvOtherRisk.SelectedIndex = i;
				ViewState["DTKEY"] = ((Label)gvOtherRisk.Rows[gvOtherRisk.SelectedIndex].FindControl("lbldtkey")).Text;

			}
		}
	}
	protected void GridOtherRisk_TabStripCommand(object sender, EventArgs e)
	{
		DataListCommandEventArgs dce = (DataListCommandEventArgs)e;
	...
	protected void gvOtherRisk_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
	...
	protected void gvOtherRisk_RowDeleting(object sender, GridViewDeleteEventArgs e)
	{
		BindData();
		SetPageNavigator();
	}
	protected void gvOtherRisk_RowEditing(object sender, GridViewEditEventArgs e)
	{
		GridView _gridView = (GridView)sender;
	...
	protected void gvOtherRisk_ItemDataBound(Object sender, System.Web.UI.WebControls.GridViewRowEventArgs e)
	{
	...
	protected void gvOtherRisk_Sorting(object sender, GridViewSortEventArgs se)
	{
		gvOtherRisk.SelectedIndex = -1;
		gvOtherRisk.EditIndex = -1;
