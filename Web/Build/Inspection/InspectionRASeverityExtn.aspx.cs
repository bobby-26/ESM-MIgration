...
public partial class InspectionRASeverityExtn : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbar.AddFontAwesomeButton("../Inspection/InspectionRASeverityExtn.aspx", "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "Excel");
            toolbar.AddFontAwesomeButton("javascript:CallPrint('gvRASeverity')", "Print Grid", "<i class=\"fas fa-print\"></i>", "PRINT");
            toolbar.AddFontAwesomeButton("../Inspection/InspectionRASeverityExtn.aspx", "Find", "<i class=\"fas fa-search\"></i>", "FIND");
    ...
    protected void ShowExcel()
    {
        ...
        string[] alColumns = { "FLDSEVERITY", "FLDSCORE", "FLDCONSCATEGORY" };
        string[] alCaptions = { "Severity", "Score", "Consequence" };
        ...
        ds = PhoenixInspectionRiskAssessmentSeverityExtn.RiskAssessmentSeveritySearch(General.GetNullableString(txtName.Text),
            ...
            if (e.Item is GridFooterItem)
            {
                if (e.CommandName.ToUpper().Equals("ADD"))
                {
                    if (!IsValidRASeverity(((RadTextBox)e.Item.FindControl("txtSeverityAdd")).Text,
                            ...
                            ((RadTextBox)e.Item.FindControl("txtconsequenceadd")).Text))
                    {
                        ucError.Visible = true;
                        return;
                    }
                ...
                if (e.CommandName.ToUpper().Equals("UPDATE"))
                {
                    ...
                    if (!IsValidRASeverity(((RadTextBox)e.Item.FindControl("txtSeverityEdit")).Text,
                       ((UserControlMaskNumber)e.Item.FindControl("ucScoreEdit")).Text,
                        ((RadTextBox)e.Item.FindControl("txtconsequenceedit")).Text))
                    {
                        ucError.Visible = true;
                        return;
                    }
    ...
    private void InsertRASeverity(string severity, string score, string Consequence)
    {

        PhoenixInspectionRiskAssessmentSeverityExtn.InsertRiskAssessmentSeverity(PhoenixSecurityContext.CurrentSecurityContext.UserCode,
              severity, int.Parse(score), General.GetNullableString(Consequence));
    }

    private void UpdateRASeverity(int Severityid, string severity, string score, string Consequence)
    {

        PhoenixInspectionRiskAssessmentSeverityExtn.UpdateRiskAssessmentSeverity(PhoenixSecurityContext.CurrentSecurityContext.UserCode,
             Severityid, severity, int.Parse(score), General.GetNullableString(Consequence));
        ucStatus.Text = "Information updated";
    }

    private bool IsValidRASeverity(string severity, string score, string Consequence)
    {
        ...
        if (General.GetNullableString(Consequence) == null)
    ...
    {
        PhoenixInspectionRiskAssessmentSeverityExtn.DeleteRiskAssessmentSeverity(PhoenixSecurityContext.CurrentSecurityContext.UserCode, Severityid);
    }
