...
public partial class InspectionRASubHazardExtn : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                ViewState["TYPENAME"] = "-1";
                if (ViewState["TYPE"].ToString() == "1")
                {
                    ViewState["TYPENAME"] = "Health and Safety Hazard / Impact";
                }
                else if (ViewState["TYPE"].ToString() == "2")
                {
                    ViewState["TYPENAME"] = "Environmental Hazard / Impact";
                }
        ...
        toolbar.AddFontAwesomeButton("../Inspection/InspectionRASubHazardExtn.aspx?" + Request.QueryString["type"], "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "EXCEL");
        toolbar.AddFontAwesomeButton("javascript:CallPrint('gvRASubHazard')", "Print Grid", "<i class=\"fas fa-print\"></i>", "PRINT");
        toolbar.AddFontAwesomeButton("../Inspection/InspectionRASubHazardExtn.aspx?type=" + Request.QueryString["type"], "Find", "<i class=\"fas fa-search\"></i>", "FIND");
        toolbar.AddFontAwesomeButton("../Inspection/InspectionRASubHazardExtn.aspx", "Clear Filter", "<i class=\"fas fa-eraser\"></i>", "CLEAR");
    ...
    protected void ShowExcel()
    {
        ...
        string[] alColumns = { "FLDHAZARDNAME", "FLDSUBHAZARDNAME", "FLDIMPACTNAME", "FLDSEVERITY", "FLDSCORE", "FLDCATEGORY", "FLDUNDESIRABLEEVENT", "FLDWORSTCASE","FLDPPELIST" };
        string[] alCaptions = { "Category", "Hazard", "Impact", "Severity", "Score", "Consequence Category", "Contact Type / Undesirable Event", "Worst Case", "PPE" };
        ...
        ds = PhoenixInspectionRiskAssessmentSubHazardExtn.RiskAssessmentSubHazardSearch(
    ...
    protected void gvRASubHazard_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
    {
        try
        {
            ...
            if ((ViewState["TYPE"].ToString() == "2") || (ViewState["TYPE"].ToString() == "4"))
            {
                gvRASubHazard.Columns[8].Visible = false;
            }
