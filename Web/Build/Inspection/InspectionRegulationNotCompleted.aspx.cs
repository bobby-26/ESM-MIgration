...
public partial class InspectionRegulationNotCompleted : PhoenixBasePage
{
    string RegulationId;
    string screen;
    bool IsMoc;
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ViewState["RegulationActionID"]= Request.QueryString["RegulationActionID"]; 
    ...
    private void ShowToolBar()
    {
        ...
        toolbar.AddFontAwesomeButton("../Inspection/InspectionRegulationNotCompleted.aspx?RegulationId=" + RegulationId + "&RegulationActionID=" + ViewState["RegulationActionID"], "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "Excel");
    ...
    private DataSet GetRegulationVesselList(ref int iRowCount, ref int iTotalPageCount)
    {
        ...
        DataSet ds = PhoenixInspectionNewRegulationActionPlan.RegulationSummaryListNotClosed(new Guid(RegulationId), IsComplian, (int)ViewState["PAGENUMBER"], gvRegulation.PageSize, ref iRowCount, ref iTotalPageCount, General.GetNullableInteger(ViewState["COMPANYID"].ToString()),
            ViewState["RegulationActionID"]==null? null:General.GetNullableGuid(ViewState["RegulationActionID"].ToString()));
        return ds;
    }
    ...
    {
        string[] alColumns = { "FLDVESSELNAME", "FLDACTIONTOBETAKEN", "FLDTARGETDATE", "FLDCOMPLETIONDATE", "FLDSTATUS" };
        return alColumns;
    }
    private string[] getAlCaptions()
    {
        string[] alCaptions = { "Vessel", "Task", "Target Date", "Completion Date", "Status" };
        return alCaptions;
    }
