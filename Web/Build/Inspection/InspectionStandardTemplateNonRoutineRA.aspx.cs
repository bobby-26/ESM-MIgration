...
public partial class InspectionStandardTemplateNonRoutineRA : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        SessionUtil.PageAccessRights(this.ViewState);
        PhoenixToolbar toolbar = new PhoenixToolbar();
        toolbar.AddFontAwesomeButton("../Inspection/InspectionStandardTemplateNonRoutineRA.aspx", "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "Excel");
        ...
        toolbar.AddFontAwesomeButton("../Inspection/InspectionStandardTemplateNonRoutineRA.aspx", "Clear Filter", "<i class=\"fas fa-eraser\"></i>", "CLEAR");
    ...
    private void BindData()
    {
        ...
        DataSet ds = PhoenixInspectionRiskAssessmentGenericExtn.NonRoutineStandardRiskAssessmentDashboardSearch(
    ...
    protected void gvRiskAssessment_ItemCommand(object sender, GridCommandEventArgs gce)
    {
        try
        {
            ...
            if (gce.CommandName.ToUpper().Equals("EDITROW"))
            {
                ...
                if (lbltypeid.Text == "1")
                {
                    Response.Redirect("../Inspection/InspectionRAGenericExtn.aspx?StandardRA=1&showall=1&genericid=" + lbl.Text + "&status=" + lblstatus.Text + "&HSEQADashboardYN=1", false);
                }
                if (lbltypeid.Text == "2")
                {
                    Response.Redirect("../Inspection/InspectionRANavigationExtn.aspx?StandardRA=1&showall=1&navigationid=" + lbl.Text + "&status=" + lblstatus.Text + "&HSEQADashboardYN=1", false);
                }
                if (lbltypeid.Text == "3")
                {
                    Response.Redirect("../Inspection/InspectionRAMachineryExtn.aspx?StandardRA=1&showall=1&machineryid=" + lbl.Text + "&status=" + lblstatus.Text + "&HSEQADashboardYN=1", false);
                }
                if (lbltypeid.Text == "4")
                {
                    Response.Redirect("../Inspection/InspectionRACargoExtn.aspx?StandardRA=1&showall=1&genericid=" + lbl.Text + "&status=" + lblstatus.Text + "&HSEQADashboardYN=1", false);
                }
            }
