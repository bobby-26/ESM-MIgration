...
public partial class InventoryComponentChangeRequestGeneral : PhoenixBasePage
{
    //protected override void Render(HtmlTextWriter writer)
    //{
    //    if (!IsPostBack && ViewState["COMPONENTID"] != null)
    //    {
    //        if (Request.QueryString["Type"] == "New" && Request.QueryString["parent"] != null && Request.QueryString["parent"].ToString() != "")
    //            tvwComponent.FindNodeByValue(tvwComponent.ThisTreeView.Nodes, Request.QueryString["parent"].ToString());
        ...
        try
        {
            if (ViewState["COMPONENTID"] != null && ViewState["COMPONENTID"].ToString() != "")
            {
                ...
                if (ds.Tables[0].Rows.Count > 0 )
                {
                    ...
                    {
                        txtParentComponentIDChange.Text = ViewState["COMPONENTID"].ToString();
                        ...
                        imgShowParentComponentChange.Disabled = true;
                ...
                if (Session["CHANGETYPE"].ToString() != "0" && Session["CHANGETYPE"].ToString() != string.Empty)
                {
                    ...
                    imgShowParentComponentChange.Disabled = false;
                    divOldValues.Visible = true;
                }
    ...
    protected void MenuRegistersComponent_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            if (CommandName.ToUpper().Equals("SAVE"))
            {
                ...
                if (!IsValidComponent(CompNo, txtComponentNameChange.Text, txtParentComponentIDChange.Text, ucComponentStatusChange.SelectedHard))
                {
                    ucError.Visible = true;
                    return;
                }
                ...
                if ((String)ViewState["OPERATIONMODE"] == "EDIT")
                {
                    PhoenixInventoryComponentChangeRequest.UpdateChangeRequestComponent(PhoenixSecurityContext.CurrentSecurityContext.UserCode, PhoenixSecurityContext.CurrentSecurityContext.VesselID
                        ...
                        , txtParentComponentIDChange.Text, General.GetNullableInteger(txtVendorIdChange.Text)
                        , General.GetNullableInteger(txtMakerIdChange.Text), General.GetNullableString(txtLocationChange.Text)
                        , null
                        , null
            ...
            else if (CommandName.ToUpper().Equals("BACK"))
            {
                ...
                Response.Redirect("../Inventory/InventoryComponentChangeRequest.aspx", false);
            }
        }
        catch (Exception ex)
        {
    ...
    protected void FillEditValues(DataRow dr2)
    {
        ...
        txtMakerIdChange.Text = dr2["FLDMAKERID"].ToString();
        txtMakerCodeChange.Text = dr2["MAKERCODE"].ToString();
        txtMakerNameChange.Text = dr2["MAKERNAME"].ToString();
        txtVendorIdChange.Text = dr2["FLDVENDORID"].ToString();
        ...
        txtParentComponentNameChange.Text = dr2["FLDPARENTID"].ToString();
        txtParentComponentIDChange.Text = dr2["PARENTCOMPONENTID"].ToString();
    ...
    protected void cmdVendorChangeClear_Click(object sender, ImageClickEventArgs e)
    {
        txtVendorCodeChange.Text = "";
        txtVendorNameChange.Text = "";
        txtVendorIdChange.Text = "";
    ...
    protected void cmdParentComponentChangeClear_Click(object sender, ImageClickEventArgs e)
    {
        txtParentComponentNumberChange.Text = "";
        txtParentComponentNameChange.Text = "";
        txtParentComponentIDChange.Text = "";
