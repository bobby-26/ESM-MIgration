...
public partial class InventorySpareItemRequestGeneral : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                cmdShowItem.OnClientClick = "return showPickList('spnPickItem', 'codehelp1', '', '" + Session["sitepath"] + "/Common/CommonPickListSpareItemByComponent.aspx?mode=custom&framename=ifMoreInfo&vesselid=" + PhoenixSecurityContext.CurrentSecurityContext.VesselID.ToString() + "&txtnumber='+ document.getElementById('" + txtNumber.UniqueID + "').value, true);";
            ...
            if (CommandName.ToUpper().Equals("SAVE"))
            {
                ...
                else if (ViewState["REQUESTID"] == null || ViewState["REQUESTID"].ToString() == "")
                {
                    ...
                    PhoenixInventorySpareItemRequest.InsertSpareItemRequest(PhoenixSecurityContext.CurrentSecurityContext.VesselID
                        ...
                        , byte.Parse(ddlChangeReqType.SelectedValue)
                        , CompNo
                        , txtName.Text, General.GetNullableInteger(txtMakerId.Text)
                        , txtMakerReference.Text
                        , General.GetNullableInteger(txtVendorId.Text)
                        , null
                        , Convert.ToInt32(ddlStockUnit.SelectedValue)
