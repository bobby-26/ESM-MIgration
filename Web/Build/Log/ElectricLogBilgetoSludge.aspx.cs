...
public partial class Log_ElectricLogBilgetoSludge : PhoenixBasePage
{
    string ReportCode = "D";
    string ItemNo = "13";
    ...
    {
        string logName = "Bilge to Sludge";

        if (isMissedOperation && isMissedOperationEdit == false)
        {
            Guid TranscationId, logId;
            TranscationInsert(usercode, out TranscationId, out logId);
    ...
    private void TranscationInsert(int usercode, out Guid TranscationId, out Guid logId)
    {
    ...
    private void SetDefaultData()
    {
        ...
        lblRecord.Text = string.Format("<b>{0}</b> m3 bilge water from <b>{1}</b>  now {2} m3", FromTransferQty, ddlTransferFrom.SelectedItem.Text, FromTankAfter);
        lblTimeRecord.Text = string.Format("Start:    <b>{0}</b>  ,  Stop:  <b>{1}</b> ", startTime.ToString(), stopTime.ToString());
        lbltorecord.Text = string.Format("Collected in <b>{0}</b>, Retained in tank  <b>{1}</b> m3 ", ddlTransferTo.SelectedItem.Text, ToTankAfter);
    }
