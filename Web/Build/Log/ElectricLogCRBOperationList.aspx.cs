...
public partial class Log_ElectricLogCRBOperationList : PhoenixBasePage
{
    ...
    string url = "ElectricLogCRBOperationList.aspx";
    string logbook = "CRB";
    ...
    private void SetLatestPage()
    {
        ...
        PhoenixMarpolLogCRB.LogCRBBookEntrySearch(PhoenixSecurityContext.CurrentSecurityContext.UserCode
    ...
    public void GetMasterSignedName(string txtIds)
    {
        ...
        DataSet ds = PhoenixMarpolLogCRB.MasterSignatureCRBSelect(usercode, txtIds);
    ...
    {
        DataSet ds = PhoenixMarpolLogCRB.LogCRBStatusList();
    ...
    private void SetMasterSignature()
    {
        if (Filter.MasterSignatureCriteria != null)
        {
            ...
            PhoenixMarpolLogCRB.MasterSignatureCRB(masterUserCode, txids, masterName, masterSignedDate);
    ...
    private void BindData()
    {
        ...
        DataSet ds = PhoenixMarpolLogCRB.LogCRBBookEntrySearch(PhoenixSecurityContext.CurrentSecurityContext.UserCode
                                         , PhoenixSecurityContext.CurrentSecurityContext.VesselID
                                         , General.GetNullableInteger(ddlStatus.SelectedValue)
                                         , General.GetNullableDateTime(txtFromDate.Text)
                                         , General.GetNullableDateTime(txtToDate.Text)
    ...
    private DataSet GetSSRSData(ref int iRowCount, ref int iTotalPageCount, DateTime? startDate, DateTime? endDate, int? pageNo)
    {
        ...
        DataSet ds = PhoenixMarpolLogCRB.LogCRBBookEntryReportSearch(PhoenixSecurityContext.CurrentSecurityContext.UserCode
                                                 , PhoenixSecurityContext.CurrentSecurityContext.VesselID
                                                 , General.GetNullableInteger(ddlStatus.SelectedValue)
    ...
    protected void gvCounterUpdate_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                EventLog(url, caption, CommandName, logbook);
                string location = HttpContext.Current.Request.MapPath("~\\Template\\Log") + @"\IRIguidance.pdf";
                string navigateUrl = string.Format("../Reports/ReportsDownload.aspx?filename={0}&type=pdf", location);
            ...
            if (CommandName.ToUpper().Equals("ADD"))
            {
                ...
                string script = String.Format("javascript:parent.openNewWindow('LogMasterSign','','" + Session["sitepath"] + "/Log/ElectricLogCRBOperation.aspx',null, null,null,null,null,{{model:true}});");
            ...
            {
                EventLog(url, caption, CommandName, logbook);
                string script = String.Format("javascript:parent.openNewWindow('ifMoreInfo','','" + Session["sitepath"] + "/Log/ElectricLogCRBTankList.aspx',null, null,null,null,null,{{model:true}});");
                ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "script", script, true);
                gvElogTransaction.Rebind();
            }
            ...
            if (CommandName.ToUpper().Equals("TRANSCATIONHISTORY"))
            {
                EventLog(url, caption, CommandName, logbook);
                string txids = GetLogIDFromGrid();
                string script = String.Format("javascript:parent.openNewWindow('LogMasterSign','','" + Session["sitepath"] + "/Log/ElectricLogCRBTranscationHistory.aspx?txid={0}',null, null,null,null,null,{{model:true}});", txids);
                ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "script", script, true);
                gvElogTransaction.Rebind();
            }
            ...
            if (CommandName.ToUpper().Equals("MASTERUNLOCK"))
            {
                ...
                PhoenixMarpolLogCRB.MasterUnlockTheLogCRB(usercode, logids);
                gvElogTransaction.Rebind();
            }

            if (CommandName.ToUpper().Equals("MISC"))
            {
                string script = String.Format("javascript:parent.openNewWindow('Log','','" + Session["sitepath"] + "/Log/ElectricLogCRBAdditionalOperation.aspx',null, null,null,null,null,{{model:true}});");
                ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "script", script, true);
            }
    ...
    private void ExportPDF(int? pageNo, DateTime? startDate, DateTime? endDate)
    {
        ...
        if (PhoenixGeneralSettings.CurrentGeneralSetting.ReportType == "1")
        {
            ...
            nvc.Add("reportcode", "OPERATIONREPORTCRB");
            nvc.Add("CRITERIA", "");
    ...
    protected void gvElogTransaction_ItemCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName == "DELETE")
            {
                ...
                string scriptpopup = "javascript:parent.openNewWindow('LogDelete','','" + Session["sitepath"] + "/Log/ElectricLogCRBBookEntryDelete.aspx?TxnId=" + lblTxnId.Text + "', 'false', '400', '250', null, null, { 'disableMinMax': true });";                
                ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "script", scriptpopup, true);
            }
    ...
    private void DisplayButton(GridItemEventArgs e)
    {
        ...
        bool isSignature = Convert.ToBoolean(drv["FLDISSIGNATURE"] == DBNull.Value ? false : drv["FLDISSIGNATURE"]);
