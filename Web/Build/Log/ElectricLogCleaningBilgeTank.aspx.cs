...
public partial class Log_ElectricLogCleaningBilgeTank : PhoenixBasePage
{
    string ReportCode = "I";
    string ItemNo = null;
    int usercode = 0;
    ...
    private void SetDefaultData()
    {
        ...
        lblRecord.Text = string.Format("{0}  cleaned", ddlTransferFrom.SelectedItem.Text);
        lblRecord1.Text = string.Format("as per manufacturer’s instruction manual");
        lblRecord2.Text = string.Format("start time <b>{0}</b>  stop time <b>{1}</b>", startTime, stopTime);
    }
    ...
    public void BindData()
    {
        try
        {
            ...
            ds = PhoenixElog.CleaningBilgeTankSearch(usercode, vesselId, txid);
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                RadComboBoxItem fromItem = ddlTransferFrom.Items.FindItem(x => x.Text == dr["FLDLOCATIONNAME"].ToString());
                if (fromItem != null)
                {
    ...
    {
        string logName = "Cleaning Bilge Tank";
    ...
    private void TransactionInsert(out Guid TranscationId, out Guid logId)
    {
        ...
        logId = Guid.NewGuid();
        PhoenixElog.CleaningBilgeTankInsert(usercode
                    , vesselId
    ...
    {
        PhoenixElog.CleaningBilgeTankUpdate(usercode
                                        , txid
                                        , vesselId
                                        , txtOperationDate.SelectedDate.Value
                                        , txtStartTime.SelectedDate.Value
    ...
    public bool isValidInput()
    {
       ucError.HeaderMessage = "Please provide the following required information";
        ...
        {
            ucError.ErrorMessage = "start time is must";
        }

        if (txtStopTime.SelectedDate.HasValue == false)
        {
            ucError.ErrorMessage = "stop time is must";
        }

        if (String.IsNullOrWhiteSpace(ddlTransferFrom.SelectedItem.Text))
        {
            ucError.ErrorMessage = "Bilge Tank is must";
        }

        if (PhoenixElog.IsValidTranscationDate(General.GetNullableDateTime((string)ViewState["lastTranscationDate"]), txtOperationDate.SelectedDate.Value) == false && isMissedOperationEdit == false)
        {
