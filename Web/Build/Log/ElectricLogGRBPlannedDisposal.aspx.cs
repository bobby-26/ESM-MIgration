...
public partial class Log_ElectricLogGRBPlannedDisposal : PhoenixBasePage

{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        logId = PhoenixMarpolLogGRB.GetGRBLogId(usercode, "PlannedDisposal");
    ...
    {
        string[] methods = new string[] { "Discharge into sea", "Discharge to reception", "Incineration" };
    ...
    {
        ddlCategory.DataSource = PhoenixMarpolLogGRB.GetCategoryList();
    ...
    {
        DataTable dt = PhoenixMarpolLogGRB.GRBLogRegisterEdit(usercode, logId);
        if (dt.Rows.Count > 0)
        {
    ...
    private void BindData()
    {
        ...
        {
            DataTable dt = PhoenixMarpolLogGRB.BookEntryReportSearchById(usercode, vesselId ,logBookId);
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                //txtAmountDischargeSea.Text = row["FLDDISCHARGEINTOSEA"] == DBNull.Value ? null : Convert.ToDecimal(row["FLDDISCHARGEINTOSEA"]).ToString();
                //txtAmountDischargeReception.Text = row["FLDDISCHARGEINTORF"] == DBNull.Value ? "" : Convert.ToDecimal(row["FLDDISCHARGEINTORF"]).ToString();
                //txtAmountIncineration.Text = row["FLDINCINERATED"] == DBNull.Value ? null : Convert.ToDecimal(row["FLDINCINERATED"]).ToString();
    ...
    {
        ViewState["lastTranscationDate"] = PhoenixMarpolLogGRB.GetLastTransactionDate(vesselId, usercode);
    }
    ...
    private void logBookEntryUpdate()
    {
        ...
        PhoenixMarpolLogGRB.BookEntryUpdate(usercode, vesselId, logBookId, logId, dtPicker.SelectedDate.Value,
            txtVesselName.Text, ddlCategory.SelectedValue, amtDischargeIntoSea, amtDischargeIntoReception, incineration
            , txtRemarks.Text, null);

        PhoenixMarpolLogGRB.LogGRBBookEntryStatusUpdate(usercode, logId, logBookId, Convert.ToInt32(lblinchId.Text), lblinchName.Text, lblincRank.Text, Convert.ToDateTime(lblincSignDate.Text), null, null, null, null, (bool)ViewState["isAttachmentAttached"]);
    }
    ...
    private void logBookEntryInsert()
    {
        ...
        PhoenixMarpolLogGRB.BookEntryInsert(usercode, vesselId, logId, dtPicker.SelectedDate.Value,
            txtVesselName.Text, ddlCategory.SelectedValue, amtDischargeIntoSea, amtDischargeIntoReception, incineration
            , txtRemarks.Text, null, false, transferDtKey, false, ref logBookTransferId);


        if (ddlMethod.SelectedItem.Value == "Incineration")
        {
            PhoenixMarpolLogGRB.LogGRBBookEntryStatusInsert(usercode, logId, logBookTransferId, Convert.ToInt32(lblinchId.Text), lblinchName.Text, lblincRank.Text, Convert.ToDateTime(lblincSignDate.Text), null, null, null, null, true, (bool)ViewState["isAttachmentAttached"]);
        }
        else
        {
            PhoenixMarpolLogGRB.LogGRBBookEntryStatusInsert(usercode, logId, logBookTransferId, Convert.ToInt32(lblinchId.Text), lblinchName.Text, lblincRank.Text, Convert.ToDateTime(lblincSignDate.Text), null, null, null, null, false, (bool)ViewState["isAttachmentAttached"]);
        }
    ...
    public bool isValidInput()
    {
        ...
        {
            ucError.ErrorMessage = "Value cannot be negative or zero";
        }
