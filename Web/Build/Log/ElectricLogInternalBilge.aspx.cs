...
public partial class Log_ElectricLogInternalBilge : PhoenixBasePage
{
    string ReportCode = "D";
    string ItemNo = "13";
    ...
    protected void bfr_selectedchanged(object sender, EventArgs e)
    {
        SetDefaultData();
    }
    protected void To_selectedchanged(object sender, EventArgs e)
    {
        SetDefaultData();
    }
    private void CalculateROB()
    {
        ...
        {
            txtAfrTrnsROBTo.Text = ((Convert.ToDecimal(txtBfrTrnsROBFrom.Text) - Convert.ToDecimal(txtAftrTrnsROBFrom.Text)) + Convert.ToDecimal(txtBfrTrnsROBTo.Text)).ToString();
            SetDefaultData();
        }
    }
    private void SetDefaultData()
    {
        ...
        lblRecord.Text = string.Format("<b>{0}</b> m3 bilge water from <b>{1}</b>  <b>{2}</b> m3 retained ", transferQty, ddlTransferFrom.SelectedItem.Text, FromTankAfter);
        lblTimeRecord.Text = string.Format("Start:    <b>{0}</b>  ,  Stop:  <b>{1}</b> ", startTime.ToString(), stopTime.ToString());
        lbltorecord.Text = string.Format("To    <b>{0}</b>  ,  Retained in tank    <b>{1}</b>   m3", ddlTransferTo.SelectedItem.Text, ToTankAfter);
    }
    ...
    public bool isValidInput()
    {
        ...
        {
            ucError.ErrorMessage = "Bilge Transfer To is required";
        }
