...
public partial class Log_ElectricLogORB2CargoLoading : PhoenixBasePage
{
    string ReportCode;
    int usercode = 0;
    int vesselId = 0;
    ...
    {        
        ...
        logId = PhoenixMarbolLogORB2.GetORB2LogId(usercode, "LoadingCargo");
        ...
        if (txid != Guid.Empty)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    if (Convert.ToInt32(row["FLDITEMID"]) == 1)
                    {
                        txtOperationDate.SelectedDate = Convert.ToDateTime(row["FLDVALUE"]);
                    }

                    if (Convert.ToInt32(row["FLDITEMID"]) == 2)
                    {
                        txtLoadTerminal.Text = row["FLDVALUE"].ToString();
                    }

                    if (Convert.ToInt32(row["FLDITEMID"]) == 3)
                    {
                        txtTotalQuantityLoaded.Text = row["FLDVALUE"].ToString();
                    }
    ...
    private void SetDefaultData()
    {
        ...
        lblRecord.Text = string.Format("<b>{0}</b>", txtLoadTerminal.Text);
        lblrecord1.Text = string.Format("<span class='left'><b>{0}</b></span>  <span class='right'><b>{1}</b></span>", txtGradeofCargo.Text, ViewState["TankName"].ToString().TrimEnd(','));
        lblrecord2.Text = string.Format("Total Quantity Loaded	<b>{0}</b>	m3	(GSV)", txtTotalQuantityLoaded.Text);
        lblrecord3.Text = string.Format("<b>{0}</b>", ViewState["TankLoaded"].ToString().TrimEnd(','));
    ...
    public bool isValidInput()
    {
        ...
        if (string.IsNullOrWhiteSpace(txtLoadTerminal.Text))
        {
            ucError.ErrorMessage = "Load Terminal is Required";
        }

        if (string.IsNullOrWhiteSpace(txtTotalQuantityLoaded.Text))
        {
            ucError.ErrorMessage = "Total Quantity Loaded is Required";
        }
    ...
    protected void txtOperationDate_SelectedDateChanged1(object sender, EventArgs e)
    {
        SetDefaultData();
    }
    ...
    protected void txtLoadTerminal_TextChanged(object sender, EventArgs e)
    {
        SetDefaultData();
    }
