...
public partial class Log_ElectricLogORB2TankList : PhoenixBasePage
{
    int usercode = 0;
    int vesselId = 0;
    ...
    private void ShowToolBar()
    {
        ...
        if (IsDraft())
        {
            toolbarmain.AddFontAwesomeButton(Session["sitepath"] + "/Log/ElectricLogORB2TankList.aspx", "Select Tank", "<i class=\"fa fa-plus-circle\"></i>", "ADD");
            toolbarmain.AddLinkButton(Session["sitepath"] + "/Log/ElectricLogORB2TankList.aspx", "Submit", "SUBMIT", ToolBarDirection.Right);
            toolbarmain.AddLinkButton(Session["sitepath"] + "/Log/ElectricLogORB2TankList.aspx", "Save", "SAVE", ToolBarDirection.Right);
        }
        else
        {
            toolbarmain.AddLinkButton(Session["sitepath"] + "/Log/ElectricLogORB2TankList.aspx", "Revise", "REVISE", ToolBarDirection.Right);
        }

        toolbarmain.AddFontAwesomeButton(Session["sitepath"] + "/Log/ElectricLogORB2TankList.aspx", "Download pdf", "<img src='../css/Theme1/images/pdf.png' title='Download pdf'>", "PDF");
        //toolbarmain.AddFontAwesomeButton(Session["sitepath"] + "/Log/ElectricLogORB2TankList.aspx", "Download excel", "<img src='../css/Theme1/images/icon_xls.png' title='Download excel'>", "EXCEL");
        ...
        toolbarmain.AddFontAwesomeButton("javascript: openNewWindow('TankRevision', '', '" + Session["sitepath"] + "/Log/ElectricORB2LogRevision.aspx'); return false;", "Revision", "<img src='../css/Theme1/images/requisition.png' title='Revision'>", "REVISION");
    ...
    {
        DataSet ds = PhoenixMarbolLogORB2.GetORB2CurrentRevisionNumber(usercode, vesselId);
    ...
    {
        bool isDraft = true;
        DataSet ds = PhoenixMarbolLogORB2.LogORB2LocationIsDraft(usercode, vesselId);
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
            DataRow row = ds.Tables[0].Rows[0];
    ...
    {
        DataSet ds = PhoenixMarbolLogORB2.LogORB2LocationGetAttachment(usercode, vesselId);
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
            DataRow row = ds.Tables[0].Rows[0];
    ...
    private DataSet GetGridData(ref int TotalPageCount)
    {
        ...
        return PhoenixMarbolLogORB2.ListORB2Location(usercode, vesselId, (int)ViewState["PAGENUMBER"], 100, ref iRowCount, ref TotalPageCount);
    }
    ...
    protected void gvTankList_ItemCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName.ToUpper().Equals("DELETE"))
            {
                ...
                PhoenixMarbolLogORB2.LocationORB2Delete(usercode, locationId, vesselId);
                Rebind();
            }
            ...
            if (e.CommandName.ToUpper().Equals("UPDATE"))
            {
                ...
                {
                    String script = string.Format("javascript:openNewWindow('tankEdit','','{0}/LOG/ElectricLogORB2TankEdit.aspx?LocationId={1}', 'false', '600', '400');", Session["sitepath"], LocationId.Text);
    ...
    protected void gvTabStrip_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                string script = string.Format("javascript:parent.openNewWindow('tankAdd','','{0}/Log/ElectricLogORB2TankAdd.aspx', null, 600, 400);", Session["sitepath"]);
            ...
            if (CommandName.ToUpper().Equals("SUBMIT"))
            {
                ...
                PhoenixMarbolLogORB2.LogORB2LocationAddRevision(usercode, vesselId);
            ...
            {
                PhoenixMarbolLogORB2.LogORB2LocationRevisie(usercode, vesselId);
                Rebind();
            }
        }
        catch (Exception ex)
