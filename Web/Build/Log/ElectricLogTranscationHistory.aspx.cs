...
public partial class Log_ElectricLogTranscationHistory : PhoenixBasePage
{
    int usercode = 0;
    int vesselId = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (IsPostBack == false)
        {
            ...
            toolbar.AddButton("Transaction Log", "TRANS");
            toolbar.AddButton("Event Log", "Event");
    ...
    protected void MenuDocument_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            if (CommandName.ToUpper().Equals("EVENT"))
            {
                Response.Redirect("../Log/ElectricLogEventLogHistory.aspx?txid=" + txid);
            }
        }
        catch (Exception ex)
        {
    ...
    private DataSet HistoryData(ref int rowCount)
    {
        ...
        return PhoenixElog.LogBookHistorySearch(usercode, vesselId, 1, gvTranscationHistory.PageSize, ref iRowCount, ref iTotalPageCount, txid, fromDate, toDate);
    }
