...
public partial class OptionsCrewAppraisalAlert : PhoenixBasePage
{
    ...
    private void GetAlertItems(string tasktype)
    {
        ...
        if (tasktype.Equals("31"))
        {
            gvAlertsTask.DataSource = PhoenixRegistersAlerts.CrewAppraisalAlert(
                PhoenixSecurityContext.CurrentSecurityContext.UserCode, PhoenixSecurityContext.CurrentSecurityContext.InstallCode
    ...
    protected void gvAlertsTask_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (int.Parse(tasktype) == 31)
        {
            ...
            Script += "parent.OpenSearchPage('Crew/CrewPersonalGeneral.aspx?appraisal=1&empid=" + ((Label)_gridView.Rows[nRow].FindControl("lblExpression")).Text + "', 'CRW-PER-PER');";
            Script += "</script>" + "\n";
