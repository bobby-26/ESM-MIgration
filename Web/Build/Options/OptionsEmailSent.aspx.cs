...
public partial class OptionsEmailSent : PhoenixBasePage
{
    ...
    protected void ShowSentMail(object sender, CommandEventArgs e)
    {
        try
        {
            ...
            Response.Redirect("OptionsEmail.aspx?mailId=" + mailid + "&sessionId=" + sessionid + "&mailFolderId=" + PhoenixMail.Folder.Sent + "");
        }
        catch (Exception ex)
        {
            throw ex;
    ...
    {      
        ...
        DataSet ds = PhoenixMail.MailSearch(General.GetNullableInteger(Filter.CurrentCrewSelection).Value, PhoenixMail.Folder.Sent
        ...
        {
            gvSent.DataSource = ds;
            gvSent.DataBind();
        }
        else
        {
            DataTable dt = ds.Tables[0];
            ShowNoRecordsFound(dt, gvSent);
        }
    ...
    protected void gvSent_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow && e.Row.RowState != DataControlRowState.Edit && e.Row.RowState != (DataControlRowState.Alternate | DataControlRowState.Edit))
        {
            ...
            if (!string.IsNullOrEmpty(lblpriority.Text))
            {
                ...
                if (!System.IO.Directory.Exists(strAttachmentPath + mailsessionid))
                {
                ...
                {
                    string unit = "KB";
                    string[] files = Directory.GetFiles(strAttachmentPath + mailsessionid);
                    double fileSize = 0;
