...
public partial class Owners_OwnersMonthlyReportsRestHours : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                lnkExpenses.Attributes.Add("onclick", "javascript:openNewWindow('code1', 'Work And Rest Hours Summary', '" + Session["sitepath"] + "/Owners/OwnersMonthlyReportsRestHoursView.aspx',500,800); return false;");
                lnkWorkandRestHoursComments.Attributes.Add("onclick", "javascript:parent.openNewWindow('codehelp1', 'Work and Rest Hours', '" + Session["sitepath"] + "/Inspection/InspectionOwnersReportComments.aspx?VESSELID=" + Filter.SelectedOwnersReportVessel + "&DATE=" + Filter.SelectedOwnersReportDate + "&CODE=WRH'); return false;");
                CheckComments();
    ...
    private void CheckComments()
    {
        ...
        dt = PhoenixOwnerReportComments.ListOwnerReportsComments("WRH", DateTime.Parse(Filter.SelectedOwnersReportDate), int.Parse(Filter.SelectedOwnersReportVessel));
        if (dt.Rows.Count > 0)
        {
            HtmlGenericControl html = new HtmlGenericControl();
            html.InnerHtml = "<span class=\"icon\"> <i class=\"fa fa-Chat\"></i></span>";
            lnkWorkandRestHoursComments.Controls.Add(html);
        }
    }
