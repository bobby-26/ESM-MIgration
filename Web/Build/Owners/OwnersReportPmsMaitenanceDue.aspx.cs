...
public partial class OwnersReportPmsMaitenanceDue : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                ViewState["POSTPONEDYN"] = Request.QueryString["PostponedYN"] ?? "0";
            ...
            toolbarmain.AddFontAwesomeButton("../Owners/OwnersReportPmsMaitenanceDue.aspx?" + Request.QueryString.ToString(), "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "Excel");
            toolbarmain.AddFontAwesomeButton("javascript:CallPrint('gvWorkOrder')", "Print Grid", "<i class=\"fas fa-print\"></i>", "PRINT");
            toolbarmain.AddFontAwesomeButton("../Owners/OwnersReportPmsMaitenanceDue.aspx?" + Request.QueryString.ToString(), "Clear Filter", "<i class=\"fas fa-eraser\"></i>", "CLEAR");
    ...
    protected void ShowExcel()
    {
        try
        {
            ...
            DataTable dt = PhoenixOwnerReportPMS.OwnersReportMaintenanceDue(General.GetNullableInteger(Filter.SelectedOwnersReportVessel)
                    , null
                   ...
                   , General.GetNullableDateTime(Filter.SelectedOwnersReportDate)
                    , int.Parse(ViewState["POSTPONEDYN"].ToString()));
    ...
    protected void gvWorkOrder_ItemDataBound(object sender, GridItemEventArgs e)
    {
        ...
        if (e.Item is GridDataItem)
        {
           ...
           // if (int.Parse(Filter.SelectedOwnersReportVessel) == 0)
            {
                vslid = "&vslid=" + Filter.SelectedOwnersReportVessel.ToString();
            }
            ...
            if (lblGroupNo != null)
            {
                ...
                {
                    lblGroupNo.Attributes.Add("onclick", "javascript:openNewWindow('codehelp1','','" + Session["sitepath"] + "/PlannedMaintenance/PlannedMaintenanceSubWorkOrderList.aspx?groupId=" + drv["FLDWORKORDERGROUPID"] + vslid + "'); return false;");
                }
            }
    ...
    protected void MenuWorkOrder_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            else if (CommandName.ToUpper().Equals("SAVE"))
            {
                ...
                PhoenixPlannedMaintenanceWorkOrderGroup.InsertSubJob(new Guid(ViewState["GROUPID"].ToString()), csvjobList, int.Parse(Filter.SelectedOwnersReportVessel));
