...
public partial class PlannedMaintenanceComponentJobGeneral : PhoenixBasePage
{
    ...
    private void BindFields()
    {
        try
        {
            ...
            else
            {
                txtPriority.Text = "3";
                ucMandatory.YesNoOption = "Yes";
                foreach (RadListBoxItem li in cblJobStatus.Items) li.Selected = (li.Value == "35" ? true : false);
    ...
    protected void PlannedMaintenanceComponent_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                if (!IsValidComponent(txtComponentJobId.Text, txtJobId.Text
                ...
                if (ViewState["REQUESTID"] != null)
                {
                   ...
                   , General.GetNullableGuid(txtJobId.Text).Value, General.GetNullableGuid(txtComponentJobId.Text).Value
                   ...
                   , General.GetNullableDecimal(txtDuration.Text), byte.Parse(ucMandatory.YesNoOption == "Yes" ? "1" : "0")
            ...
            else if (CommandName.ToUpper().Equals("NEW"))
            {
                ...
                foreach (RadListBoxItem li in cblOJobStatus.Items) li.Selected = (li.Value == "35" ? true : false);
                txtOReference.Text = "";
    ...
    {
        if (General.GetNullableGuid(txtComponentJobId.Text).HasValue)
        {
            DataSet ds = PhoenixPlannedMaintenanceComponentJob.EditComponentJob(new Guid(txtComponentJobId.Text), PhoenixSecurityContext.CurrentSecurityContext.VesselID);
            if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                DataRow dr = ds.Tables[0].Rows[0];
