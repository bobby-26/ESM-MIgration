...
public partial class PlannedMaintenanceDefectListRegister : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbarNoonReporttap.AddImageButton("../PlannedMaintenance/PlannedMaintenanceDefectListRegister.aspx", "Export to Excel", "icon_xls.png", "Excel");
            toolbarNoonReporttap.AddImageLink("javascript:CallPrint('gvDefectList')", "Print Grid", "icon_print.png", "PRINT");
            toolbarNoonReporttap.AddImageButton("../PlannedMaintenance/PlannedMaintenanceDefectListRegister.aspx", "Find", "search.png", "FIND");
            toolbarNoonReporttap.AddImageButton("../PlannedMaintenance/PlannedMaintenanceDefectListRegister.aspx", "Clear Filter", "clear-filter.png", "CLEAR");
            ...
            toolbarNoonReporttap.AddFontAwesomeButton("javascript:showDialog('Add');", "Add", "<i class=\"fa fa-plus-circle\"></i>", "ADD");
    ...
    protected void gvDefectList_ItemDataBound(Object sender, GridItemEventArgs e)
    {
        if (e.Item is GridDataItem)
        {
            ...
            if (verify != null && drv["FLDSTATUSCODE"].ToString() != "CMT")
            {
            ...
            if (postpone != null)
            {
                if (drv["FLDSTATUSCODE"].ToString() != "CMT")
                {
            ...
            if (drv["FLDWORKORDERREQUIRED"].ToString() == "1" && drv["FLDSTATUSCODE"].ToString() != "CMT")
            {
                if (verify != null)
                {
                    verify.Visible = false;
            ...
            if (del != null && drv["FLDSTATUSCODE"].ToString() != "CMT")
            {
    ...
    protected void ShowExcel()
    {
        try
        {
            ...
            string[] alColumns = { "FLDDEFECTNO", "FLDCOMPONENTNUMBER", "FLDVIQCODE", "FLDCOMPONENTNAME", "FLDDETAILS", "FLDJOBNO", "FLDJOBPRIORITY", "FLDACTIONTAKEN", "FLDACTIONREQUIRED", "FLDDDJOBYN", "FLDREQUISITIONNO", "FLDJOBISSUEDDATE", "FLDDUEDATE", "FLDDONEDATE", "FLDRESPONSIBILITY", "FLDSTATUSNAME" };
            string[] alCaptions = { "Defect No", "Component No", "VIQ Code","Component Name", "Defect Details","Job No","Priority","Interim Action Taken(SI and HT comment)","Action Required","DD Job(Y/N)","Req No.","Issue Date","Due Date","Completion Date", "Responsibility", "Status" };
            ...
            ds = PhoenixPlannedMaintenanceDefectJob.DefectJobSearch(General.GetNullableString(txtComponentNumber.Text),
    ...
    protected void MenuDivWorkOrder_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            //else if (CommandName.ToUpper().Equals("CHART"))
            //{
            //    string compname = "";
            //    string valuelist = "";
            //    string colorlist = "";
            //    DataTable dt = PhoenixPlannedMaintenanceDefectJob.ComponentWiseDefect();
            ...
            //    }
            //    RadScriptManager.RegisterStartupScript(this, this.GetType(), "chart popup", "var dataSName = [" + compname + "]; var dataValues= [" + valuelist + "]; var colourList = [" + colorlist + "]; callDia('Equipment wise Defect', '', 'PMS',dataSName,dataValues,colourList);", true);
            //}
        }
    ...
    protected void gvDefectList_ItemCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
            ...
            else if (e.CommandName.ToUpper().Equals("EDIT"))
            {
                ...
                string status = ((RadLabel)item.FindControl("lblStatusCode")).Text;
                if (status != "CMT")
                {
