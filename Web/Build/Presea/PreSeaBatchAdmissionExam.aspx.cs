...
public partial class PreSeaBatchAdmissionExam : PhoenixBasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            courseid = Filter.CurrentPreSeaCourseMasterSelection;
            BatchId  = Filter.CurrentPreSeaCourseMasterBatchSelection;
            if (!IsPostBack)
            {
                SessionUtil.PageAccessRights(this.ViewState);
                PhoenixToolbar toolbar = new PhoenixToolbar();
                toolbar.AddImageButton("../PreSea/PreSeaBatchAdmissionExam.aspx", "Export to Excel", "icon_xls.png", "Excel");
                toolbar.AddImageLink("javascript:CallPrint('gvPreSeaExam')", "Print Grid", "icon_print.png", "PRINT");
                toolbar.AddImageButton("../PreSea/PreSeaBatchAdmissionExam.aspx", "Find", "search.png", "FIND");
    ...
    protected void ShowExcel()
    {
        ...
        ds = PhoenixPreSeaBatchAdmissionExam.PreSeaBatchAdmissionExamSearch(General.GetNullableInteger(Filter.CurrentPreSeaCourseMasterSelection)
    ...
    private void BindData()
    {
        ...
        DataSet ds = PhoenixPreSeaBatchAdmissionExam.PreSeaBatchAdmissionExamSearch(General.GetNullableInteger(Filter.CurrentPreSeaCourseMasterSelection)
                  , General.GetNullableInteger(Filter.CurrentPreSeaCourseMasterBatchSelection)
                  , General.GetNullableInteger(ucExam.SelectedExam)
                  , sortexpression
                  , sortdirection
    ...
    private void InsertPreSeaExam(int courseid, int BatchId, int examid, int semesterid, int subjectid, int? type, decimal maxmarks, decimal passmarks, int activeyn)
    {
       PhoenixPreSeaBatchAdmissionExam.InsertPreSeaBatchAdmissionExam(PhoenixSecurityContext.CurrentSecurityContext.UserCode
            , courseid
            , BatchId
            , examid
            , semesterid
    ...
    {
        PhoenixPreSeaBatchAdmissionExam.UpdatePreSeaBatchAdmissionExam(PhoenixSecurityContext.CurrentSecurityContext.UserCode
            , courseexamid
            , examid
            , semesterid
            , subjectid
    ...
    {
        PhoenixPreSeaBatchAdmissionExam.DeletePreSeaBatchAdmissionExam(PhoenixSecurityContext.CurrentSecurityContext.UserCode, courseexamid);
    }

    private bool IsValidPreSeaCourseExam(string courseid,string BatchId, string examid, string semesterid, string sujectid, string maxmark, string passmark)
    {
    ...
    protected void gvPreSeaExam_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName.ToUpper().Equals("ADD"))
            {
                ...
                if (!IsValidPreSeaCourseExam(courseid
                    , BatchId
                    , examid
                    , semesterid
                    , subjectid
                ...
                InsertPreSeaExam(int.Parse(courseid)
                    , int.Parse(BatchId)
                    , int.Parse(examid)
    ...
    protected void gvPreSeaExam_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.Footer)
        {
            ...
            if (ucSem != null)
            {
                DataSet ds = PhoenixPreSeaBatchAdmissionSemester.ListPreSeaBatchAdmissionSemester(General.GetNullableInteger(Filter.CurrentPreSeaCourseMasterSelection),General.GetNullableInteger(Filter.CurrentPreSeaCourseMasterBatchSelection), null);
    ...
    protected void ucSemesterAdd_TextChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                DataSet ds = PhoenixPreSeaBatchAdmissionSubjects.ListPreSeaBatchAdmissionSubject(General.GetNullableInteger(Filter.CurrentPreSeaCourseMasterSelection), General.GetNullableInteger(Filter.CurrentPreSeaCourseMasterBatchSelection), General.GetNullableInteger(sem));
