...
public partial class Presea_PreSeaConfirmedCandidates : PhoenixBasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbar.AddButton("Generate Letter", "SHOWREPORT");
            PreSeaQuery.AccessRights = this.ViewState;
            PreSeaQuery.MenuList = toolbar.Show();

            PhoenixToolbar toolbarsub = new PhoenixToolbar();
            toolbarsub.AddImageButton("../PreSea/PreSeaConfirmedCandidates.aspx", "Export to Excel", "icon_xls.png", "EXCEL");
            toolbarsub.AddImageButton("../PreSea/PreSeaConfirmedCandidates.aspx", "Filter", "search.png", "FIND");
            toolbarsub.AddImageButton("../PreSea/PreSeaConfirmedCandidates.aspx", "Clear Filter", "clear-filter.png", "CLEAR");
        ...
        try
        {
            ...
            if (dce.CommandName.ToUpper().Equals("SHOWREPORT"))
            {
                ...
                else
                {
                    ...
                    {
                        string script = "parent.Openpopup('Bank','','../Reports/ReportsViewWithSubReport.aspx?applicationcode=10&reportcode=OFFERLETTER&showword=no&showexcel=no&candidateslist=" + candidates + "&batch=" + ucBatch.SelectedBatch + "');";
                        ScriptManager.RegisterStartupScript(this, typeof(Page), "openScript", script, true);
                    }
                    else
                    {
                        string script = "parent.Openpopup('Bank','','../Reports/ReportsView.aspx?applicationcode=10&reportcode=ADMISSIONLETTER&showword=no&showexcel=no&candidateslist=" + candidates + "&batch=" + ucBatch.SelectedBatch + "');";
                        ScriptManager.RegisterStartupScript(this, typeof(Page), "openScript", script, true);
                    }
                }
            }
