...
public partial class PreSeaCourseFees : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                toolbar.AddImageButton("../PreSea/PreSeaCourseFees.aspx", "Export to Excel", "icon_xls.png", "Excel");
                toolbar.AddImageLink("javascript:CallPrint('gvPreSeaFees')", "Print Grid", "icon_print.png", "PRINT");
                toolbar.AddImageButton("../PreSea/PreSeaCourseFees.aspx", "Find", "search.png", "FIND");
    ...
    protected void ShowExcel()
    {
        ...
        string[] alColumns = { "FLDPRESEACOURSENAME", "FLDFEESNAME", "FLDAMOUNT", "FLDPAYTERM" };
        string[] alCaptions = { "Course Name", "Fees Name", "Amount", "Pay term" };
        ...
        ds = PhoenixPreSeaCourseFees.PreSeaCourseFeesSearch(General.GetNullableInteger(Filter.CurrentPreSeaCourseMasterSelection), txtFeesName.Text
        ...
        Response.Write("<td><h3>Pre Sea Course Fees</h3></td>");
    ...
    private void BindData()
    {
        ...
        General.SetPrintOptions("gvPreSeaFees", "Pre Sea Course Fees", alCaptions, alColumns, ds);
    ...
    private void InsertPreSeaCourseFees(int feesid, int courseid, decimal? amount, int? payterm, decimal? waiveramount)
    {
        PhoenixPreSeaCourseFees.InsertPreSeaCourseFees(PhoenixSecurityContext.CurrentSecurityContext.UserCode,feesid,courseid,amount,payterm,waiveramount);
    }

    private void UpdatePreSeaCourseFees(int coursefeeid, int feesid , int courseid , decimal? amount, int? payterm, decimal? waiveramount)
    {
        PhoenixPreSeaCourseFees.UpdatePreSeaCourseFees(PhoenixSecurityContext.CurrentSecurityContext.UserCode, coursefeeid , feesid, courseid, amount, payterm, waiveramount);
    }

    private void deletePreSeaFees(int coursefeeid)
    {
        PhoenixPreSeaCourseFees.DeletePreSeaCourseFees(PhoenixSecurityContext.CurrentSecurityContext.UserCode, coursefeeid);
    }

    private bool IsValidPreSeaCoursefees(int? feesid)
    {
    ...
    protected void gvPreSeaFees_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            {
                if (!IsValidPreSeaCoursefees(General.GetNullableInteger((((UserControlPreSeaFees)_gridView.FooterRow.FindControl("ddlFeesAdd")).SelectedFees))))
                {
                    ucError.Visible = true;
                    return;
                }
                InsertPreSeaCourseFees(
                     int.Parse(((UserControlPreSeaFees)_gridView.FooterRow.FindControl("ddlFeesAdd")).SelectedFees),
                     int.Parse(ddlCourse.SelectedCourse),
            ...
            {
                if (!IsValidPreSeaCoursefees(General.GetNullableInteger((((UserControlPreSeaFees)_gridView.Rows[nCurrentRow].FindControl("ddlFeesEdit")).SelectedFees))))
                {
                    ucError.Visible = true;
                    return;
                }
                UpdatePreSeaCourseFees(
                    int.Parse(((Label)_gridView.Rows[nCurrentRow].FindControl("LblCourseFeesIdEdit")).Text),
            ...
            {
                deletePreSeaFees(
                    int.Parse(((Label)_gridView.Rows[nCurrentRow].FindControl("LblCourseFeesId")).Text)
                    );
            }
            BindData();
            SetPageNavigator();
