...
public partial class PreSeaNewApplicantPersonal : PhoenixBasePage
{
    ...
    {
        Response.Redirect("../PreSea/PreSeaNewApplicantPersonalGeneral.aspx?showimportantremarks=yes", false);
    }
        ...
        try
        {
            if (Request.Files["txtFileUpload"].ContentLength > 0)
            {
                if (string.IsNullOrEmpty(ViewState["dtkey"].ToString()))
                {
                    ...
                    PhoenixCommonFileAttachment.InsertAttachment(Request.Files["txtFileUpload"], new Guid(ViewState["attachmentcode"].ToString()), PhoenixModule.PRESEA, null, ".jpg,.png,.gif", string.Empty, "PRESEAAPPLICANTPHOTO");
                }
                else
                {
                    PhoenixCommonFileAttachment.UpdateAttachment(Request.Files["txtFileUpload"], new Guid(ViewState["dtkey"].ToString()), PhoenixModule.PRESEA, ".jpg,.png,.gif");
                }
            }
        ...
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (dta.Rows.Count > 0)
                {
                    ...
                    //this.Response.AddCacheDependency(ViewState["dtkey"].ToString());
