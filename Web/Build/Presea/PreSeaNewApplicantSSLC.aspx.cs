...
public partial class PreSeaNewApplicantSSLC : PhoenixBasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                BindMarks();
                ucAcademicBoard.Focus();
                rblMarksGrade_Changed(null, null);

              Page.ClientScript.RegisterStartupScript(
              typeof(PreSeaNewApplicantSSLC),
              "ScriptDoFocus",
    ...
    private void BindMarks()
    {
        ...
        try
        {
            ...
            if (ViewState["ACADEMICTYPE"] != null)
                ...
                academictype = ddlCertificate.SelectedQualification;

            if (General.GetNullableInteger(ddlCertificate.SelectedQualification) == null)
            {
                DataSet ds1 = new DataSet();
                DataTable dt1 = new DataTable();
                dt1 = PhoenixRegistersDocumentQualification.ListQualifications(",SSLC,");
                ds1.Tables.Add(dt1.Copy());
                ddlCertificate.QualificationList = ds1;
                ddlCertificate.SelectedQualificationName = "SSLC";
            }
            academictype = ddlCertificate.SelectedQualification;
            ...
            )
    {
        ...
        if (institution == "" || institution == "Dummy" || General.GetNullableString(txtInstituteName.Text) == null)
        {
            ucError.ErrorMessage = "School,Others Names  required";
        ...
        try
        {
                ...
                ))
            {
                if (ViewState["EMPLOYEEACADEMICID"] != null)
                {
                    ...
                    UpdateSSLCMarks(int.Parse(ViewState["EMPLOYEEACADEMICID"].ToString()));
                    BindMarks();
                }
                else
                {
    ...
    private void InsertSSLCMarks(int? academicid)
    {
        GridView _gridView = gvMarks;
        if (_gridView.Rows.Count > 0)
        {
    ...
    private void UpdateSSLCMarks(int academicid)
    {
        GridView _gridView = gvMarks;
    ...
    private void ShowSSLCDetails()
    {
        try
        {
            string qualify = "";
            DataTable dt = PhoenixRegistersDocumentQualification.ListQualifications(",SSLC,");
            if (dt.Rows.Count > 0)
            {
                qualify = dt.Rows[0]["FLDQUALIFICATIONID"].ToString();
                ddlCertificate.SelectedQualification = dt.Rows[0]["FLDQUALIFICATIONID"].ToString();
    ...
    protected void gvMarks_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            ...
            else if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (lblTotal != null)
                    totalmarks = lblTotal.Text;
                if (lblAvg != null)
                {                    
                    overallpercentage = lblAvg.Text;
                }
            }
            else if (e.Row.RowType == DataControlRowType.Footer)
            {
                txtTotal.Text = totalmarks.ToString();
                txtPercentage.Text = overallpercentage.ToString();
            }
        }
        catch (Exception ex)
        {
