...
public partial class PreSeaTraineeSection : PhoenixBasePage
{
	protected void Page_Load(object sender, EventArgs e)
	{
		try
		{
			...
			toolbarsub.AddImageButton("../PreSea/PreSeaTraineeSection.aspx", "Export to Excel", "icon_xls.png", "Excel");
			toolbarsub.AddImageLink("javascript:CallPrint('gvSection')", "Print Grid", "icon_print.png", "PRINT");
			if (!IsPostBack)
			{
				...
				PreSeaQuery.SelectedMenuIndex = 1;
			...
			if (ddlSection.SelectedValue != "Dummy" && ViewState["confirmedyn"].ToString()!="1")
			{
				toolbarsub.AddImageLink("javascript:Openpopup('codehelp1','','../PreSea/PreSeaTraineeSelectedList.aspx?sectionid=" + ddlSection.SelectedValue + "&batchid=" + ddlBatch.SelectedBatch + "');", "Add", "add.png", "ADDTRAINEE");
				
			}
			MenuPreSeaSection.AccessRights = this.ViewState;
			MenuPreSeaSection.MenuList = toolbarsub.Show();
			BindData();
			SetPageNavigator();
		}
		catch (Exception ex)
	...
	protected void PreSeaQuery_TabStripCommand(object sender, EventArgs e)
	{
		...
		{
			Response.Redirect("..\\PreSea\\PreSeaTraineePractical.aspx?batchid=" + General.GetNullableInteger(ddlBatch.SelectedBatch)+"&sectionid="+ddlSection.SelectedValue, false);
		}

	}
	protected void PreSeaSection_TabStripCommand(object sender, EventArgs e)
	{
		DataListCommandEventArgs dce = (DataListCommandEventArgs)e;
	...
	private void BindData()
	{
		...
		DataSet ds = PhoenixPreSeaTrainee.PreSeaTraineeSectionSearch(General.GetNullableInteger(ddlBatch.SelectedBatch),
		...
		General.SetPrintOptions("gvSection", "Section Mapping", alCaptions, alColumns, ds);
	...
	protected void OnBatchChanged(object sender, EventArgs e)
	{
			...
			toolbarsub.AddImageButton("../PreSea/PreSeaTraineeSection.aspx", "Export to Excel", "icon_xls.png", "Excel");
			toolbarsub.AddImageLink("javascript:CallPrint('gvSection')", "Print Grid", "icon_print.png", "PRINT");
			if (ddlSection.SelectedValue != "Dummy")
			{
				toolbarsub.AddImageLink("javascript:Openpopup('codehelp1','','../PreSea/PreSeaTraineeSelectedList.aspx?sectionid=" + ddlSection.SelectedValue + "&batchid=" + ddlBatch.SelectedBatch + "')", "Add", "add.png", "ADDTRAINEE");
			}
			MenuPreSeaSection.AccessRights = this.ViewState;
	...
	protected void ShowExcel()
	{
		...
		Response.AddHeader("Content-Disposition", "attachment; filename=PreSeaTraineeSectionWiseList.xls");
		Response.ContentType = "application/vnd.msexcel";
		Response.Write("<TABLE BORDER='0' CELLPADDING='2' CELLSPACING='0' width='100%'>");
		Response.Write("<tr><td colspan='" + (alColumns.Length).ToString() + "'><h3><center>Section Mapping</center></h3></td></tr>");
	...
	protected void gvSection_RowDeleting(object sender, GridViewDeleteEventArgs de)
	{
		try
		{
			...
			PhoenixPreSeaTrainee.DeletePreSeaTraineeSection(PhoenixSecurityContext.CurrentSecurityContext.UserCode,
