...
public partial class PurchasePOIssued : PhoenixBasePage
{
    string vesselname;
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbar.AddImageButton("../Purchase/PurchasePOIssued.aspx", "Find", "search.png", "FIND");
            toolbar.AddImageButton("../Purchase/PurchasePOIssued.aspx", "Clear Filter", "clear-filter.png", "CLEAR");
            toolbar.AddImageButton("../Purchase/PurchasePOIssued.aspx", "Export to Excel", "icon_xls.png", "Excel");
    ...
    protected void ShowExcel()
    {
        ...
        ds = PhoenixPurchaseSearch.POIssuedSearch(vesselname, txtFormNo.Text.Trim(),
              txtTitle.Text.Trim(), General.GetNullableDateTime(txtFromDate.Text)
        ...
        Response.AddHeader("Content-Disposition", "attachment; filename=POIssued.xls");
        ...
        Response.Write("<h3><center>PO Issued</center></h3></td>");
        Response.Write("</tr>");
    ...
    protected void rgvForm_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
    {
        ...
        DataSet ds = new DataSet();
        ds = PhoenixPurchaseSearch.POIssuedSearch(General.GetNullableString(vesselname), txtFormNo.Text.Trim(), txtTitle.Text.Trim()
    ...
    protected void rgvForm_ItemDataBound(object sender, Telerik.Web.UI.GridItemEventArgs e)
    {
        if (e.Item is GridDataItem)
        {
            ...
            ImageButton imgInvoice = (ImageButton)item["ACTION"].FindControl("imgInvoice");

            RadLabel lblInvoiceNumber = (RadLabel)item.FindControl("lblInvoiceNumber");
            RadLabel lblInvoice = (RadLabel)item.FindControl("lblInvoice");
            ...
            ImageButton cmdshippedquantity = (ImageButton)item["ACTION"].FindControl("cmdshippedquantity");
            if (cmdshippedquantity != null)
                cmdshippedquantity.Attributes.Add("onclick", "openNewWindow('detail', '', 'Purchase/PurchaseShippedQuantity.aspx?orderid=" + drv["FLDORDERID"] + "&VesselId=" + drv["FLDVESSELID"] + "&StockType=" + drv["FLDSTOCKTYPE"] + "&Callfrom=PURCHASE" + "'); return false;");
        }
    }

    protected void rgvForm_ItemCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
    {
        if (e.CommandName.ToUpper().Equals("INVOICE"))
        {
            GridDataItem item = (GridDataItem)e.Item;
            PhoenixAccountsVoucher.InvoiceNumber = ((RadLabel)item.FindControl("lblInvoiceNumber")).Text;
            NameValueCollection criteria = new NameValueCollection();
            criteria.Clear();
            criteria.Add("txtInvoiceNumberSearch", PhoenixAccountsVoucher.InvoiceNumber);
            Filter.CurrentInvoiceSelection = criteria;
            Response.Redirect("../Accounts/AccountsInvoiceAdjustmentMaster.aspx");
        }
    }
