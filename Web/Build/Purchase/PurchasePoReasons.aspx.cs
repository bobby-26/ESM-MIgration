...
public partial class PurchasePoReasons : PhoenixBasePage
{
    string vesselname;
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbar.AddImageButton("../Purchase/PurchasePoReasons.aspx", "Find", "search.png", "FIND");
            toolbar.AddImageButton("../Purchase/PurchasePoReasons.aspx", "Clear Filter", "clear-filter.png", "CLEAR");
            toolbar.AddImageButton("../Purchase/PurchasePoReasons.aspx", "Export to Excel", "icon_xls.png", "Excel");
    ...
    protected void ShowExcel()
    {
        ...
        DataSet ds = new DataSet();
        string[] alColumns = { "FLDSTOCKTYPE", "FLDFORMNO", "FLDTITLE", "FLDSUBACCOUNT", "FLDVENDORNAME", "FLDPRIORITYVENDOR", "FLDPORTNAME", "VENDORPRIORITY", "FLDREASON", "FLDREASONMSG" };
        string[] alCaptions = { "Type", "Number", "Title", "Budget Code", "Vendor","Priority Vendor","Port","Priority" ,"Reason","Reason Message"};
        ...
        ds = PhoenixPurchasePOReasons.POReasonSearchXL(vesselname, txtFormNo.Text.Trim(),
              txtTitle.Text.Trim()
              , General.GetNullableDateTime(txtFromDate.Text), General.GetNullableDateTime(txtToDate.Text),General.GetNullableInteger(ddlmulticolumnport.SelectedValue)
              , ddlStockType.SelectedValue,
              sortexpression, sortdirection, 1, rgvForm.VirtualItemCount > 0 ? rgvForm.VirtualItemCount : 10,
                  ref iRowCount, ref iTotalPageCount);

        Response.AddHeader("Content-Disposition", "attachment; filename=POReasons.xls");
        ...
        Response.Write("<h3><center>PO Reasons</center></h3></td>");
        Response.Write("</tr>");
    ...
    protected void rgvForm_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
    {
        ...
        string[] alColumns = { "FLDSTOCKTYPE", "FLDFORMNO", "FLDTITLE", "FLDSUBACCOUNT", "FLDVENDORNAME" };
        string[] alCaptions = { "Type", "Number", "Title", "Budget Code", "Vendor" };
        ...
        DataSet ds = new DataSet();
        ds = PhoenixPurchasePOReasons.POReasonSearch(General.GetNullableString(vesselname), txtFormNo.Text.Trim(), txtTitle.Text.Trim()
                                                                , General.GetNullableDateTime(txtFromDate.Text)
                                                                , General.GetNullableDateTime(txtToDate.Text)
                                                                , General.GetNullableInteger(ddlmulticolumnport.SelectedValue)
        ...
        General.SetPrintOptions("rgvForm", "PO Reasons", alCaptions, alColumns, ds);
    }
    ...
    protected void rgvForm_ItemDataBound(object sender, Telerik.Web.UI.GridItemEventArgs e)
    {
        if (e.Item is GridDataItem)
        {
            ...
            ImageButton cmdPortlist = (ImageButton)item["ACTION"].FindControl("cmdPortlist");
            if (cmdPortlist != null)
                cmdPortlist.Attributes.Add("onclick", "openNewWindow('detail', '', 'Purchase/PurchaseSupplierPortPOReasonlist.aspx?orderid=" + drv["FLDORDERID"] + "'); return false;");
