...
public partial class Registers_RegisterDocumentFlagEndorsementList : PhoenixBasePage
{
    ...
    {
        DataTable dt = PhoenixRegisterDocumentRankGroup.DocumentFlagEndorementList(General.GetNullableInteger(ViewState["DOCUMENTCOURSEID"].ToString()), ViewState["TYPE"].ToString());
    ...
    protected void MenuRankList_TabStripCommand(object sender, EventArgs e)
    {
        ...
        if (CommandName.ToUpper().Equals("SAVE"))
        {
            ...
            {
                PhoenixRegisterCrewList.UpdateDocumentCourseFlagEndorseList(PhoenixSecurityContext.CurrentSecurityContext.UserCode
                          , Int16.Parse(ViewState["DOCUMENTCOURSEID"].ToString()), selectedalternatecourse);
            }
            if (ViewState["TYPE"].ToString().ToUpper().Equals("LICENSE"))
            {

                PhoenixRegisterCrewList.UpdateDocumentLicenseEndorseList(
                                 Int16.Parse(ViewState["DOCUMENTCOURSEID"].ToString()), selectedalternatecourse);
            }
