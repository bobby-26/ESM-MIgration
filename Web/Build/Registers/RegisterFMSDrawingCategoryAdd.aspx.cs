...
public partial class RegisterFMSDrawingCategoryAdd : PhoenixBasePage
{
    ...
    private void BindCategory()
    {
        ...
        {
            DataSet dt = PhoenixRegisterFMSDrawing.FMSDrawingCategoryEdit(Guid.Parse(ViewState["categoryid"].ToString()));

            if (dt.Tables[0].Rows.Count > 0)
            {
                DataRow dr = dt.Tables[0].Rows[0];
                txtcode.Text = dr["FLDDRAWINGCODE"].ToString();
                txtcode.ReadOnly = true;
                txtname.Text = dr["FLDDRAWINGCATEGORY"].ToString();
            }
        }
    }
    protected void MenuRegistersTypeofChangeCategoryAdd_TabStripCommand(object sender, EventArgs e)
    {
        ...
        if (CommandName.ToUpper().Equals("UPDATE"))
        {
            ...
            if (Request.QueryString["categoryid"] != null && Request.QueryString["categoryid"].ToString() != string.Empty)
            {
                //PhoenixRegisterFMSDrawing.FMSDrawingCategoryInsert(PhoenixSecurityContext.CurrentSecurityContext.UserCode, new Guid(ViewState["categoryid"].ToString()), txtname.Text, txtcode.Text);
            }
            else
            {
                //PhoenixRegisterFMSDrawing.FMSDrawingCategoryInsert(PhoenixSecurityContext.CurrentSecurityContext.UserCode, null, txtname.Text, txtcode.Text);
            }
        }
