...
public partial class Registers_RegisterProfessionalConductVesselType : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                txtevent.Text = ViewState["ID"].ToString();
                
                DataTable dt = PhoenixRegisterProfessionalConductVesselType.EditProfessionalConduct(General.GetNullableInteger(ViewState["ID"].ToString()));
                if (dt.Rows.Count > 0)
                {
                    txtevent.Text = dt.Rows[0]["FLDCATEGORYDESC"].ToString();
                    txtmeasure.Text = dt.Rows[0]["FLDCONDUCTQUESTION"].ToString();
                }
                BindSource();
                BindMapping();
            }
    ...
    {
        DataSet ds = PhoenixRegisterProfessionalConductVesselType.PersonalProfileVesselTypeMappingList();
    ...
    protected void BindMapping()
    {
        ...
        {
            DataSet ds = PhoenixRegisterProfessionalConductVesselType.GetprofessionalConductVesselTypeMappingList(
        ...
        try
        {
            ...
            if (CommandName.ToUpper().Equals("SAVE"))
            {
                ...
                if (strvesseltypeid.Length > 1)
                {
                    ...
                    PhoenixRegisterProfessionalConductVesselType.InsertProfessionalConductVesselTypeMapping(
