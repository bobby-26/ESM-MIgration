...
public partial class Registers_RegistersAdminAssetHardwareAssembly : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbarlist.AddImageLink("javascript:Openpopup('Filter','','RegistersAdminAssetFilter.aspx'); return false;", "Asset Search", "search.png", "SEARCH");
            toolbarlist.AddImageLink("javascript:Openpopup('Filter','','RegistersAdminDisposedAssetFilter.aspx'); return false;", "Disposed Asset Search", "search.png", "DISPOSEDASSETSEARCH");
            toolbarlist.AddImageButton("../Registers/RegistersAdminAssetHardwareAssembly.aspx", "Export to Excel", "icon_xls.png", "EXCEL");
            toolbarlist.AddImageButton("../Registers/RegistersAdminAssetHardwareAssembly.aspx", "Clear Filter", "clear-filter.png", "RESET");
    ...
    private void BindAssembly()
    {
        try
        {
              ...
              string[] alColumns = { "FLDNAME", "FLDSERIALNO", "FLDASSETTYPENAME",   "FLDMAKER", "FLDIDENTIFICATIONNUMBER", "FLDZONENAME" };
              string[] alCaptions = { "Name", "Serial Number", "Asset",   "Maker", "Model", "Zone" };
            ...
            else
            {
                ds = PhoenixAdministrationAsset.SearchAsset(null, null, sortdirection, sortexpression, Int32.Parse(ViewState["PAGENUMBER"].ToString()),
                                                            ...
                                                            null, null, null, null, null);        //Type-Hardware 
            }
            ...
            if (General.GetNullableGuid(ViewState["ASSETID"].ToString()) == null)
            {
                ...
                ViewState["NAME"] = ds.Tables[0].Rows[0]["FLDNAME"].ToString(); 
                ViewState["DISPOSALDATE"] = ds.Tables[0].Rows[0]["FLDDISPOSALDATE"].ToString();
    ...
    protected void Asset_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                if (!IsValidAssetId(ViewState["ASSETID"].ToString(),ViewState["DISPOSALDATE"].ToString()))
                {
                    ucError.Visible = true;
                    return;
                }
                Response.Redirect("../Registers/RegistersAdminAssetHardwareItem.aspx?ASSETID=" + ViewState["ASSETID"].ToString() + "&PGNO=" + ViewState["PAGENUMBER"].ToString() + "&NAME=" + ViewState["NAME"].ToString() + "&ZONEID=" + ViewState["ZONEID"].ToString(), false);
            }
        }
        catch (Exception ex)
        {
    ...
    private void BindData()
    {
        try
        {
            int iRowCount = 0;
            int iTotalPageCount = 0;
            string[] alColumns = { "FLDASSETTYPENAME", "FLDSERIALNO", "FLDNAME", "FLDMAKER", "FLDIDENTIFICATIONNUMBER", "FLDZONENAME" };
            string[] alCaptions = { "Asset", "Serial Number", "Name", "Maker", "Model", "Zone" };
    ...
    protected void BindAssetType()
    {
        ...
        DataTable dt = PhoenixRegistersAssetType.ListAssetType(1, 1);   //Type- Hardware, ItemType - Assembly
    ...
    protected void MenuAdminAssetAdd_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                if (!IsValidAsset(ddlAssetType.SelectedValue, txtAssetName.Text, txtMaker.Text, txtSerialNo.Text, ddlLocation.SelectedZone))
                {
                    ucError.Visible = true;
                    return;
                }
    ...
    private bool IsValidAsset(string assesttypeid, string AssetName, string maker, string SerialNo, string Location)
    {
    ...
    protected void ShowExcel()
    {
        ...
        Response.Write("<td><h3>Hardware</h3></td>");
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (cmdMap != null)
            {
                if (drv["FLDITEMTYPE"].ToString() == "1")
                {
                    ...
                    cmdMap.Attributes.Add("onclick", "Openpopup('codehelp1', '', '../Registers/RegistersAdminAssetMapping.aspx?ASSETID=" + drv["FLDASSETID"].ToString() + "&ASSETNAME=" + drv["FLDNAME"] + "');return true;");
                    cmdMap.Visible = true;
                }
                else
                {
            ...
            ImageButton cmdUserMap = (ImageButton)e.Row.FindControl("cmdUserMapping");
            {
                if (drv["FLDITEMTYPE"].ToString() == "1")
                {
                    cmdUserMap.Visible = SessionUtil.CanAccess(this.ViewState, cmdMap.CommandName);
                    cmdUserMap.Attributes.Add("onclick", "Openpopup('codehelp1', '', '../Registers/RegistersAdminAssetUserMapping.aspx?ASSETID=" + drv["FLDASSETID"].ToString() + "&ASSETNAME=" + drv["FLDNAME"] + "&ZONEID=" + drv["FLDCOMPANYID"]  + "');return true;");
                    cmdUserMap.Visible = true;
                }
                else
                {
                    cmdUserMap.Visible = SessionUtil.CanAccess(this.ViewState, cmdMap.CommandName);
                    cmdUserMap.Visible = false;
                }
            }
        }
    }
    ...
    protected void gvAdminAsset_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName.ToUpper().Equals("USERMAPPING"))
            {
                ...
                ViewState["NAME"] = ((Label)gvAdminAsset.Rows[Int32.Parse(e.CommandArgument.ToString())].FindControl("lblName")).Text;
                ViewState["DISPOSALDATE"] = ((Label)gvAdminAsset.Rows[Int32.Parse(e.CommandArgument.ToString())].FindControl("lblDisposalDate")).Text;
            ...
            if (e.CommandName.ToUpper().Equals("SOFTWAREMAPPING"))
            {
                ViewState["ASSETID"] = ((Label)gvAdminAsset.Rows[Int32.Parse(e.CommandArgument.ToString())].FindControl("lblAdminAssetID")).Text;
                ViewState["PARENTID"] = ((Label)gvAdminAsset.Rows[Int32.Parse(e.CommandArgument.ToString())].FindControl("lblAssetTypeId")).Text;
                ViewState["NAME"] = ((Label)gvAdminAsset.Rows[Int32.Parse(e.CommandArgument.ToString())].FindControl("lblName")).Text;
            ...
            if (e.CommandName.ToUpper().Equals("DELETE"))
            {
                ...
                ucConfirm.ErrorMessage = "Are you sure want to Dispose this Asset?";
    ...
    private bool IsValidDelete(string DisposalDate)
    {
        ...
        if (General.GetNullableDateTime(DisposalDate) == null)
            ucError.ErrorMessage = "Before Disposing Asset, Please provide Disposal date and Disposal reason.";
