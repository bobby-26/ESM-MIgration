...
public partial class Registers_RegistersAdminAssetSoftwareItem : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            toolbarlist.AddImageButton("../Registers/RegistersAdminAssetSoftwareItem.aspx", "Export to Excel", "icon_xls.png", "EXCEL");
    ...
    private void BindAssembly()
    {
        try
        {
            ...
            string[] alCaptions = { "Item", "Serial Number", "Name", "Media Part No", "Identification No" };
    ...
    protected void Asset_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                Response.Redirect("../Registers/RegistersAdminAssetSoftware.aspx?ASSETID=" + ViewState["ASSETPARENTID"].ToString() + "&PGNO=" + ViewState["PGNO"].ToString(), false);
            }
    ...
    private bool IsValidAsset(string assesttypeid, string AssetName, string maker, string SerialNo)
    {
        ...
        if (SerialNo.Trim().Equals(""))
            ucError.ErrorMessage = "Serial No is Required.";
    ...
    protected void ShowExcel()
    {
        ...
        General.SetPrintOptions("gvAdminAssetItems", "Item", alCaptions, alColumns, ds);
    ...
    protected void gvAdminAssetItems_RowDataBound(Object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (cmdMap != null)
            {
                if (drv["FLDITEMTYPE"].ToString() == "2")
                {
                    cmdMap.Visible = SessionUtil.CanAccess(this.ViewState, cmdMap.CommandName);
                    cmdMap.Attributes.Add("onclick", "Openpopup('codehelp1', '', '../Registers/RegistersAdminAssetSoftwareList.aspx?ASSETTYPEID=" + drv["FLDASSETTYPEID"].ToString() + "&SOFTWARENAME=" + drv["FLDNAME"].ToString() + "&ZONEID=" + ViewState["ZONEID"].ToString() + "');return true;");
                    cmdMap.Visible = true;
                }
                else
                {
