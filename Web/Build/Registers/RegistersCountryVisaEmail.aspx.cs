...
public partial class RegistersCountryVisaEmail : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                PhoenixCrew2XL.Export2XLCountryVisa(int.Parse(ViewState["VISAID"].ToString()), path);
    ...
    protected void CorrespondenceEmail_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            if (CommandName.ToUpper().Equals("SEND"))
            {
                ...
                PhoenixMail.SendMail(txtTO.Text.Trim(), txtCC.Text.Trim(), txtCC.Text.Trim(), txtSubject.Text.Trim(), txtBody.Content
    ...
    {
        DataSet ds = PhoenixRegistersCountryVisaDocument.CountryVisaSearchByVisaId(int.Parse(ViewState["VISAID"].ToString()));
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            strbody = strbody + "Please find attached the details of " + dt.Rows[0]["FLDCOUNTRYNAME"].ToString() + " " + dt.Rows[0]["FLDVISATYPENAME"].ToString() + " visa for your reference.";
            strbody = strbody + "<br/>";
            strbody = strbody + "<br/>";
            strbody = strbody + "<br/>";

            strbody = strbody + "Thanks and best Regards";
            strbody = strbody + "<br/>";
            strbody = strbody + PhoenixSecurityContext.CurrentSecurityContext.UserName + ".";
            strbody = strbody + "<br/>";
            strbody = strbody + HttpContext.Current.Session["companyname"].ToString();

        }

        txtBody.Content = strbody;

        txtSubject.Text = dt.Rows[0]["FLDCOUNTRYNAME"].ToString() + " " + dt.Rows[0]["FLDVISATYPENAME"].ToString() + " visa details";
    }
