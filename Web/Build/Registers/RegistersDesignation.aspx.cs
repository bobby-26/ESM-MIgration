...
public partial class RegistersDesignation : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
	{
		try
		{
			...
			toolbar.AddFontAwesomeButton("../Registers/RegistersDesignation.aspx", "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "Excel");
			toolbar.AddFontAwesomeButton("javascript:CallPrint('gvDesignation')", "Print Grid", "<i class=\"fas fa-print\"></i>", "PRINT");
			toolbar.AddFontAwesomeButton("../Registers/RegistersDesignation.aspx", "Find", "<i class=\"fas fa-search\"></i>", "FIND");
            toolbar.AddFontAwesomeButton("../Registers/RegistersDesignation.aspx", "Clear Filter", "<i class=\"fas fa-eraser\"></i>", "CLEAR");
    ...
    protected void ShowExcel()
	{
		...
		ds = PhoenixRegistersDesignation.DesignationSearch(txtCode.Text,txtName.Text, sortexpression, sortdirection,
			1,
			iRowCount,
			ref iRowCount,
			ref iTotalPageCount);
		...
		Response.Write("<td><h3> Designation </h3></td>");
    ...
    {
        gvDesignation.SelectedCellIndexes.Clear();
        gvDesignation.EditIndexes.Clear();
	...
	private void BindData()
	{
        ...
        {
            gvDesignation.Columns[2].Visible = false;
		...
		General.SetPrintOptions("gvDesignation", "Designation", alCaptions, alColumns, ds);
    ...
    protected void gvDesignation_ItemCommand(object sender, GridCommandEventArgs e)
	{
		try
		{
			...
			{
                ViewState["DesignationId"] = ((RadLabel)e.Item.FindControl("lblDesignationId")).Text;
	...
	{
        PhoenixRegistersDesignation.InsertDesignation(PhoenixSecurityContext.CurrentSecurityContext.UserCode,
			 code, name);
	}

	private void UpdateDesignation(int designationid, string code, string name)
	{
		PhoenixRegistersDesignation.UpdateDesignation(PhoenixSecurityContext.CurrentSecurityContext.UserCode,
	...
	private bool IsValidDesignation(string code,string name)
	{
		...
		RadGrid _gridView = gvDesignation;
	...
	{
		PhoenixRegistersDesignation.DeleteDesignation(PhoenixSecurityContext.CurrentSecurityContext.UserCode, Designationcode);
	}
    protected void gvDesignation_SortCommand(object sender, Telerik.Web.UI.GridSortCommandEventArgs e)
    {
    ...
    protected void ucConfirmDelete_Click(object sender, EventArgs e)
    {
        try
        {
            DeleteDesignation(Int32.Parse(ViewState["DesignationId"].ToString()));
            Rebind();
        }
        catch (Exception ex)
        {
