...
public partial class RegistersNoonReportRangeConfig : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        toolbar.AddFontAwesomeButton("../Registers/RegistersNoonReportRangeConfig.aspx", "Export to Excel", "<i class=\"fas fa-file-excel\"></i>", "Excel");
        ...
        try
        {
            if (!IsPostBack)
            {
                ...
                ucFromVessel.DataBind();
                ucFromVessel.bind();
                   

                gvNRConfig.PageSize = int.Parse(PhoenixGeneralSettings.CurrentGeneralSetting.Records);
                cmdRevision.Attributes.Add("onclick", "openNewWindow('codehelp','', '" + Session["sitepath"] + "/Registers/RegistersNoonreportRangeConfigRevision.aspx?VesselId=" + PhoenixSecurityContext.CurrentSecurityContext.VesselID + "');return true;");
            }
            revisiondetail();
        }
        catch (Exception ex)
    ...
    protected void ShowExcel()
    {
        ...
        string[] alColumns = { "FLDDISPLAYTEXT", "FLDVESSELNAME", "FLDMINVALUE", "FLDMAXVALUE", "FLDREQUIREDYESNO", "FLDALERTLEVEL", "FLDMAXALERTLEVEL", "FLDREQUIREDINVPSYN", "FLDREQUIREDINLOGYN" };
        string[] alCaptions = { "Field Name", "Vessel Name", "Min Value", "Max Value", "Active Y/N", "Min Alert Level", "Max Alert Level", "Required in VPS", "Required in LOG" };
    ...
    protected void imgbtnSend_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            if (General.GetNullableInteger(ucFromVessel.SelectedVessel) == null)
            {
                ucError.ErrorMessage = "From Vessel is required";
                ucError.Visible = true;
                return;
            }

            PhoenixRegistersNoonReportRangeConfig.NoonReportRangeConfigCopy(PhoenixSecurityContext.CurrentSecurityContext.UserCode
