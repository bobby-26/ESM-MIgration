...
public partial class StandardFormFBFormCreate : PhoenixBasePage

{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		try
		{
			...
			//json.Attributes.Add("style", "visibility:hidden");
			SessionUtil.PageAccessRights(this.ViewState);
			PhoenixToolbar toolbarmain = new PhoenixToolbar();
            toolbarmain.AddButton("Back", "BACK", ToolBarDirection.Right);
			toolbarmain.AddButton("Save", "SAVE",ToolBarDirection.Right);
			
            menuFormbuilder.AccessRights = this.ViewState;
			menuFormbuilder.MenuList = toolbarmain.Show();
            menuFormbuilder.SelectedMenuIndex = 0;
    ...
    protected void menuFormbuilder_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            if (CommandName.ToUpper().Equals("SAVE"))
            {
                ...
                string jsonschema = LZString.decompressFromUTF16(renderJson.Value.ToString());
                PhoenixFormBuilder.FormJsonUpdate(new Guid(ViewState["FormId"].ToString())
                                                 , jsonschema, frmType.Value, signature.Value.TrimEnd('`'));
                ...
                {
                    PhoenixDocumentManagementForm.FormDesignRevisionUpdate(
                        PhoenixSecurityContext.CurrentSecurityContext.UserCode
                        , new Guid(ViewState["FormId"].ToString())
                        , General.GetNullableGuid(ViewState["formrevisionid"].ToString())
                        , jsonschema);
                ...
                ViewState["string"] = ds.Tables[0].Rows[0]["FLDJSONSCHEMA"].ToString();


            }
            if (CommandName.ToUpper().Equals("BACK"))
            {
                if (ViewState["DMSFORMTYPE"].ToString() == "DMSForm")
                {
                    Response.Redirect("../DocumentManagement/DocumentManagementFormList.aspx?FORMID=" + ViewState["FORMID"]);
                }
                else
                {
                    Response.Redirect("StandardFormFBformsList.aspx?cat=" + ViewState["category"].ToString());
                }
            }
