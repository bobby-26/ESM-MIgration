...
public partial class StandardFormRecordOfNoxVerification : PhoenixBasePage
{
    ...
    private void LoadVesselData(Guid WorkOrderId)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            if (dt.Rows[0]["FLDENGINETYPE"].ToString() != DBNull.Value.ToString() && dt.Rows[0]["FLDMAKE"].ToString() != DBNull.Value.ToString())
                txtEngineTyeMake.Text = dt.Rows[0]["FLDENGINETYPE"].ToString() + "&" + dt.Rows[0]["FLDMAKE"].ToString();
            else
                txtEngineTyeMake.Text = dt.Rows[0]["FLDENGINETYPE"].ToString() + dt.Rows[0]["FLDMAKE"].ToString();           
