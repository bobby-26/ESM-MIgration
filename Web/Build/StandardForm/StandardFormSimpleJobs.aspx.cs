...
public partial class StandardFormSimpleJobs : PhoenixBasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {           
            ...
            if (!IsPostBack)
            {
                ...
                rblComments_SelectedIndexChanged(null, null);
                if (Request.QueryString["mode"] == null && (ViewState["DONEID"] == null || ViewState["DONEID"].ToString() == string.Empty))
                {
                    SetPrimaryDetails(new Guid(ViewState["WORKORDERID"].ToString()));
                }
            }
        }
        catch (Exception ex)
    ...
    private void SetPrimaryDetails(Guid gWorkOrderId)
    {
        DataSet ds = PhoenixPlannedMaintenanceWorkOrderReport.EditWorkOrderReport(gWorkOrderId);
        DataTable dt = ds.Tables[0];
        if (dt.Rows.Count > 0)
        {
            txtComponentNumber.Text = dt.Rows[0]["FLDCOMPONENTNUMBER"].ToString();
            txtComponentName.Text = dt.Rows[0]["FLDCOMPONENTNAME"].ToString();           
            txtRunnigHrs.Text = dt.Rows[0]["FLDCURRENTVALUES"].ToString();
        }
    }
    protected void rblComments_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (rblComments.SelectedValue == "1" || rblComponentCondition.SelectedValue =="3")
        {
        ...
        {           
            ...
            txtComments.CssClass = "input readonlytextbox";
        }
    }   
