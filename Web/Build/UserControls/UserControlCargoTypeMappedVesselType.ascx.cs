...
public partial class UserControlCargoTypeMappedVesselType : System.Web.UI.UserControl
{
    string showyesno = "1";
    ...
    public void bind()
    {
        ddlCargoType.DataSource = PhoenixRegistersCargo.ListCargoTypeMappedVesselType(General.GetNullableInteger(showyesno), vesselid);
        ddlCargoType.DataBind();
        foreach (RadComboBoxItem item in ddlCargoType.Items)
        {
            if (item.Value == _selectedValue.ToString())
