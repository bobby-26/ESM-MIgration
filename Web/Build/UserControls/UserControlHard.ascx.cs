...
public partial class UserControlHard : System.Web.UI.UserControl
{
    int hardtypecode;
    ...
    public void bind()
    {
        try
        {
            ddlHard.DataSource = PhoenixRegistersHard.ListHard(hardtypecode, sortbyshortname ? byte.Parse("1") : byte.Parse("0"), csvShortName, Pool);
            ddlHard.DataBind();
            foreach (RadComboBoxItem item in ddlHard.Items)
            {
                if (item.Value == _selectedValue.ToString())
