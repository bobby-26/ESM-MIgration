...
public partial class UserControlRAFrequencyExtn : System.Web.UI.UserControl
{
    ...
    private void bind()
    {
        ddlFrequency.DataSource = PhoenixInspectionRiskAssessmentFrequencyExtn.ListRiskAssessmentFrequency(Convert.ToInt32(_typeid));
        ddlFrequency.DataBind();
        foreach (RadComboBoxItem item in ddlFrequency.Items)
        {
            if (item.Value == _selectedValue.ToString())
