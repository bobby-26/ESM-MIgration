...
public partial class UserControlRAHazardTypeExtn : System.Web.UI.UserControl
{
    ...
    private void bind()
    {
        ddlHazardType.DataSource = PhoenixInspectionRiskAssessmentHazardExtn.ListRiskAssessmentHazard(Convert.ToInt32(_typeid), 0);
        ddlHazardType.DataBind();
        foreach (RadComboBoxItem item in ddlHazardType.Items)
        {
            if (item.Value == _selectedValue.ToString())
