...
public partial class UserControlRASubHazardTypeExtn : System.Web.UI.UserControl
{
    ...
    {
        cblSubHazardType.SelectedIndex = -1;
        cblSubHazardType.DataSource = PhoenixInspectionRiskAssessmentSubHazardExtn.ListRiskAssessmentSubHazard(General.GetNullableInteger(mainhazardid));
        cblSubHazardType.DataBind();
        foreach (RadListBoxItem item in cblSubHazardType.Items)
        {
            if (item.Value == _selectedValue.ToString())
