...
public partial class VesselAccountsBondSubsidyIssue : PhoenixBasePage
{
    ...
    protected void gvCrewSearch_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            UserControlMaskNumber cans = ((UserControlMaskNumber)e.Row.FindControl("txtSubsidyQtyEdit"));
            if (cans != null && (drv["FLDEMPLOYEEID"].ToString() == "-1" || drv["FLDEMPLOYEEID"].ToString() == "-2"))
            {
                cans.Enabled = false;
    ...
    protected void gvCrewSearch_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        try
        {
            ...
            Guid id = (Guid)_gridView.DataKeys[nCurrentRow].Value;
            string empid = ((Label)gr.FindControl("lblEmpId")).Text;
            ...
            string subqty = ((UserControlMaskNumber)gr.FindControl("txtSubsidyQtyEdit")).Text;
            if (!IsValidIssue(empid, id.ToString(), date, quantity, rob, oldqty))
            {
                ucError.Visible = true;
                return;
            }
            PhoenixVesselAccountsStoreIssue.UpdateStoreIssue(id, DateTime.Parse(date), decimal.Parse(quantity), remarks, General.GetNullableDecimal(subqty)
    ...
    private void CreateMenu()
    {
        ...
        toolbar.AddImageButton("../VesselAccounts/VesselAccountsBondSubsidyIssue.aspx", "Export to Excel", "icon_xls.png", "Excel");
        toolbar.AddImageLink("javascript:CallPrint('gvCrewSearch')", "Print Grid", "icon_print.png", "PRINT");
        toolbar.AddImageLink("javascript:showPickList('spnPickListStore', 'codehelp1', '', 'VesselAccountsBondSubsidyIssueSelection.aspx?storeclass=" + ddlHard.SelectedHard + "&accountfor=" + ddlEmployee.SelectedEmployee + "', true); return false;", "Store Item", "add.png", "ADDSTORE");
        toolbar.AddImageButton("../VesselAccounts/VesselAccountsBondSubsidyIssue.aspx", "Find", "search.png", "FIND");
