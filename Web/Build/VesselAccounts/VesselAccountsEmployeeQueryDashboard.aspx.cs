...
public partial class VesselAccountsEmployeeQueryDashboard : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["dcode"] != null && Request.QueryString["dcode"].ToString() != "")
                    ViewState["DCODE"] = Request.QueryString["dcode"].ToString();

                if (Request.QueryString["dvessel"] != null && Request.QueryString["dvessel"].ToString() != "")
                    ViewState["DVESSELID"] = Request.QueryString["DVESSELID"].ToString();

                if (Request.QueryString["vslid"] != null && Request.QueryString["vslid"].ToString() != "")
                    ViewState["DVESSELID"] = Request.QueryString["vslid"].ToString();
    ...
    protected void CrewQueryMenu_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            if (CommandName.ToUpper().Equals("EXCEL"))
            {
                ...
                int vesselid = ViewState["DVESSELID"] != null && General.GetNullableInteger(ViewState["DVESSELID"].ToString()) != null ? int.Parse(ViewState["DVESSELID"].ToString()) : PhoenixSecurityContext.CurrentSecurityContext.VesselID;


                if (ViewState["DCODE"] != null && ViewState["DCODE"].ToString() == "NCCM")
                    dt = PhoenixVesselAccountsRH.DashboardCrewListNCCM(vesselid);
                else if (ViewState["DCODE"] != null && ViewState["DCODE"].ToString() == "RHWM")
                    dt = PhoenixVesselAccountsRH.DashboardCrewListRHWM(vesselid);
                else if (ViewState["DCODE"] != null && ViewState["DCODE"].ToString() == "RHWH")
                    dt = PhoenixVesselAccountsRH.DashboardCrewListRHWH(vesselid);
                else if (ViewState["DCODE"] != null && ViewState["DCODE"].ToString() == "RHWS")
                    dt = PhoenixVesselAccountsRH.DashboardCrewListRHWS(vesselid);
                else if (ViewState["DCODE"] != null && ViewState["DCODE"].ToString() == "VSWHS")
                    dt = PhoenixVesselAccountsRH.DashboardCrewListVSWHS(vesselid);
                else if (ViewState["DCODE"] != null && ViewState["DCODE"].ToString() == "NCCM2")
                    dt = PhoenixVesselAccountsRH.DashboardCrewListNCCM2(vesselid);

                else
                    dt = PhoenixVesselAccountsRH.DashboardCrewList(vesselid, ucDate.SelectedDate, null
                                                                            , sortexpression, sortdirection
                                                                            , gvCrewSearch.CurrentPageIndex + 1, 1000
    ...
    protected void gvCrewSearch_ItemCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
            ...
            else if (e.CommandName.ToUpper().Equals("CREWATTENDANCE"))
            {
                ...
                dcode = ViewState["DCODE"] != null ? ViewState["DCODE"].ToString() : "";
                ...
                if (PhoenixSecurityContext.CurrentSecurityContext.InstallCode != 0)
                {
                    ...
                    if (General.GetNullableGuid(RHStartId.Text) != null)
                    {
                        ...
                        string scriptpopup = String.Format("javascript:parent.openNewWindow('code1','','" + Session["sitepath"] + "/VesselAccounts/VesselAccountsRHWorkCalender.aspx?EMPID=" + EmpId.Text + "&RESTHOURSTARTID=" + RHStartId.Text + "&MODELYN=Y&SHIPSTARTCALENDARID=" + ShipCalendarId.Text + "&RESTHOUREMPLOYEEID=" + RHEmpId.Text + "&DCODE="+dcode+"');");
    ...
    protected void gvCrewSearch_ItemDataBound(object sender, GridItemEventArgs e)
    {
        ...
        if (e.Item is GridDataItem)
        {
            ...
            if (sg != null && General.GetNullableInteger(drv["FLDSIGNONOFFID"].ToString()) != null && General.GetNullableInteger(drv["FLDSIGNONOFFID"].ToString()) > 0)
            {
                if (!SessionUtil.CanAccess(this.ViewState, sg.CommandName)) 
                    sg.Visible = false;
