...
public partial class VesselAccountsExtraMealsEdit :  PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            MenuExtraMealsEdit.AccessRights = this.ViewState;
            ...
            if ((Request.QueryString["purpose"] == "EXTRAMEALSEDIT") && (Request.QueryString["extramealsid"] != null))
            {
                ...
                MenuExtraMealsEdit.MenuList = toolbar.Show();
    ...
    protected void MenuExtraMealsEdit_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
    ...
    protected void UpdateExtraMeals(Guid id, string accounttype, string fromdate, string todate, string mandays, string rate, string servedto, string isvictualrate)
    {
        ...
        PhoenixVesselAccountsExtraMeals.UpdateExtraMeals(id
                  , PhoenixSecurityContext.CurrentSecurityContext.VesselID
                  , Convert.ToDateTime(fromdate)
                  , Convert.ToDateTime(todate)
                  , Int32.Parse(accounttype)
                  , int.Parse(mandays.ToString())
                  , General.GetNullableDecimal(rate)
                  , servedto
                  , Int16.Parse(isvictualrate)
                 );
    }
    ...
    private bool IsValidExtraMeals(
          string accounttype
        , string fromdate
        , string todate
        , string mandays
