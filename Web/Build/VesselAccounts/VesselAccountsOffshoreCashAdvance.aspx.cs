...
public partial class VesselAccountsOffshoreCashAdvance :  PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                ddlEmployee_TextChangedEvent(sender, e);
            }
            BindData();
        }
        catch (Exception ex)
    ...
    {
        PhoenixToolbar toolbar = new PhoenixToolbar();
        toolbar.AddImageButton("../VesselAccounts/VesselAccountsOffshoreCashAdvance.aspx", "Export to Excel", "icon_xls.png", "Excel");
        ...
        if (!title.Contains("Onboard"))
        {
            ...
            {
                toolbar.AddImageLink("javascript:return showPickList('spnPickListStore', 'codehelp1', '', '../Common/CommonOffshorePickListEarningDeduction.aspx?m=" + month + "&y=" + year + "&t=" + title + "', true);", title, "add.png", "ADDSTORE");
            }
        }
    ...
    protected void ucConfirm_OnClick(object sender, EventArgs e)
    {
        if (sender != null && ((UserControlConfirmMessage)sender).confirmboxvalue == 1)
        {
            ...
            string signonoff = nvc["signonoff"].ToString();

            UpdategvAmount(id.Value, General.GetNullableInteger(employee)
                      , General.GetNullableGuid(accit), General.GetNullableInteger(wageheadid), byte.Parse(month), int.Parse(year)
                      , General.GetNullableDateTime(date), int.Parse(earningordeduction), decimal.Parse(amount), purpose, General.GetNullableInteger(signonoff));
