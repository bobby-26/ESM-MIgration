...
public partial class VesselAccountsPhoneCardAdmin :  PhoenixBasePage
{
    ...
    protected void OrderForm_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                ViewState["CURRENTTAB"] = "../VesselAccounts/VesselAccountsPhoneCardPriceCorrection.aspx";
            }
            else if (dce.CommandName.ToUpper().Equals("PHCISSUE"))
            {
                ViewState["CURRENTTAB"] = "../VesselAccounts/VesselAccountsPhoneCardIssue.aspx";
            }
    ...
    private void ResetMenu()
    {
        try
        {
            ...
            toolbar.AddButton("Issue of Phone Card", "PHCISSUE");
    ...
    protected void SetTabHighlight()
    {
        try
        {
            ...
            if (dl.Items.Count > 0)
            {
                ...
                else if (ViewState["CURRENTTAB"].ToString().Trim().Contains("VesselAccountsPhoneCardPriceCorrection.aspx"))
                {
    ...
    protected void gvPhonReq_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName == "CORRECT")
            {
                ...
                ViewState["REQUESTID"] = requestid;
                if (!reqstatus.Contains("Req")) ViewState["ALLOWEDIT"] = "false";
                PhoenixVesselAccountsCorrections.DeleteVesselAccountPhoneCardDuplicateLineItem(PhoenixSecurityContext.CurrentSecurityContext.VesselID, new Guid(requestid));
                ViewState["REQUESTID"] = null;
                //BindData();
            }
