...
public partial class VesselAccountsReportCaptainCash : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            MenuReportCaptainCash.AccessRights = this.ViewState;
            MenuReportCaptainCash.MenuList = toolbar.Show();
            if (!IsPostBack)
            {
    ...
    protected void MenuReportCaptainCash_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            if (CommandName.ToUpper().Equals("SHOWREPORT"))
            {
                if (IsValidDates(txtFromDate.Text, txtToDate.Text))
                {
                    ...
                    ifMoreInfo.Attributes["src"] = ViewState["REPORTPAGEURL"].ToString() + "?applicationcode=7&reportcode=CAPTAINCASH&showmenu=false&showexcel=no&showword=no&vesselid=" + PhoenixSecurityContext.CurrentSecurityContext.VesselID.ToString() + "&FromDate=" + txtFromDate.Text + "&ToDate=" + txtToDate.Text;
                }
                else
                {
                    ucError.Visible = true;
