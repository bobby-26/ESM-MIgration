...
public partial class VesselAccountsReportProvision : PhoenixBasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            MenuReportProvision.AccessRights = this.ViewState;
            MenuReportProvision.MenuList = toolbar.Show();
            if (!IsPostBack)
            {
    ...
    protected void MenuReportProvision_TabStripCommand(object sender, EventArgs e)
    {
        try
        {
            ...
            if (CommandName.ToUpper().Equals("SHOWREPORT"))
            {
                ...
                {
                    ifMoreInfo.Attributes["src"] = ViewState["REPORTPAGEURL"].ToString() + "?applicationcode=7&reportcode=PROVISION&showmenu=false&showexcel=no&showword=no&vesselid=" + PhoenixSecurityContext.CurrentSecurityContext.VesselID.ToString() + "&FromDate=" + txtFromDate.Text + "&ToDate=" + txtToDate.Text;
                }
                else
                {
                    ucError.Visible = true;
