...
public partial class VesselPositionArrivalReviewMail : PhoenixBasePage
{
    ...
    protected void bind()
    {
        DataSet ds = PhoenixVesselPositionNoonReport.ArivalReportReviewList(General.GetNullableInteger(Request.QueryString["VesselId"]), General.GetNullableGuid(Request.QueryString["NoonReportID"]));
        string html = "";
        html = "<html>Dear Captain,<br /><br />Your Arrival Report of " + DateTime.Parse( ds.Tables[1].Rows[0]["FLDDATE"].ToString()).ToString("dd MMM yyyy") + " has been well received. We note the following parameters are not within limits.<br /><br />";
        ...
        try
        {
            ...
            if (CommandName.ToUpper().Equals("MAIL"))
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    string subject = "Arrival Report " + DateTime.Parse(ViewState["date"].ToString()).ToString("dd MMM yyyy hh:mm") + "- Review";
