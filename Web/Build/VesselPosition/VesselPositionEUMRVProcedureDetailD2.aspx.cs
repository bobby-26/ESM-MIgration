...
public partial class VesselPositionEUMRVProcedureDetailD2 : PhoenixBasePage
{
        ...
        try
        {
            ...
            if (CommandName.ToUpper().Equals("SAVE"))
            {
                    PhoenixVesselPositionEUMRV.InsertEUMRVProcedureDetail(PhoenixSecurityContext.CurrentSecurityContext.UserCode,
                                                                            ...
                                                                            null, General.GetNullableString(txtMethodTreat.Text.Trim()), null, null);
    ...
    private void ProcedureDetailEdit()
    {
        ...
        {
            DataSet ds = PhoenixVesselPositionEUMRV.EUMRVProcedureEdit(General.GetNullableGuid(Request.QueryString["ProcedureID"].ToString()));
            DataTable dt = ds.Tables[0];
            if (dt.Rows.Count > 0)
            {
                txtVersion.Text = dt.Rows[0]["FLDVERSION"].ToString();
                ViewState["PROCEDUREID"] = dt.Rows[0]["FLDEUMRVPROCEDUERID"].ToString();
                string Guidance = General.GetNullableString(dt.Rows[0]["FLDGUIDANCE"].ToString()) != null ? " (" + dt.Rows[0]["FLDGUIDANCE"].ToString() + ")" : "";
                lblProceduretxt.Text = dt.Rows[0]["FLDCODE"].ToString() + "-" + dt.Rows[0]["FLDPROCEDURE"].ToString() + Guidance;

            }
            ViewState["PROCEDUREID"] = Request.QueryString["ProcedureID"].ToString();
