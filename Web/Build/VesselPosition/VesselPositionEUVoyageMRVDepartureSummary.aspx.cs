...
public partial class VesselPositionEUVoyageMRVDepartureSummary : PhoenixBasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                toolbar.AddImageButton("../VesselPosition/VesselPositionEUVoyageMRVDepartureSummary.aspx", "Export to Excel", "icon_xls.png", "Excel");
                toolbar.AddImageLink("javascript:CallPrint('gvConsumption')", "Print Grid", "icon_print.png", "PRINT");
                toolbar.AddImageButton("../VesselPosition/VesselPositionEUVoyageMRVDepartureSummary.aspx", "Search", "search.png", "SEARCH");
                toolbar.AddImageButton("../VesselPosition/VesselPositionEUVoyageMRVDepartureSummary.aspx", "Clear-Filter", "clear-filter.png", "CLEAR");
    ...
    protected void MenuDepartureReport_TabStripCommand(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("../VesselPosition/VesselPositionEUVoyageMRVSummary.aspx");
        }
    }
        ...
        try
        {
            ...
            if (e.CommandName.ToUpper().Equals("EMAIL"))
            {
                ...
                if (ds.Tables[0].Rows.Count > 0 && General.GetNullableString(ds.Tables[0].Rows[0]["FLDTOMAIL"].ToString()) != null && General.GetNullableString(ds.Tables[0].Rows[0]["FLDDATAFORMAT"].ToString()) != null)
                {
                    ...
                    string subject = "EUMRV Port-emissions";

                    StringBuilder emailbody = new StringBuilder();
                    emailbody.Append("Good Day,");
                    emailbody.AppendLine("\n");
                    emailbody.Append("Please find attached EUMRV Port Emission of " + dt.Rows[0]["FLDVESSELNAME"].ToString());
                    emailbody.AppendLine(" ");
                    ...
                    if (dt.Rows[0]["FLDDATAFORMAT"].ToString().ToUpper() == "XML")
                        Filepath = ConvertVoyageXML((((Label)_gridView.Rows[nCurrentRow].FindControl("lblDepartureSummaryID")).Text), int.Parse(((Label)_gridView.Rows[nCurrentRow].FindControl("lblVesselid")).Text), dt.Rows[0]["FLDVESSELNAME"].ToString());
                    if (dt.Rows[0]["FLDDATAFORMAT"].ToString().ToUpper() == "EXCEL")
                        Filepath = SaveVoyageEXCEL((((Label)_gridView.Rows[nCurrentRow].FindControl("lblDepartureSummaryID")).Text), int.Parse(((Label)_gridView.Rows[nCurrentRow].FindControl("lblVesselid")).Text), dt.Rows[0]["FLDVESSELNAME"].ToString());
                    if (dt.Rows[0]["FLDDATAFORMAT"].ToString().ToUpper() == "PDF")
                        Filepath = SaveVoyagePDF((((Label)_gridView.Rows[nCurrentRow].FindControl("lblDepartureSummaryID")).Text), int.Parse(((Label)_gridView.Rows[nCurrentRow].FindControl("lblVesselid")).Text), dt.Rows[0]["FLDVESSELNAME"].ToString());
    ...
    private void BindData()
    {
        ...
        string[] alColumns = { "FLDVESSELNAME", "FLDVOYAGENO", "FLDSEAPORTNAME", "FLDARRIVALDATE", "FLDDEPARTUREDATE", "FLDTOTALTIMESPENTINPORT", "FLDCARGOONARRIVAL", "FLDCARGOLOADED", "FLDCARGODISCHARGED", "FLDCARGOONDEPARTURE", "FLDAGGCO2EMISSION" };
        string[] alCaptions = { "Vessel", "Voy No.", "Port", "Arrival", "Departure", "Time In Port", "On Arrival", "Loaded", "Discharged", "On Departure", "Agg CO₂ Emitted (T-CO₂)" };
        ...
        DataSet ds = PhoenixVesselPositionEUMRVSummaryReport.EUMRVDepartureSummarySearch(General.GetNullableInteger(PhoenixSecurityContext.CurrentSecurityContext.VesselID == 0 ? ((nvc.Get("ddlVessel") == null) ? "" : nvc.Get("ddlVessel").ToString()) : PhoenixSecurityContext.CurrentSecurityContext.VesselID.ToString())
                                                                                , sortexpression
                                                                                 , sortdirection
