...
public partial class VesselPositionShiftingReportOperations : PhoenixBasePage
{
        ...
        if (Filter.CurrentVPRSShiftingReportSelection != null)
        {
            ...
            if (ds.Tables.Count > 0)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    chbCargoOperationExists.Checked = General.GetNullableInteger(dr["FLDCARGOOPERATIONYN"].ToString()) == null ? false : true;
        ...
        try
        {
            ...
            if (CommandName.ToUpper().Equals("LOAD"))
            {
                String scriptpopup = String.Format(
                 "javascript:parent.openNewWindow('codehelp1', '', '"+Session["sitepath"]+"/VesselPosition/VesselPositionShiftingCargoOperations.aspx?vesselid="
    ...
    private bool IsValidOperation()
    {
        ...
        if (General.GetNullableDateTime(ViewState["DepDate"].ToString()) < General.GetNullableDateTime(blstcommence))
            ucError.ErrorMessage = "Commenced Ballasting Date/Time cannot be later than Shifting Date/Time.";
        ...
        if (General.GetNullableDateTime(ViewState["DepDate"].ToString()) < General.GetNullableDateTime(blstcomplete))
            ucError.ErrorMessage = "Completed Ballasting Date/Time cannot be later than Shifting Date/Time.";
        ...
        if (General.GetNullableDateTime(ViewState["DepDate"].ToString()) < General.GetNullableDateTime(deblstcommence))
            ucError.ErrorMessage = "Commenced Deballasting Date/Time cannot be later than Shifting Date/Time.";
        ...
        if (General.GetNullableDateTime(ViewState["DepDate"].ToString()) < General.GetNullableDateTime(deblstcomplete))
            ucError.ErrorMessage = "Completed Deballasting Date/Time cannot be later than Shifting Date/Time.";
    ...
    private bool IsValidDepartureReport()
    {
            ...
            if (e.Item is GridEditableItem)
            {
                ...
                if (cmdOperation != null)
                    cmdOperation.Attributes.Add("onclick",
                        "openNewWindow('codehelp1', '', '" + Session["sitepath"] + "/VesselPosition/VesselPositionShiftingCargoOperationsDetailEdit.aspx?operationid=" + lblOperationId.Text
                        + "&vesselid=" + lblVesselid.Text + "&voyageid=" + lblVoyageid.Text + "');return false;");
    ...
    protected void gvOtherOpr_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
    {
        DataSet ds = PhoenixVesselPositionDepartureReport.ListDepartureOtherOperation(
           General.GetNullableInteger(ViewState["VESSELID"].ToString())
           , General.GetNullableGuid(Filter.CurrentVPRSShiftingReportSelection == null ? "" : Filter.CurrentVPRSShiftingReportSelection));
